...
async function generateImageHandler(args, { requestId, executor }) {
	...
	const nodeConfig = {
		...
		...(typeof imageCount === 'number'
			? { imageCount, count: imageCount, numImages: imageCount }
			: {}),
...
const mockExec = {
	callTool: async (name, args) => {
		...
		if (name === 'list_node_tasks') {
			...
			if (n < 2) return { tasks: [] }
			return {
				tasks: [{ nodeId: 'n1', status: 'completed', createdAt: Date.now(), outputFiles: [tmpIn] }]
			}
		}
...
assert(
	fs.existsSync(expPath) && expPath.endsWith('origin.png') && expPath.startsWith(tmpOutDir),
	`exported at correct location: ${path.relative(tmpDir, expPath)}`
)
...
assert(
	order.filter((n) => n === 'list_node_tasks').length >= 2,
	'list_node_tasks polled >= 2 times (first empty, then completed)'
)
...
assert(
	createArgs.config.prompt === 'A cute red panda sitting on bamboo',
	'create_node config.prompt forwarded'
)
assert(
	createArgs.config.width === 1024 && createArgs.config.height === 1024,
	'create_node config.width/height forwarded'
)
assert(
	createArgs.config.negativePrompt === 'ugly, blurry' &&
		createArgs.config.negative_prompt === 'ugly, blurry',
	'create_node config.negativePrompt dual-keyed'
)
assert(
	createArgs.config.model === 'sdxl-turbo' && createArgs.config.modelKey === 'sdxl-turbo',
	'create_node config.model dual-keyed'
)
assert(createArgs.config.seed === 1337, 'create_node config.seed forwarded')
assert(
	createArgs.config.imageCount === 1 &&
		createArgs.config.count === 1 &&
		createArgs.config.numImages === 1,
	'create_node config.imageCount triple-keyed'
)
assert(
	Array.isArray(createArgs.config.references) &&
		createArgs.config.references[0] === '/tmp/a.png' &&
		createArgs.config.referenceImages[1] === '/tmp/b.jpg',
	'references dual-keyed'
)
		...
		executor: {
			callTool: async (name) => {
				...
				if (name === 'list_node_tasks')
					return {
						tasks: [
							{ nodeId: 'n2', status: 'completed', createdAt: Date.now(), outputFiles: [tmpIn] }
						]
					}
				if (name === 'get_node_info') return { outputFiles: [tmpIn] }
				return {}
...
assert(
	r2.ok === true && r2.exportedFiles[0] === tmpOutFileSingle,
	`outputPath treated as single file (exported=${r2.exportedFiles[0]})`
)
