...
describe('buildPublicObjectUrl', () => {
	...
	it('encodes Chinese and special characters in filenames', () => {
		expect(
			buildPublicObjectUrl('bucket', 'oss-cn-beijing.aliyuncs.com', 'uploads/测试文件.png')
		).toBe(
			'https://bucket.oss-cn-beijing.aliyuncs.com/uploads/%E6%B5%8B%E8%AF%95%E6%96%87%E4%BB%B6.png'
		)
		expect(
			buildPublicObjectUrl('bucket', 'oss-cn-beijing.aliyuncs.com', 'images/photo 1.jpg')
		).toBe('https://bucket.oss-cn-beijing.aliyuncs.com/images/photo%201.jpg')
	...
	it('preserves directory separators while encoding path segments', () => {
		expect(
			buildPublicObjectUrl('bucket', 'oss-cn-beijing.aliyuncs.com', 'a/b/c/file name.txt')
		).toBe('https://bucket.oss-cn-beijing.aliyuncs.com/a/b/c/file%20name.txt')
	})
})
