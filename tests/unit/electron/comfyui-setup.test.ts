...
describe('comfyui setup-service: exported venv path safety', () => {
	let setupSetVenvPath: (
		_ctx: unknown,
		payload: { path?: string }
	) => { ok: boolean; error?: string; venvPath?: string }
	let setupGetDefaultVenvPath: (
		_ctx: unknown,
		payload?: { installPath?: string }
	) => { path: string }
	let setupClearVenv: (
		_ctx: unknown,
		payload?: { venvPath?: string }
	...
	it('setupGetDefaultVenvPath returns userData-based path, not installPath', () => {
		...
		expect(path.resolve(result.path).toLowerCase()).not.toBe(
			path.resolve(testInstallPath).toLowerCase()
		)
	})
	...
	it('setupSetVenvPath rejects venv inside installPath and auto-corrects to safe path', () => {
		...
		expect(venvResolved.toLowerCase() + path.sep).not.toContain(
			installResolved.toLowerCase() + path.sep
		)
