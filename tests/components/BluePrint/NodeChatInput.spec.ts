...
describe('NodeChatInput', () => {
	...
	describe('available references - allows multiple mentions of same asset', () => {
		...
		it('does NOT filter out already selected references by nodeId:anchorId pair - same asset can be mentioned multiple times', async () => {
			const item1: InputParamPreviewRef = {
				kind: 'image',
				label: 'Img',
				fromNodeId: 'n1',
				fromAnchorId: 'a1'
			}
			const item2: InputParamPreviewRef = {
				kind: 'image',
				label: 'Img2',
				fromNodeId: 'n2',
				fromAnchorId: 'a2'
			}
			const selected: InputParamPreviewRef[] = [
				{ kind: 'image', label: 'Img', fromNodeId: 'n1', fromAnchorId: 'a1' }
			]
	...
	describe('paste handling - plain text only', () => {
		it('binds paste event listener to editor', () => {
			...
			const hasPasteBinding =
				editor.attributes('onpaste') !== undefined ||
