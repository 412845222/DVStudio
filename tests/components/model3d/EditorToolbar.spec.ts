...
import type { RenderMode, LightingPreset, TransformMode } from '@/ui/WorkFlow/WorlFlowNodes/model3d/editor/types'
...
describe('EditorToolbar', () => {
  ...
  describe('render mode buttons', () => {
    it('renders pbr, solid-white, normal, unlit buttons but no wireframe mode button', () => {
      ...
      const wireframeBtn = buttons.find(b => b.text() === '线框' && b.classes().includes('m3de-toolbar-btn'))
      expect(wireframeBtn).toBeUndefined()
    })

    it('marks the active render mode button with active class', () => {
      const wrapper = mount(EditorToolbar, { props: { ...defaultProps, currentRenderMode: 'solid-white' } })
      const buttons = wrapper.findAll('.m3de-toolbar-btn')
