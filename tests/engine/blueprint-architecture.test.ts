...
const blueprintEngineFiles = allSourceFiles.filter((f) =>
	f.relativePath.startsWith('src/engine/blueprint/')
)

const graphbaseFiles = allSourceFiles.filter((f) =>
	f.relativePath.startsWith('src/engine/graphbase/')
)

const blueprintHostFiles = allSourceFiles.filter((f) =>
	f.relativePath.startsWith('src/views/AIWorkflow/')
)

function findViolations(
	files: SourceFile[],
	patterns: RegExp[],
	allowedLines: Map<string, number[]> = new Map()
...
describe('🔴 Blueprint Architecture Compliance Tests', () => {
	describe('Rule 1: No direct transform.position assignment in blueprint layer', () => {
		it('should not directly assign to node.transform.position.x/y in blueprint engine files (outside GraphObject base class)', () => {
			...
			const files = blueprintEngineFiles.filter((f) => {
				return (
					!f.relativePath.endsWith('/GraphObject.ts') &&
			...
			const actualViolations = violations.filter((v) => {
				...
				if (v.file.endsWith('BlueprintScene.ts') && v.match.includes('_tempConnection'))
					return false
			...
			expect(
				actualViolations,
				`Found ${actualViolations.length} direct transform.position assignments in blueprint layer`
			).toEqual([])
		})
	})
	...
	describe('Rule 2: No direct data.worldX/worldY assignment in business code', () => {
		it('should not directly assign to node.data.worldX/worldY in host files (use node.setPosition())', () => {
			...
			const files = blueprintHostFiles.filter((f) => {
				return (
					!f.relativePath.includes('workflowStateAdapter') &&
			...
			expect(
				actualViolations,
				`Found ${actualViolations.length} direct data.worldX/worldY assignments in host layer`
			).toEqual([])
		})
	})
	...
	describe('Rule 3: Vuex must not be the source of truth for blueprint drawing state', () => {
		it('selectedNodeIds watch must not trigger syncBlueprintNow/loadBlueprint reverse sync', () => {
			...
			const watchBlocks = content.match(
				/watch\s*\(\s*\(\s*\)\s*=>\s*\[([\s\S]*?)\]\s*,\s*\([^)]*\)\s*=>\s*\{([\s\S]*?)\}/g
			)
			...
			if (watchBlocks) {
				for (const block of watchBlocks) {
					if (
						block.includes('selectedNodeIds') &&
						(syncBlueprintCall.test(block) || /scheduleStoreSyncToEditor/.test(block))
					) {
			...
			expect(
				selectedNodeIdsTriggersSync,
				'watch on selectedNodeIds must not trigger syncBlueprintNow/scheduleStoreSyncToEditor'
			).toBe(false)
		})
	})
	...
	describe('Rule 4: GraphObject.translate() must delegate to setPosition() for polymorphism', () => {
		it('GraphObject.translate should call setPosition to ensure BlueprintNode override works', () => {
			const graphObjectFile = allSourceFiles.find((f) =>
				f.relativePath.endsWith('/graphbase/scene/GraphObject.ts')
			)
			expect(graphObjectFile, 'GraphObject.ts should exist').toBeTruthy()
			if (!graphObjectFile) return

			const translateMatch = graphObjectFile.content.match(
				/translate\s*\([^)]*\)\s*:\s*[^{]*\{([\s\S]*?)\n\s*\}/
			)
			...
			const callsSetPosition =
				/this\.setPosition\s*\(/.test(translateBody) ||
				/return\s+this\.setPosition/.test(translateBody)
			...
			expect(
				callsSetPosition,
				'translate() must call this.setPosition() to ensure subclass overrides are invoked'
			).toBe(true)
		})
	})
	...
	describe('Rule 5: BlueprintNode.setPosition must sync data.worldX/worldY', () => {
		it('BlueprintNode.setPosition should update both transform.position and data.worldX/Y', () => {
			...
			const setPositionMatch = nodeFile.content.match(
				/setPosition\s*\([^)]*\)\s*:\s*[^{]*\{([\s\S]*?)(?=\n\s*public\s+|\n\s*private\s+|\n\s*protected\s+|\n\s*updateSize|\n\s*syncDataFromTransform|\n\s*\}\s*$)/
			)
	...
	describe('Rule 6: isEngineDragging must be set before executeCommand in pointerup', () => {
		it('pointerup NODES path should set isEngineDragging=false before executeCommand to allow emitChange', () => {
			...
			expect(
				allSetFalseBeforeExecute,
				'All MoveNodeCommand executeCommand calls must be preceded by isEngineDragging=false to prevent emitChange from being blocked'
			).toBe(true)
		})
	})
	...
	describe('Rule 7: serialize() must include defensive position sync', () => {
		it('BlueprintScene.serialize() should sync node data from transform before serializing', () => {
			...
			const serializeMatch = content.match(
				/serialize\s*\(\s*\)\s*:\s*BlueprintData\s*\{([\s\S]*?)(?=\n\s*public\s+|\n\s*private\s+|\n\s*serializeLegacy|\n\s*\}\s*$)/
			)
			if (!serializeMatch) {
			...
			const hasDefensiveSync =
				/syncDataFromTransform/.test(body) ||
				/\.data\.worldX\s*=.*transform\.position\.x/.test(body)
			...
			expect(
				hasDefensiveSync,
				'serialize() should defensively sync data.worldX/Y from transform.position as a safety net'
			).toBe(true)
		})
	})
	...
	describe('Rule 8: emitChange must not be blocked by isEngineDragging after command execution', () => {
		it('BlueprintEditor.vue emitChange guard should check isEngineDragging correctly', () => {
			...
			expect(
				hasIsDraggingGuard,
				'emitChange should have isEngineDragging guard to prevent mid-drag syncs'
			).toBe(true)
		})
	})
	...
	describe('Rule 9: loadBlueprint must have dedup protection', () => {
		it('BlueprintScene.loadBlueprint should have signature-based deduplication to prevent redundant rebuilds', () => {
			...
			const loadMatch = content.match(
				/loadBlueprint\s*\([^)]*\)\s*:\s*void\s*\{([\s\S]*?)(?=\n\s*loadLegacyBlueprint|\n\s*addBlueprintNode|\n\s*public\s+|\n\s*private\s+|\n\s*\}\s*$)/
			)
			if (!loadMatch) {
			...
			expect(
				hasDedup,
				'loadBlueprint should have signature-based dedup to prevent redundant node rebuilds'
			).toBe(true)
		})
	})
	...
	describe('Rule 10: isUpdatingFromStore guard on selection change commit', () => {
		it('onBlueprintEditorSelectionChange must set isUpdatingFromStore around Vuex commits', () => {
			...
			const selChangeMatch = content.match(
				/function\s+onBlueprintEditorSelectionChange\s*\([^)]*\)\s*\{([\s\S]*?)(?=\n\s*function\s+|\n\s*const\s+\w+\s*=|\n\s*$)/
			)
			if (!selChangeMatch) {
			...
			expect(
				hasGuard,
				'onBlueprintEditorSelectionChange must set isUpdatingFromStore=true before committing selection changes to Vuex'
			).toBe(true)
		})
	})
	...
	describe('Rule 11: main.ts must pass through Ctrl+Z/Y/Delete for blueprint routes', () => {
		it('window keydown handler in main.ts should skip stopPropagation for AIWorkflow/BlueprintTest routes', () => {
			const mainFile = allSourceFiles.find((f) => f.relativePath === 'src/main.ts')
			...
			expect(
				hasPassThrough,
				'main.ts should pass through keyboard events for blueprint routes instead of stopping propagation'
			).toBe(true)
		})
	})
	...
	describe('Rule 12: loadBlueprint must support incremental updates without clearing command stack', () => {
		it('loadBlueprint should have incremental update path and must NOT call commands.clear() within loadBlueprint', () => {
			...
			const hasIncrementalUpdate =
				/existing\s*=.*_nodeMap\.get/.test(loadBody) || /posChanged|sizeChanged/.test(loadBody)
			...
			expect(
				...
				'loadBlueprint should incrementally update existing nodes instead of full dispose/recreate'
			).toBe(true)
			expect(
				hasCommandsClearInLoad,
				'loadBlueprint must NOT call this.commands.clear() as it destroys undo history'
			).toBe(false)
		})
	})
	...
	describe('Rule 13: syncLoadSignature must be called after command execution/undo/redo', () => {
		it('executeCommand/undo/redo must update _lastLoadSignature to prevent feedback loops', () => {
			...
			const hasSyncMethod =
				/syncLoadSignature\s*\(/.test(content) ||
				/_lastLoadSignature\s*=.*node\.data\.worldX/.test(content)
			...
			expect(
				executeCallsSync,
				'executeCommand must call syncLoadSignature after executing command'
			).toBe(true)
	...
	describe('Rule 14: Scene.setupKeyboardShortcuts must handle Ctrl+Z/Y for undo/redo', () => {
		it('base Scene class should handle Ctrl+Z (undo) and Ctrl+Y/Shift+Z (redo) keyboard shortcuts', () => {
			...
			const hasCtrlZ =
				/ctrl\s*&&\s*key\s*===\s*['"]z['"]/.test(content) ||
				/KeyZ.*ctrlKey|ctrlKey.*KeyZ/.test(content)
	...
	describe('Rule 15: workflowStateToLegacyBlueprint cache must update node coordinates on cache hit', () => {
		it('cache hit path must sync worldX/worldY/x/y/width/height from state nodes to cached result', () => {
			...
			const cacheHitBlock = content.match(
				/if\s*\(\s*_cachedResult\s*&&\s*_cacheKey\s*===\s*structureKey\s*\)\s*\{([\s\S]*?)return\s+_cachedResult\s*;?/
			)
			...
			const hasNodeCoordSync =
				/cachedNode\.worldX\s*=/.test(cacheBody) && /cachedNode\.worldY\s*=/.test(cacheBody)
			const hasLegacyXYUpdate =
				/cachedNode\.x\s*=/.test(cacheBody) && /cachedNode\.y\s*=/.test(cacheBody)
			const hasSizeSync =
				/cachedNode\.width\s*=/.test(cacheBody) && /cachedNode\.height\s*=/.test(cacheBody)
			const loopsOverNodes = /for\s*\(\s*const\s+nodeId\s+of\s+state\.nodeOrder\s*\)/.test(
				cacheBody
			)
			...
			expect(
				loopsOverNodes,
				'cache hit block should iterate state.nodeOrder to update node positions'
			).toBe(true)
			expect(
				hasNodeCoordSync,
				'cache hit block should sync cachedNode.worldX/worldY from current state'
			).toBe(true)
			expect(hasLegacyXYUpdate, 'cache hit block should update legacy x/y fields too').toBe(true)
			expect(
				hasSizeSync,
				'cache hit block should sync cachedNode.width/height from current state'
			).toBe(true)
		})
		...
		it('cache key must NOT rely solely on node order/edges/selection but must invalidate on node count changes', () => {
			...
			expect(
				hasNodeCountInKey,
				'structureKey should include node count to invalidate when nodes are added/removed'
			).toBe(true)
		})
	})
})
