...
const { initCliControlService, shutdownCliControlService } = await import(
	new URL('./../electron/backend/modules/cli-control-server/service.mjs', import.meta.url)
)
const { getCliControlServerToken } = await import(
	new URL('./../electron/backend/modules/cli-control-server/httpServer.mjs', import.meta.url)
)
const { addTaskChangeListener } = await import(
	new URL('./../electron/backend/modules/cli-control-server/taskStore.mjs', import.meta.url)
)
...
async function main() {
	...
	assert(
		'mcp.builtinToolsCount is number',
		typeof h1.data?.mcp?.builtinToolsCount === 'number',
		h1.data?.mcp
	)
	...
	const h3 = await requestJson({
		method: 'GET',
		port,
		path: '/tools',
		token: 'dvs_cli_DEADBEEF_DEADBEEF_DEADBEEF_DEADBEEF'
	})
	...
	const hasGenImg =
		Array.isArray(h4.data?.tools) && h4.data.tools.some((t) => t.name === 'generate_image')
	assert(
		'generate_image tool present',
		hasGenImg,
		h4.data?.tools?.map((t) => t.name)
	)

	// 测试 5: POST /v1/generate-image 参数校验（无 prompt）
	console.log('\n--- 5. generate-image param validation (missing prompt)')
	const h5 = await requestJson({
		method: 'POST',
		port,
	...
	assert(
		'task.payload.prompt matches',
		h8.data?.task?.payload?.prompt === '一只可爱的猫咪',
		h8.data?.task?.payload
	)
	assert(
		'task.payload.outputPath matches',
		h8.data?.task?.payload?.outputPath === 'C:/tmp/out/cat.png',
		h8.data?.task?.payload
	)
	...
	assert(
		'filter status=running → all match',
		r9b.tasks.every((t) => t.status === 'running'),
		r9b.tasks.map((t) => t.status)
	)

	// 测试 10: 任务取消
	console.log('\n--- 10. task cancel')
	const h10 = await requestJson({
		method: 'POST',
		port,
	...
	assert(
		...
		afterCancel.data?.task?.status === 'cancelled',
		afterCancel.data?.task
	)
