...
function assetName(resourceId, resource) {
  ...
  const fromRel = basenameFromAnyPath(resource?.projectRelativePath || resource?.relativePath || resource?.url || resource?.sourcePath)
  return fromRel || String(resourceId || `asset-${Date.now()}`)
}

function assetKind(resource) {
  const raw = String(resource?.kind || '').trim().toLowerCase()
  if (raw === 'image' || raw === 'video' || raw === 'audio' || raw === 'file' || raw === 'model') return raw === 'model' ? 'file' : raw
  return 'file'
}
...
function makeCanonicalAsset(projectId, root, filePath, resourceId, resource, relOverride = '') {
  const rel = canonicalProjectRelativePath(relOverride || path.relative(root, filePath).split(path.sep).join('/'))
  if (!rel) return null
...
async function repairOneProjectAsset(projectId, resourceId, resource) {
  ...
  const explicitRel = canonicalProjectRelativePath(resource.projectRelativePath || resource.relativePath || '')
  ...
  for (const rel of candidates.filter(Boolean)) {
    ...
    if (fileExists(resolved)) return makeCanonicalAsset(pid, root, resolved, resourceId, resource, rel)
  }

  const sourcePath = fileUrlToPath(String(resource.sourcePath || resource.absolutePath || '').trim())
  if (fileExists(sourcePath)) {
    ...
    if (copied?.ok && rel && fileExists(abs)) return makeCanonicalAsset(pid, root, abs, resourceId, resource, rel)
  }
  ...
  if (/^https?:\/\//i.test(sourceUrl)) {
    ...
    if (dl?.ok && rel && fileExists(abs)) return makeCanonicalAsset(pid, root, abs, resourceId, resource, rel)
  }

  const names = new Set()
  names.add(assetName(resourceId, resource))
  for (const raw of [resource.projectRelativePath, resource.relativePath, resource.url, resource.sourcePath, dwebRel]) {
  ...
  if (repaired?.ok && repaired?.repaired && repaired.asset) {
    return {
      ...
      ...(repaired.asset || {}),
      id: resource?.id || resourceId,
      url: buildDwebAssetUrl(pid, repaired.asset.projectRelativePath || repaired.asset.relativePath || ''),
    }
  }
...
export async function repairAllProjectAssets({ projectId, resourcesById }) {
  ...
  for (const [resourceId, resource] of Object.entries(resources)) {
    ...
    if (repaired) {
      ...
      try { upsertAssetManifestEntry(root, repaired) } catch { /* ignore manifest failures */ }
    } else {
