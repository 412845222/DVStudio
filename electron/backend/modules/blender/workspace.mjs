...
class BlenderNodeWorkspace {
	...
	async saveScreenshot(projectRoot, nodeId, base64Data, mimeType, screenshotId) {
		try {
			...
			logger.info(
				`[BlenderWorkspace] Screenshot saved: ${fileName}, size=${imageBuffer.length} bytes, path=${filePath}`
			)
			...
			const shotEntry = {
				fileName,
				relativePath: path
					.join(WORKSPACE_ROOT, sanitizeNodeId(nodeId), 'screenshots', fileName)
					.replace(/\\/g, '/'),
				timestamp: timestamp,
				screenshotId: sId,
	...
	async saveReferenceImage(projectRoot, nodeId, base64Data, fileName, mimeType) {
		try {
			...
			const imageBuffer = Buffer.from(cleanBase64, 'base64')
			const safeName = String(fileName || 'reference')
	...
	readWorkspaceImage(projectRoot, nodeId, imagePath) {
		try {
			...
			if (
				normalizedResolved.startsWith(normalizedScreenshots + path.sep) ||
				normalizedResolved === normalizedScreenshots
			) {
				...
				if (isFromScreenshots) {
					...
					if (latestScreenshot) {
						return {
							...
							warning:
								'⚠️注意：这是工作区中最新保存的截图，但可能不是Blender当前最新画面！查看当前画面状态请务必调用blender_get_screenshot_of_area_as_image工具获取实时截图！'
						}
					}
				}
			...
			if (isFromScreenshots) {
				result.warning =
					'⚠️注意：你正在读取screenshots目录中的历史截图！这可能不是Blender当前最新画面状态。要查看当前画面，请务必调用blender_get_screenshot_of_area_as_image工具获取实时最新截图！'
			}
