...
async function verifyPublicUrl(url, maxRetries = 3, retryDelayMs = 2000) {
	...
	if (
		upperUrl.includes('X-TOS-SIGNATURE=') ||
		upperUrl.includes('SIGNATURE=') ||
		upperUrl.includes('X-OSS-EXPIRES=')
	) {
	...
	for (let i = 0; i < maxRetries; i++) {
		try {
			...
			console.log(
				`[seedance] URL verification attempt ${i + 1}/${maxRetries} failed: ${err.message}`
			)
			if (i < maxRetries - 1) {
	...
	console.warn(
		`[seedance] WARNING: Could not verify URL accessibility after ${maxRetries} attempts, proceeding anyway`
	)
	return false
}
...
async function uploadVideoFileAndGetUrl(ctx, apiKey, fileObj, model) {
	...
	const baseName =
		rawName.replace(/\.[^.]+$/, '').replace(/[^\w.\-]/g, '_') || `video_${Date.now()}`
	...
	if (!cloudfsResult.ok || !cloudfsResult.publicUrl) {
		...
		if (
			errorMsg.includes('No active cloud storage') ||
...
async function ensureWebUrl(ctx, urlOrData, index, kind) {
	...
	if (isDataUrl(urlOrData)) {
		...
		throw new Error(
			`Video data URL is not supported for ${kind} #${index + 1}, video must be uploaded as file`
		)
	}
...
function extractVideoUrls(obj) {
	...
	// 1. content 对象（产物阶段）
	const content =
		typeof obj.content === 'object' && !Array.isArray(obj.content) ? obj.content : null
	if (content) {
...
function extractPrompt(obj) {
	...
	if (obj.content && typeof obj.content === 'object') {
		if (typeof obj.content.text === 'string' && obj.content.text.trim())
			return obj.content.text.trim()
		if (typeof obj.content.prompt === 'string' && obj.content.prompt.trim())
			return obj.content.prompt.trim()
	}
	...
	if (req && typeof req === 'object') {
		...
		if (req.content && typeof req.content === 'object') {
			if (typeof req.content.text === 'string' && req.content.text.trim())
				return req.content.text.trim()
			if (typeof req.content.prompt === 'string' && req.content.prompt.trim())
				return req.content.prompt.trim()
		}
	}
	return ''
}
...
function normalizeSeedanceStatus(raw) {
	const s = String(raw || 'queued')
		.trim()
		.toLowerCase()
	if (s === 'success' || s === 'completed' || s === 'processed' || s === 'ready' || s === 'active')
		return 'succeeded'
...
function buildContent(prompt, refImageUrls, refMode, refVideoUrls, refAudioUrls) {
	...
	const mode = String(refMode || 'auto')
		.trim()
		.toLowerCase()
...
export async function* generateVideoStream(ctx, payload) {
	...
	const ratioRaw =
		String(payload?.ratio || payload?.aspect_ratio || payload?.aspectRatio || 'auto').trim() ||
		'auto'
	...
	const priority = Number.isFinite(priorityRaw)
		? Math.max(0, Math.min(9, Math.floor(priorityRaw)))
		: 0
	const refMode =
		String(payload?.refMode || 'auto')
			.trim()
			.toLowerCase() || 'auto'
	...
	const effectiveClientRequestId =
		clientRequestIdRaw || `seedance-auto-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
	...
	if (Array.isArray(payload?.imageUrls)) {
		...
		refImageUrls = payload.ref_images
			.map((u) => {
	...
	if (Array.isArray(payload?.audioUrls)) {
		...
		refAudioUrls = payload.ref_audios
			.map((u) => {
				if (typeof u === 'string') return u.trim()
				if (u && typeof u.data === 'string') return u.data.trim()
				if (u && typeof u.url === 'string') return u.url.trim()
	...
	try {
		...
		for (let i = 0; i < videoFileObjects.length; i++) {
			...
			yield wrapTaskStatusMsg(
				`Seedance：准备参考视频 ${i + 1}/${videoFileObjects.length}（${fileSizeMB} MB）…`,
				'generating'
			)
			yield wrapTaskStatusMsg(
				`Seedance：正在上传参考视频到您的云存储（${fileSizeMB} MB）…`,
				'generating'
			)

			const uploadResult = await uploadVideoFileAndGetUrl(ctx, apiKey, fileObj, model)
			uploadedVideoFileUrls.push(uploadResult.url)
			videoUploadSources.push({
				source: uploadResult.source,
				providerName: uploadResult.providerName
			})
		...
		console.log(
			'[seedance] files prepared: images=',
			uploadedImageUrls.length,
			'videos(string)=',
			uploadedVideoStrUrls.length,
			'videos(file)=',
			videoFileObjects.length,
			'audios=',
			uploadedAudioUrls.length
		)
		...
		const contentForLog = content.map((item) => {
			...
			if (
				copy.image_url?.url &&
				typeof copy.image_url.url === 'string' &&
				copy.image_url.url.startsWith('data:')
			) {
				copy.image_url = {
					...copy.image_url,
					url: copy.image_url.url.slice(0, 50) + '...[base64 truncated]'
				}
			}
			if (
				copy.audio_url?.url &&
				typeof copy.audio_url.url === 'string' &&
				copy.audio_url.url.startsWith('data:')
			) {
				copy.audio_url = {
					...copy.audio_url,
					url: copy.audio_url.url.slice(0, 50) + '...[base64 truncated]'
				}
			}
			return copy
		})
		...
		if (!createRes.ok) {
			...
			if (createRes.body) {
				...
				else if (typeof createRes.body === 'object')
					errMsg = JSON.stringify(createRes.body).slice(0, 500)
				else if (typeof createRes.body === 'string') errMsg = createRes.body.slice(0, 500)
			}
			console.error(
				'[seedance] create task failed:',
				createRes.status,
				JSON.stringify(createRes.body?.error || createRes.body || {}).slice(0, 500)
			)
		...
		if (!taskId) {
			throw new Error(
				`Seedance create task failed: invalid response ${JSON.stringify(createObj).slice(0, 500)}`
			)
		}
		...
		while (true) {
			...
			const status = String(taskObj.status || '')
				.trim()
				.toLowerCase()
					...
					try {
						tq?.completeTask(globalTaskId, {
							...
							resultAssets: [
								...
								...(lastFrameUrl
									? [{ type: 'image', url: lastFrameUrl, thumbnailUrl: lastFrameUrl }]
									: [])
							],
							nodeId: nodeId || null,
							projectId
...
export async function syncTasks(ctx, payload) {
	...
	if (taskId) {
		...
		const remoteCreatedAt = parseSeedanceTimestamp(
			taskObj.created_at || taskObj.createdAt || taskObj.create_time
		)
	...
	for (const remoteTask of remoteItems) {
		...
		const remoteCreatedAt = parseSeedanceTimestamp(
			remoteTask.created_at || remoteTask.createdAt || remoteTask.create_time
		)
		...
		if (existing?.resolution && !remoteTask.resolution)
			upsertPayload.resolution = existing.resolution
	...
	return {
		...
		totalCount: Number(data.total || data.total_count || 0) || syncedItems.length,
	...
	if (taskRes.status === 404) {
		...
		if (remoteStatus === 'succeeded') {
			...
			resourceUnavailableReason =
				extractSeedanceErrorMessage(remoteTask, remoteStatus) || `任务状态：${remoteStatus}`
		} else {
			resourceAvailable = false
...
export async function downloadAssetToProject(ctx, payload) {
	...
	const url =
		assetKind === 'lastFrame' && detail.lastFrameUrlRemote
...
export async function listAllTasksRemote(ctx, payload) {
	...
	const requestedPageSize = Math.max(
		1,
		Math.min(100, parseInt(String(payload?.pageSize || 50), 10) || 50)
	)
	...
	const syncRemoteTask = (remoteTask) => {
		...
		const fallbackPrompt =
			extractedPrompt || (existing?.prompt ? String(existing.prompt).trim() : '')

		const remoteCreatedAt = parseSeedanceTimestamp(
			remoteTask.created_at || remoteTask.createdAt || remoteTask.create_time
		)
		const remoteUpdatedAt =
			parseSeedanceTimestamp(
				remoteTask.updated_at || remoteTask.updatedAt || remoteTask.update_time
			) || Date.now()
		...
		const upsertPayload = {
			...
			model: String(remoteTask.model || existing?.model || model || DEFAULT_MODEL).trim(),
			taskType: String(
				remoteTask.task_type || existing?.taskType || existing?.task_type || ''
			).trim(),
			source: 'sync',
			status: taskStatus,
	...
	console.log(
		`[seedance] listAllTasksRemote: fetched page ${pageNum}, got ${syncedItems.length} items, total=${totalFromRemote}, hasMore=${hasMore}`
	)
