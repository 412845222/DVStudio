...
export function validateRoot(ctx, payload) {
  ...
  if (expected) {
    ...
    return { ok: true, reRegistered: result?.ok, registerResult: result, validation: ctx.protocol.validateProjectRoot(projectId) }
  }
...
export function getAccessLogs(ctx, payload) {
  ...
  return {
    ok: true,
    logs: ctx.protocol.getAccessLogs(Number.isFinite(maxEntries) && maxEntries > 0 ? Math.floor(maxEntries) : 100)
  }
}
