...
export class CliLLMProvider extends ILLMProvider {
	...
	async createSession(options = {}) {
		...
		const sessionId = await cliAdapterManager.startSession(
			this.adapterName,
			{ cwd },
			options.config || {}
		)
	...
	formatMessagesToPrompt(messages) {
		...
		for (const msg of messages) {
			if (msg.role === 'system') {
				parts.push(
					typeof msg.content === 'string' ? msg.content : this.extractTextFromParts(msg.content)
				)
				parts.push('')
	...
	async *streamGenerate(sessionId, input) {
		...
		if (!sessInfo) {
			try {
				sessionId = await cliAdapterManager.startSession(
					this.adapterName,
				...
				yield {
					...
					message: `Failed to start ${this.displayName} session: ${err.message}`
				}
				return
			}
		}
		...
		try {
			const availability = await cliAdapterManager.checkAvailability(
				this.adapterName,
				config.cliConfig || {}
			)
			...
			for await (const event of stream) {
				...
				if (parsedEvent && typeof parsedEvent === 'object') {
					...
					if (eventType === CLIEventType.TEXT_DELTA || eventType === 'text_delta') {
						...
						eventType === CLIEventType.THINKING_DELTA ||
						...
						eventType === CLIEventType.TOOL_CALL_START ||
						...
						eventType === CLIEventType.TOOL_CALL_ERROR ||
						...
						yield {
							type: ProviderEventType.ERROR,
							message: parsedEvent.error || parsedEvent.message || 'Unknown CLI error'
						}
						return
