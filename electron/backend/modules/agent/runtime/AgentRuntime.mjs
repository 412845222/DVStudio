...
export class AgentRuntime {
	...
	async *streamMessage(payload) {
		...
		const parsedMaxToolCalls =
			p.maxToolCalls !== undefined && p.maxToolCalls !== null
				? Number(p.maxToolCalls)
				: DEFAULT_MAX_TOOL_CALLS
		const maxToolCalls =
			!isNaN(parsedMaxToolCalls) && parsedMaxToolCalls > 0
				? parsedMaxToolCalls
				: DEFAULT_MAX_TOOL_CALLS
		...
		logger.info(
			`AgentRuntime: using backend=${providerId}, model=${model}, apiSource=${apiSource}, agentType=${agentType}, customSystemPrompt=${useCustomSystemPromptOnly ? 'yes' : 'no'}`
		)
		let provider
		try {
		...
		try {
			...
			if (providedTools.length > 0) {
				...
				logger.info(
					`AgentRuntime: REQUESTED TOOLS MODE - requested tools: ${Array.from(allowedNames).join(', ')}`
				)
				logger.info(
					`AgentRuntime: REQUESTED TOOLS MODE - found ${allTools.length} total tools, ${tools.length} matched: ${tools.map((t) => t.function.name).join(', ')}`
				)
				if (tools.length === 0) {
					const allNames = allTools.map((t) => t.function.name).join(', ')
					logger.warn(
						`AgentRuntime: REQUESTED TOOLS MODE - no matching tools found! Available tool names: ${allNames}`
					)
					yield {
						type: 'error',
						message: `工具不可用：找不到 ${Array.from(allowedNames).join(', ')}。当前可用工具：${allNames}`
					}
					return
				}
			} else {
				...
				logger.info(
					`AgentRuntime: SCENE MODE (${agentType}) - ${tools.length} tools available: ${tools.map((t) => t.function.name).join(', ')}`
				)
			}

			const toolPromptText =
				!provider.supportsNativeToolCalls && !provider.executesOwnTools
			...
			if (useCustomSystemPromptOnly) {
				...
				logger.info(
					`AgentRuntime: CUSTOM SYSTEM PROMPT MODE - using custom system prompt only, cliToolInstructions appended: ${needCliToolPrompt}, executesOwnTools: ${provider.executesOwnTools}`
				)
				const userContent =
					attachments.length > 0
						? (() => {
								...
								for (const att of attachments) {
									if (
										att.type === 'image_url' ||
										String(att.name || '').match(/\.(jpg|jpeg|png|gif|webp)$/i)
									) {
										parts.push({
											type: 'image_url',
				...
				if (initialTokens > budget * TRUNCATION_THRESHOLD) {
					truncResult = ContextBuilder.truncateMessagesInLoop(
						rawMessages,
						budget * TRUNCATION_THRESHOLD
					)
					logger.info(
						`AgentRuntime: CUSTOM SYSTEM PROMPT MODE - initial truncation: ${initialTokens} -> ${truncResult.tokenCount} tokens (budget=${budget}), truncated=${truncResult.truncated}`
					)
				}
				...
				const baseSystemPrompt = this.contextBuilder.buildSystemPrompt(context, {
					includeToolInstructions:
						!provider.supportsNativeToolCalls && !provider.executesOwnTools && !customSystemPrompt,
					toolPromptText,
					agentType
				})
				...
				currentMessages = [{ role: 'system', content: systemPrompt }, ...buildResult.messages]
			...
			const openaiTools = provider.supportsNativeToolCalls ? tools : undefined
			...
			while (toolCallCount < maxToolCalls) {
				...
				if (provider.supportsNativeToolCalls) {
					const assistantMsg = {
						...
						tool_calls: assistantToolCalls.map((atc) => ({
							...
							function: {
								name: atc.name,
								arguments:
									typeof atc.arguments === 'string' ? atc.arguments : JSON.stringify(atc.arguments)
							}
						}))
					}
					...
					for (const atc of assistantToolCalls) {
						...
						if (atc.error) {
							...
							if (duplicateCount >= 1) {
								...
								logger.warn(
									`AgentRuntime: Detected duplicate error (${duplicateCount + 1}x): ${errorSignature}`
								)
							}
							toolContent = JSON.stringify(errorObj)
						} else if (
							atc.sanitizedResult &&
							typeof atc.sanitizedResult === 'object' &&
							atc.sanitizedResult.status === 'error'
						) {
							...
							if (duplicateCount >= 1) {
								...
								logger.warn(
									`AgentRuntime: Detected duplicate error in result (${duplicateCount + 1}x): ${errorSignature}`
								)
							}
					...
					if (roundImages.length > 0) {
						...
						const screenshotMsgText =
							'以下是操作后的最新截图（screenshot_id: ' +
							Date.now() +
							'），请仔细查看截图验证当前Blender画面状态并继续：'
						const imageParts = [
							{
								type: 'text',
						...
						for (const img of roundImages) {
							...
							logger.info(
								`AgentRuntime: Adding screenshot to vision context: ${img.fileName}, using ${img.dataUrl ? 'dataUrl' : 'fileUrl'}`
							)
						}
							...
							if (msg.role === 'user' && Array.isArray(msg.content)) {
								const hasText = msg.content.some(
									(p) =>
										...
										p.text.includes('screenshot_id:')
								)
								const hasImage = msg.content.some((p) => p.type === 'image_url')
								if (hasText && hasImage) {
									currentMessages.splice(i, 1)
									logger.info(
										`AgentRuntime: Removed previous screenshot message to save tokens (index: ${i})`
									)
									break
								}
							}
						...
						logger.info(
							`AgentRuntime: Added ${roundImages.length} screenshot(s) as vision context, total this session: ${sessionImages.length}`
						)
					}
					...
					if (currentTokens > budget * LOOP_THRESHOLD) {
						...
						const compressResult = ContextBuilder.truncateMessagesInLoop(
							currentMessages,
							budget * LOOP_THRESHOLD
						)
						currentMessages = compressResult.messages
						logger.info(
							`AgentRuntime: Dynamic context compression in tool loop: ${beforeCount} -> ${compressResult.tokenCount} tokens (truncated=${compressResult.truncated})`
						)
					}
					...
					for (const atc of assistantToolCalls) {
						...
						resultParts.push(
							`结果: ${JSON.stringify(atc.error ? { error: atc.error } : atc.sanitizedResult, null, 2)}`
						)
						if (atc.error) {
					...
					resultParts.push(
						'请根据工具执行结果继续回答用户问题。如需调用更多工具，请使用相同的 <|FunctionCallBegin|> 格式。'
					)
			...
			if (toolCallCount >= maxToolCalls) {
				...
				logger.warn(
					`AgentRuntime: Max tool call iterations (${maxToolCalls}) reached, enableToolCallWarning=${enableToolCallWarning}`
				)
			}
			...
			try {
				...
				logger.warn(
					`AgentRuntime: Failed to cleanup session screenshot files: ${cleanupErr.message}`
				)
			}
		}
	}
