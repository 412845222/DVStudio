...
function normalizeTask(taskId, obj) {
	...
	const collectFromObject = (target, depth = 0) => {
		...
		for (const [key, value] of Object.entries(target)) {
			...
			if (
				...
				keyLower === 'image' ||
				keyLower === 'image_url' ||
				keyLower === 'output_images' ||
				keyLower === 'generated_images' ||
				keyLower === 'result'
			) {
	...
	if (isImageMode) {
		console.log(
			`[Tripo3D Backend] normalizeTask image mode=${mode} status=${status} imageUrls count=${imageUrls.length} thumbnail=${thumbnailUrl ? 'yes' : 'no'}`
		)
		if (
			imageUrls.length === 0 &&
			...
			console.log(
				`[Tripo3D Backend] normalizeTask - Task success but no images found. Full response:`,
	...
	if (!statusText) {
		if (status === 'queued') statusText = 'Tripo3D：任务排队中'
			...
			statusText = isImageMode ? 'Tripo3D：图片生成完成' : 'Tripo3D：生成完成'
		else if (status === 'failed' || status === 'error')
			statusText = errorMessage || 'Tripo3D：生成失败'
...
function buildCreatePayload(payload) {
	const mode = String(payload.mode || 'text_to_model')
		.trim()
		.toLowerCase()
	const validModes = [
		'text_to_model',
	...
	if (mode === 'text_to_model') {
		...
		const input = String(
			payload.input ||
				payload.model_task_id ||
				payload.original_model_task_id ||
				payload.model_url ||
				''
		).trim()
		if (!input) {
		...
		if (
			...
			Number.isFinite(Number(payload.texture_seed)) &&
			Number(payload.texture_seed) > 0
		) {
		...
		const input = String(payload.input || payload.model_task_id || '').trim()
		if (!input)
			return [null, null, 'input (mesh/segment task_id) is required for mesh_complete mode']
		body.input = input
		if (payload.model) body.model = String(payload.model)
		if (Array.isArray(payload.part_names) && payload.part_names.length) {
			body.part_names = payload.part_names.map(String)
	...
	if (!isPostProcess) {
		...
		if (
			typeof payload.model_seed === 'number' &&
			Number.isFinite(payload.model_seed) &&
			payload.model_seed >= 0
		) {
	...
	if (!isPostProcess) {
		...
		if (
			typeof payload.texture_seed === 'number' &&
			Number.isFinite(payload.texture_seed) &&
			payload.texture_seed >= 0
		) {
...
export async function generateModel(ctx, payload) {
	...
	console.log(
		'[Tripo3D Backend] 构建的请求 body (mode=' + mode + '):',
...
export async function getBalance(ctx) {
	...
	try {
		...
		const frozenValue =
			typeof balanceData === 'object' && balanceData !== null
				? typeof balanceData.frozen === 'number'
					? balanceData.frozen
					: null
				: null
		const displayText =
...
function buildImageCreatePayload(payload, mode) {
	...
	const seed =
		typeof payload.seed === 'number' && payload.seed >= 0 ? Math.floor(payload.seed) : undefined
	const input = String(payload.input || payload.image_url || payload.input_url || '').trim()
	const strength =
		typeof payload.strength === 'number' && payload.strength >= 0 && payload.strength <= 1
			? payload.strength
			: 0.7
