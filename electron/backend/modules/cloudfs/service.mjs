...
function ensureInitialized() {
	...
	try {
		...
		initialized = true
		logger.info(
			`[cloudfs] Registered providers: ${listProviders()
				.map((p) => p.id)
				.join(', ')}`
		)
	} catch (err) {
...
export async function getConfigStatus(ctx) {
	...
	try {
		...
		let hasActiveBucket = bucketResult?.ok && bucketResult?.bucket?.bucketName

		if (!hasActiveBucket) {
			const bucketsResult = repos.cloudStorageConfig.listConfiguredBuckets()
			if (bucketsResult.ok && bucketsResult.buckets.length > 0) {
				const targetBucket =
					bucketsResult.buckets.find((b) => b.isActive) || bucketsResult.buckets[0]
				repos.cloudStorageConfig.setActiveBucket(targetBucket.id)
				bucketResult = repos.cloudStorageConfig.getActiveBucketWithConfig()
				hasActiveBucket = bucketResult?.ok && bucketResult?.bucket?.bucketName
			}
		}

		// Fallback: if bucket table is empty but config JSON contains bucketName (legacy data or incomplete migration),
		// automatically create bucket record and set it as active
		if (!hasActiveBucket && activeResult.config?.bucketName) {
			logger.info(
				`[cloudfs] getConfigStatus: bucket table empty but config has bucketName=${activeResult.config.bucketName}, auto-recovering`
			)
			try {
				const configId = activeResult.id
				const addResult = repos.cloudStorageConfig.addBucket(
					configId,
					activeResult.config.bucketName,
					activeResult.config.region || '',
					activeResult.config.endpoint || '',
					'public-read'
				)
				if (addResult.ok) {
					...
					hasActiveBucket = bucketResult?.ok && bucketResult?.bucket?.bucketName
					logger.info(
						`[cloudfs] getConfigStatus: auto-recovery succeeded, hasActiveBucket=${!!hasActiveBucket}`
					)
				}
			} catch (recoveryErr) {
				logger.warn(`[cloudfs] getConfigStatus: auto-recovery failed: ${recoveryErr.message}`)
			}
		}

		const activeBucketName = hasActiveBucket
			? bucketResult.bucket.bucketName
			: activeResult.config?.bucketName || ''
...
async function getActiveConfigFull() {
	...
	// Fallback: recover bucket from config JSON if bucket table is empty
	if (!result.ok) {
		const activeResult = repos.cloudStorageConfig.getActive()
		if (activeResult.configured && activeResult.config?.bucketName) {
			logger.info(
				`[cloudfs] getActiveConfigFull: bucket table empty but config has bucketName=${activeResult.config.bucketName}, auto-recovering`
			)
			try {
				const addResult = repos.cloudStorageConfig.addBucket(
					activeResult.id,
				...
				if (addResult.ok) {
					repos.cloudStorageConfig.setActiveBucket(addResult.bucket.id)
					result = repos.cloudStorageConfig.getActiveBucketWithConfig()
					logger.info(`[cloudfs] getActiveConfigFull: auto-recovery succeeded`)
				}
			} catch (recoveryErr) {
				logger.warn(`[cloudfs] getActiveConfigFull: auto-recovery failed: ${recoveryErr.message}`)
			}
		}
	}
	if (!result.ok) {
...
export async function testConfig(ctx, providerId, config) {
	...
	try {
		...
		logger.info(
			`[cloudfs] testConfig config keys: ${config ? Object.keys(config).join(', ') : 'null'}`
		)
		if (config?.credentials) {
			...
			logger.info(
				`[cloudfs] testConfig credentials before normalize: accessKeyId length=${akId.length}, prefix=${akId.slice(0, 6)}..., suffix=...${akId.slice(-4)}, accessKeySecret length=${akSecret.length}`
			)
		...
		try {
			...
			logger.warn(
				`[cloudfs] Failed to get active config (this is OK when testing new config): ${activeErr.message}`
			)
			active = null
		}
		logger.info(
			`[cloudfs] active config exists: ${!!active}, active.providerId=${active?.providerId}`
		)
		...
		if (cfg && cfg.credentials) {
			...
			logger.info(
				`[cloudfs] after normalize: accessKeyId length=${afterAkId.length}, accessKeySecret length=${afterAkSecret.length}`
			)
		}

		const hasCredentials = cfg?.credentials?.accessKeySecret || cfg?.apiKey
		logger.info(
			`[cloudfs] hasCredentials=${!!hasCredentials}, userExplicitlyProvidedConfig=${userExplicitlyProvidedConfig}`
		)
		...
		if (!hasCredentials) {
			...
			if (active && active.providerId === pid) {
				...
				logger.warn(
					`[cloudfs] Active provider (${active.providerId}) does not match requested provider (${pid}), cannot fallback`
				)
				return { ok: false, error: 'Credentials are required' }
			} else {
		...
		logger.info(
			`[cloudfs] Calling provider.testConnection for ${pid} with region=${cfg?.region}, endpoint=${cfg?.endpoint}`
		)
		...
		if (result.ok) {
			try {
				...
				logger.warn(
					`[cloudfs] Could not update test status in repos (OK for standalone test): ${reposErr.message}`
				)
			}
...
export async function setupProviderBucket(
	ctx,
	providerId,
	credentials,
	region,
...
export async function listBuckets(ctx, payload = {}) {
	...
	try {
		...
		logger.info(
			`[cloudfs] listBuckets: provider=${providerId}, region=${region}, endpoint=${endpoint}, hasCredentials=${!!credentials?.accessKeySecret}`
		)
...
export async function updateActiveBucket(ctx, payload = {}) {
	...
	try {
			...
			try {
				...
				if (active?.provider?.ensureBucketPublicRead) {
					...
					if (aclResult.ok) {
						...
						logger.info(
							`[cloudfs] Bucket ${bucketName} ACL: ${aclResult.alreadyPublic ? 'already public-read' : 'set to public-read'}`
						)
					} else {
						logger.warn(
							`[cloudfs] Failed to ensure public-read for bucket ${bucketId}: ${aclResult.error}`
						)
					}
				}
			} catch (aclErr) {
		...
		try {
			if (active.provider?.ensureBucketPublicRead) {
				const aclResult = await active.provider.ensureBucketPublicRead({
					...active.config,
					bucketName
				})
				if (aclResult.ok) {
...
export async function listFiles(ctx, payload = {}) {
	...
	try {
		...
		if (!active) {
			return {
				ok: false,
				error: 'No active cloud storage config',
				items: [],
				prefixes: [],
				isTruncated: false
			}
...
export async function addBucketFromCloud(ctx, payload = {}) {
	...
	try {
		...
		if (credentials) {
			...
			logger.info(
				`[cloudfs] addBucketFromCloud: looking for config with fingerprint ${fp.slice(0, 16)}...`
			)

			const existingConfig = repos.cloudStorageConfig.findConfigByFingerprint(fp)
			if (existingConfig) {
				logger.info(
					`[cloudfs] addBucketFromCloud: found existing config id=${existingConfig.id}, reusing`
				)
		...
		if (existingBucket) {
			logger.info(
				`[cloudfs] addBucketFromCloud: bucket ${bucketName} already exists in config ${targetConfigId}, switching to it`
			)
			repos.cloudStorageConfig.setActiveConfigById(targetConfigId)
			repos.cloudStorageConfig.setActiveBucket(existingBucket.id)
			const bucketRow = repos.cloudStorageConfig
				.listConfiguredBuckets()
				.buckets.find((b) => b.id === existingBucket.id)
		...
		try {
			...
			if (active?.provider?.ensureBucketPublicRead) {
				...
				if (aclResult.ok) {
					...
					logger.warn(
						`[cloudfs] addBucketFromCloud: failed to set public-read for ${bucketName}: ${aclResult.error}`
					)
				}
			}
		} catch (aclErr) {
			logger.warn(
				`[cloudfs] addBucketFromCloud: ACL check failed for ${bucketName}: ${aclErr.message}`
			)
		}
		...
		if (active.provider?.ensureBucketPublicRead) {
			try {
				...
				if (aclResult.ok) {
					...
					logger.warn(
						`[cloudfs] fixBucketAcl: failed to set public-read for bucket ${bucketId}: ${aclResult.error}`
					)
				}
			} catch (aclErr) {
				aclError = aclErr.message
				logger.warn(
					`[cloudfs] fixBucketAcl: exception setting public-read for bucket ${bucketId}: ${aclErr.message}`
				)
			}
		}
