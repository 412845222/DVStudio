import {
	listBuckets,
	setupBucket,
	createBucket as createBucketImpl,
	setBucketPublicRead,
	setupLifecycle,
	getBucketAcl,
	ensureBucketPublicRead as ensureBucketPublicReadImpl
} from './bucket.mjs'
...
const provider = {
	...
	async createBucket(credentials, region, bucketName, options = {}) {
		try {
			const endpoint =
				options.endpoint || credentials.endpoint || resolveEndpoint(region || 'cn-beijing')
	...
	async getBucketAcl(config) {
		...
		const endpoint =
			config.endpoint || credentials.endpoint || resolveEndpoint(region || 'cn-beijing')
		const client = getTosClient(credentials, region, bucketName, endpoint)
