...
function tosObjectToFileItem(obj, bucketName, endpoint) {
  const key = obj.Key || ''
  ...
  return createCloudFileItem({
    ...
    lastModified,
    etag,
    publicUrl: buildPublicUrl(bucketName, endpoint, key),
  })
}
...
export async function listFiles(config, prefix = '', options = {}) {
  ...
  try {
    ...
    const items = (data.Contents || [])
      ...
      .map(obj => tosObjectToFileItem(obj, bucketName, endpoint))
...
export async function getFileMetadata(config, key) {
  ...
  try {
    ...
    return createCloudFileItem({
      ...
      contentType: headers['content-type'] || data.ContentType,
