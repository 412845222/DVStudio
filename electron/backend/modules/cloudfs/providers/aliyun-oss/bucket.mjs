...
export async function listBuckets(credentials, region, endpoint) {
  const finalEndpoint = endpoint || credentials.endpoint || resolveEndpoint(region || 'oss-cn-hangzhou')
...
export async function createBucket(client, bucketName, region) {
  try {
    await withNoProxyEnv(() => client.putBucket(bucketName, {
    ...
    const isOwnedByYou = msg.includes('BucketAlreadyOwnedByYou') || code === 'BucketAlreadyOwnedByYou'
    const isExistsButNotOwned = (status === 409 || code === 'BucketAlreadyExists' || msg.includes('BucketAlreadyExists')) && !isOwnedByYou
