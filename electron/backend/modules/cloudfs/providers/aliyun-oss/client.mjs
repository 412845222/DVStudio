	...
	function cleanRequest(url, args, callback) {
		...
		if (callback) {
			result.then(
				(res) => {
					restoreEnv(prev)
					callback(null, res.data, res.res)
				},
				(err) => {
					restoreEnv(prev)
					callback(err)
...
function getClientKey(credentials, region, bucketName, endpoint) {
	const secretHash = crypto
		.createHash('sha256')
		.update(credentials.accessKeySecret || '')
		.digest('hex')
		.slice(0, 12)
...
export function getOssClient(credentials, region, bucketName, endpoint) {
	...
	logger.info(
		`[cloudfs:oss] Creating OSS client: region=${regionId}, endpoint=${finalEndpoint}, bucket=${bucketName || '(none)'}, ak=${credentials.accessKeyId.slice(0, 8)}...`
	)
