...
export async function validateCredentials(ctx, payload) {
  try {
    const p = payload || {}
    return await service.validateProviderCredentials(ctx, p.providerId, p.credentials, p.region, p.endpoint)
  } catch (err) {
    return { ok: false, error: String(err?.message || err) }
  }
}
...
export async function setupBucket(ctx, payload) {
  try {
    const p = payload || {}
    return await service.setupProviderBucket(ctx, p.providerId, p.credentials, p.region, p.bucketName, p.options)
  } catch (err) {
    return { ok: false, error: String(err?.message || err) }
  }
}
...
export async function listFiles(ctx, payload) {
  try {
    ...
    return { ok: false, error: String(err?.message || err), items: [], prefixes: [], isTruncated: false }
  }
}
