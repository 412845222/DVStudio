...
function emitChange(event) {
    ...
    for (const fn of _listeners.values()) {
        try { fn(event) }
        catch (err) { log.warn(`taskStore listener error (swallowed): ${err.message}`) }
    }
}
...
export function markTaskFailed(taskId, error) {
    return updateTask(taskId, {
        status: 'failed',
        error: error && typeof error === 'object' ? { message: String(error.message || ''), code: error.code || 'UNKNOWN' } : { message: String(error || '') },
        completedAt: Date.now()
    })
}
