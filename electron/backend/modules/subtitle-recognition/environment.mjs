...
import { getWhisperBinaryPath, getWhisperModelPath, getWhisperModelDir, getWhisperBinaryDir, ensureWhisperDirs, getFfmpegBinaryPath, getFfprobeBinaryPath, getFfmpegDir } from './paths.mjs'
...
export async function verifyEnvironment() {
  ...
  const defaultModel = models.find(m => m.ok && m.size === ModelSize.BASE)
    || models.find(m => m.ok)
    || null
