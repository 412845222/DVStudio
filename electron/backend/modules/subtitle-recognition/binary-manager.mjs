...
import { getWhisperBinaryDir, getWhisperBinaryPath, ensureWhisperDirs, getTempDir } from './paths.mjs'
...
async function extractZip(zipPath, targetDir, binaryNames, finalBinaryName) {
  ...
  console.log('[SubtitleRecog] extracting zip, files in archive:', Object.keys(zip.files).filter(f => !f.endsWith('/')).join(', '))
  ...
  if (!bestBinaryFile) {
    for (const [filename, file] of Object.entries(zip.files)) {
      if (file.dir) continue
      const baseName = path.basename(filename).toLowerCase()
      if ((baseName.includes('whisper') || baseName === 'main.exe' || baseName === 'main') &&
          !baseName.includes('bench') && !baseName.includes('test') &&
          !baseName.includes('blas') && !baseName.includes('cublas') &&
  ...
  if (!bestBinaryFile) {
    const available = Object.keys(zip.files).filter(f => !f.endsWith('/')).slice(0, 30).join(', ')
  ...
  for (const [filename, file] of Object.entries(zip.files)) {
    ...
    if (isWinExe || isDll || isLib || isBinary || lowerBase === 'readme.md' || lowerBase === 'license') {
      try {
        const content = await file.async('nodebuffer')
  ...
  if (path.basename(bestBinaryFile.filename).toLowerCase() !== finalBinaryName.toLowerCase()) {
    ...
    if (fs.existsSync(extractedBinaryPath) && !fs.existsSync(mainTargetPath)) {
      try {
        ...
        if (process.platform !== 'win32') {
          try { fs.chmodSync(mainTargetPath, 0o755) } catch {}
        }
  ...
  const deprecatedStub = path.join(targetDir, process.platform === 'win32' ? 'whisper.exe' : 'whisper')
  if (deprecatedStub !== mainTargetPath && fs.existsSync(deprecatedStub)) {
    try { fs.unlinkSync(deprecatedStub) } catch {}
...
export async function* downloadAndInstallBinary(options = {}) {
  ...
  try {
    if (config.archiveName.endsWith('.zip')) {
      await extractZip(archivePath, getWhisperBinaryDir(), config.binaryNames, config.finalBinaryName)
    } else {
  ...
  yield { type: 'done', path: finalStatus.path, size: finalStatus.size, version: WHISPER_CPP_VERSION }
}
