...
async function resolveDwebUrl(dwebUrl) {
  ...
  try {
    ...
    if (!rootPath) {
      ...
      throw new Error(`项目根目录未注册，projectId: ${projectId}。已注册的项目: ${Object.keys(snapshot).join(', ') || '无'}。请确保项目已正确加载。`)
    }

    const normalizedRoot = path.resolve(rootPath)
    console.log('[SubtitleRecog] normalized root:', normalizedRoot)

    const rel = String(relPath).replace(/\\/g, '/').split('/').filter(s => s && s !== '.')
    if (rel.some(s => s === '..')) {
    ...
    console.log('[SubtitleRecog] first attempt resolved:', normalized, 'exists:', fs.existsSync(normalized))
    ...
    if (!fs.existsSync(normalized)) {
      ...
      console.log('[SubtitleRecog] file NOT found. Parent dir:', parentDir, 'contents:', parentContents)
      throw new Error(`dweb://资源文件不存在: ${normalized}。目录内容: ${parentContents.join(', ')}`)
    }
    ...
    if (err.message?.includes('不存在') || err.message?.includes('未注册') || err.message?.includes('非法') || err.message?.includes('不支持') || err.message?.includes('缺少') || err.message?.includes('无效')) {
      throw err
    }
  ...
  for (let i = 0; i < relSegments.length; i++) {
    ...
    try {
      ...
      if (match) {
        ...
        const partialMatch = entries.find(e => e.toLowerCase().startsWith(lowerSegment.split('.')[0].toLowerCase() + '.'))
...
export async function* recognize(payload) {
  ...
  if (Number.isFinite(numericProjectId) && numericProjectId > 0) {
    ...
    if (projectRoot) {
      ...
      console.log('[SubtitleRecog] project root found:', projectRoot, 'audio will be saved to:', projectMediaDir)
    } else {
      console.warn('[SubtitleRecog] projectId provided but project root not registered:', numericProjectId)
    }
  }
  ...
  try {
    ...
    const extractOptions = {
      onProgress: (progress) => {
        ...
        audioQueue.push({ type: 'progress', ...progress, percent: Math.floor(progress.percent * 0.3) })
      },
    }
    if (projectMediaDir) {
    ...
    console.log('[SubtitleRecog] calling extractAudio with:', normalizedVideoPath, 'outputPath:', extractOptions.outputPath || '(temp)')
    ...
    while (!audioDone || audioQueue.length > 0) {
      if (audioError) {
        ...
        yield { type: 'error', message: `音频提取失败: ${audioError.message || String(audioError)}` }
        return
      }
    ...
    console.log('[SubtitleRecog] audio wait loop done, audioResult:', !!audioResult, 'audioPath:', audioResult?.audioPath)
    ...
    console.log('[SubtitleRecog] effectiveModel:', effectiveModel, 'requested modelSize:', modelSize)
    ...
    for await (const chunk of runWhisperRecognition({
      ...
      whisperChunkCount++
      console.log('[SubtitleRecog] whisper chunk #' + whisperChunkCount + ':', chunk.type, chunk.percent || '', chunk.message || '')
      if (chunk.type === 'progress' && typeof chunk.percent === 'number') {
        ...
        const cuesInSeconds = (chunk.cues || []).map(c => ({
        ...
        console.log('[SubtitleRecog] audioPath preserved:', audioPath, 'duration:', audioResult?.duration, 'audioDwebUrl:', audioDwebUrl)
    ...
    console.log('[SubtitleRecog] whisper for-await loop ended naturally, chunks:', whisperChunkCount)
  } catch (err) {
    console.error('[SubtitleRecog] UNEXPECTED ERROR in recognize:', err)
    yield { type: 'error', message: err.message || String(err) }
    if (audioPath && !audioDwebUrl) {
      try { cleanupAudio(audioPath) } catch {}
    }
  }
}
