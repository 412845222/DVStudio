...
import { getWhisperBinaryPath, getWhisperBinaryDir, getWhisperModelPath, getTempDir, getFfmpegDir } from './paths.mjs'
...
function parseWhisperPhase(text) {
  if (text.includes('load_model') || text.includes('loading model') || text.includes('load_backend')) return 'Loading model'
  if (text.includes('convert') || text.includes('converting') || text.includes('convert_pcm')) return 'Converting audio'
  if (text.includes('mel') || text.includes('spectrogram') || text.includes('log_mel')) return 'Processing audio'
  if (text.includes('encode') || text.includes('encoding')) return 'Encoding'
  if (text.includes('decode') || text.includes('decoding') || text.includes('search')) return 'Decoding'
  if (text.includes('save') || text.includes('writing') || text.includes('output') || text.includes('srt')) return 'Saving output'
...
function runWhisperProcess(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log('[SubtitleRecog][whisper] modelPath:', modelPath, 'exists:', fs.existsSync(modelPath))
    console.log('[SubtitleRecog][whisper] audioPath:', audioPath, 'exists:', fs.existsSync(audioPath))
    console.log('[SubtitleRecog][whisper] audio stat:', fs.existsSync(audioPath) ? fs.statSync(audioPath).size : 'N/A', 'bytes')
    ...
    const audioBasename = path.basename(audioPath, path.extname(audioPath)).replace(/[^a-zA-Z0-9_\u4e00-\u9fa5]/g, '_')
    ...
    const handleOutput = (text, isStderr) => {
      ...
      for (const line of lines) {
        ...
        if (trimmed.includes('error') || trimmed.includes('ERROR') || trimmed.includes('failed') || trimmed.includes('FAILED')) {
          console.log('[SubtitleRecog][whisper][output]', isStderr ? 'STDERR' : 'STDOUT', ':', trimmed)
        }
    ...
    child.on('close', (code) => {
      ...
      console.log('[SubtitleRecog][whisper] expected SRT path:', expectedSrtPath, 'exists:', fs.existsSync(expectedSrtPath))
      ...
      if (!fs.existsSync(expectedSrtPath)) {
        ...
        try {
          ...
          console.log('[SubtitleRecog][whisper] tempDir files matching:', allFiles.filter(f => f.includes('whisper_output') || f.endsWith('.srt')).join(', '))
          ...
          if (possibleFiles.length > 0) {
            ...
            const anySrt = allFiles.filter(f => f.endsWith('.srt')).sort((a, b) => {
      ...
      if (!fs.existsSync(finalSrtPath)) {
        const allTempFiles = fs.existsSync(tempDir) ? fs.readdirSync(tempDir).slice(-20).join(', ') : 'tempDir not found'
        reject(new Error(`未找到 SRT 输出文件。期望路径: ${expectedSrtPath}。临时目录文件: ${allTempFiles}`))
        return
      }
      ...
      try {
        ...
        console.log('[SubtitleRecog][whisper] SRT preview (first 500 chars):', srtContent.slice(0, 500))
        const cues = parseSrt(srtContent)
        console.log('[SubtitleRecog][whisper] parsed cues count:', cues.length)

        try { fs.unlinkSync(finalSrtPath) } catch {}
