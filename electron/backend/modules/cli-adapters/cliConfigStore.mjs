...
class CliConfigStore {
	...
	getAdapterConfig(adapterName) {
		...
		if (config.models && Array.isArray(config.models)) {
			...
			if (changed) {
				logger.warn(
					`[CliConfigStore] Found ${invalid.length} invalid model(s) for adapter ${adapterName}, auto-cleaning...`
				)
				config.models = valid
	...
	async updateAdapterConfig(adapterName, updates) {
		...
		if (merged.models && Array.isArray(merged.models)) {
			...
			if (changed) {
				logger.warn(
					`[CliConfigStore] Removed ${invalid.length} invalid model(s) for adapter ${adapterName}`
				)
				merged.models = valid
			}
		}
