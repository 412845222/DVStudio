...
function ensureDvstudioMcpProfile() {
	try {
		...
		logger.info(
			`[CodexCLI] MCP bridge path: ${STDIO_BRIDGE_PATH} (exists: ${fs.existsSync(STDIO_BRIDGE_PATH)})`
		)
		logger.info(
			`[CodexCLI] Node executable path: ${nodePath} (exists: ${fs.existsSync(runtime.path)})`
		)
...
function parseDeviceAuthOutput(text) {
	...
	if (!result.userCode) {
		for (let i = 0; i < lines.length; i++) {
			const line = lines[i]
			if (
				/one-time code|enter this code|verification code|device code/i.test(line) &&
				!/authorization|phishing|share/i.test(line)
			) {
				...
				if (fs.existsSync(filePath)) {
					try {
						...
						const hasToken =
							auth &&
							((auth.tokens &&
								(auth.tokens.access_token || auth.tokens.api_key || auth.tokens.token)) ||
								auth.access_token ||
								auth.api_key ||
								auth.token ||
...
export class CodexCliAdapter extends BaseCLIAdapter {
			...
			if (fs.existsSync(codexConfigPath)) {
				try {
					...
					if (cliPathMatch && cliPathMatch[1] && fs.existsSync(cliPathMatch[1])) {
						...
						logger.info(
							`[CodexCLI] Found codex binary from config.toml CODEX_CLI_PATH: ${cliPathMatch[1]}`
						)
	...
	isAuthenticated() {
		try {
			...
			if (fs.existsSync(configPath)) {
				...
				if (
					/openai_api_key\s*=\s*"sk-[^"]+"/.test(content) ||
					/api_key\s*=\s*"sk-[^"]+"/.test(content)
				) {
					return true
				}
			}
	...
	async checkEnvironment() {
		...
		checks.push({
			...
			message: nodeAvailable
				? 'Node.js 已安装（npm安装方式需要）'
				: 'Node.js 未找到（如通过npm安装则需要）',
	...
	async *startAuthFlow() {
		...
		logger.info(
			`[CodexCLI] Spawning auth process, shell=${isWinCmd}, proxy=${proxyEnv.HTTPS_PROXY || 'none'}`
		)
					...
					exec(cmd, (error) => {
						if (error) {
							resolve({
								...
								interactive: true,
								message:
									'无法打开终端，请手动运行: npm install -g @openai/codex（若有代理请先设置HTTP_PROXY环境变量）'
							})
						} else {
							resolve({
								ok: true,
		...
		try {
			const models = await new Promise((resolve, reject) => {
				...
				proc.on('close', (code) => {
					if (code !== 0) {
						reject(
							new Error(`codex debug models exited with code ${code}: ${stderr.substring(0, 200)}`)
						)
						return
					}
					try {
	...
	async startSession(options = {}) {
		...
		logger.info(
			`[CodexCLI] Session created: ${sessionId}, binary: ${codexBin}, cwd: ${session.cwd}`
		)
		return sessionId
	}
	...
	async *sendMessage(sessionId, content, options = {}) {
		...
		if (requestedModel !== model) {
			logger.warn(
				`[CodexCLI] Model "${requestedModel}" not available in configured models, falling back to "${model}". Please refresh models in Settings.`
			)
		}
...
`
		...
		const processLine = (line) => {
			...
			logger.debug(
				`[CodexCLI] JSON type=${obj.type || 'unknown'}: ${JSON.stringify(obj).substring(0, 300)}`
			)
			...
			switch (obj.type) {
				...
				case 'item.started': {
					...
					logger.debug(
						`[CodexCLI] item.started type=${itemType}: ${JSON.stringify(item).substring(0, 400)}`
					)

					if (itemType === 'command_execution') {
						const cmd = item.command || ''
						logger.debug(
							`[CodexCLI] Command execution started (should be disabled): ${cmd.substring(0, 200)}`
						)
					} else if (
				...
				case 'item.completed': {
					...
					const itemType = String(item.type || '')
					logger.debug(
						`[CodexCLI] item.completed type=${itemType}: ${JSON.stringify(item).substring(0, 400)}`
					)
					...
					if (itemType === 'agent_message' || itemType === 'message') {
						...
						logger.debug(
							`[CodexCLI] Command execution completed: ${cmd.substring(0, 150)}, exit_code=${item.exit_code}`
						)
					} else {
