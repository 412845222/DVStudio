...
class CLIAdapterManager {
	...
	async listModels(adapterName, options = {}) {
			...
			if (config?.models && config.lastCheckedAt) {
				...
				if (cacheAge < 60 * 60 * 1000) {
					...
					if (validModels.length !== config.models.length) {
						logger.warn(
							`[CLIAdapterManager] Filtered ${config.models.length - validModels.length} invalid model(s) for ${adapterName}, consider refreshing in Settings`
						)
		...
		if (validModels.length !== models.length) {
			logger.warn(
				`[CLIAdapterManager] Filtered ${models.length - validModels.length} invalid model(s) from ${adapterName} adapter`
			)
		}
