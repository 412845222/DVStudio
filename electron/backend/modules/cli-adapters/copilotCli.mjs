...
function getNodeRuntime() {
	try {
		...
		if (nodePath && fs.existsSync(nodePath)) {
			logger.info(`[CopilotCLI] Using system Node: ${nodePath}`)
	...
	if (execPath && fs.existsSync(execPath)) {
		logger.warn(`[CopilotCLI] System Node not found, falling back to Electron: ${execPath}`)
		return { path: execPath, isElectron: true }
	}

	logger.warn('[CopilotCLI] Could not determine Node.js executable path, using "node" as fallback')
...
function ensureDvstudioMcpConfig() {
	try {
		...
		logger.info(
			`[CopilotCLI] MCP bridge path: ${STDIO_BRIDGE_PATH} (exists: ${fs.existsSync(STDIO_BRIDGE_PATH)})`
		)
		logger.info(
			`[CopilotCLI] Node executable path: ${nodePath} (exists: ${fs.existsSync(runtime.path)})`
		)

		if (!fs.existsSync(STDIO_BRIDGE_PATH)) {
			logger.error(`[CopilotCLI] MCP stdio bridge script does not exist: ${STDIO_BRIDGE_PATH}`)
		}
		...
		if (runtime.isElectron) {
			dvstudioConfig.env = { ELECTRON_RUN_AS_NODE: '1' }
		}
		...
		const configChanged =
			...
			JSON.stringify(existingConfig.env) !== JSON.stringify(dvstudioConfig.env)

		if (configChanged) {
			config.mcpServers.dvstudio = dvstudioConfig
			fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n', 'utf8')
			logger.info(
				`[CopilotCLI] DVStudio MCP server config updated in ${configPath}, node: ${nodePath}`
			)
		}
...
export class CopilotCliAdapter extends BaseCLIAdapter {
	...
	async ensureCopilotBinary() {
		...
		logger.info(
			'[CopilotCLI] Copilot binary not found, triggering download via gh copilot --version'
		)
		try {
	...
	async runFixAction(checkKey) {
		switch (checkKey) {
			case 'copilot_extension': {
				try {
					logger.info(
						'[CopilotCLI] Running fix: triggering copilot binary download via gh copilot --version'
					)
					await this.runCommand(['copilot', '--version'], { timeout: 120000 })
					...
					exec(cmd, (error) => {
						if (error) {
							resolve({
								...
								interactive: true,
								message:
									'无法打开终端，请手动运行: gh auth login --web（若有代理请先设置HTTP_PROXY环境变量）'
							})
						} else {
							resolve({
								ok: true,
								output: '',
								interactive: true,
								message: '已打开终端窗口，请在其中完成 GitHub 登录'
							})
						}
					})
				})
	...
	async *sendMessage(sessionId, content, options = {}) {
		...
		const requestedModel = options.model || this.cliConfig.model || 'auto'
		const configuredModels = this.cliConfig.models || FALLBACK_MODELS
		const validModelIds = getValidModelIds(configuredModels)
		const model =
			validModelIds.length > 0 && validModelIds.includes(requestedModel)
				? requestedModel
		...
		if (requestedModel !== model) {
			logger.warn(
				`[CopilotCLI] Model "${requestedModel}" not available in configured models, falling back to "${model}". Please refresh models in Settings.`
			)
		}
		const proxyEnv = getProxyEnvVars()
	...
	processStreamEvent(
		...
		const extractToolInfo = (item) => {
			...
			if (
				...
				const toolInput =
					item.arguments || item.input || item.parameters || item.function?.arguments || {}
				const parsedInput =
		...
		switch (type) {
			...
			case 'tool.execution_start': {
				...
				if (!toolName) {
					logger.debug(
						`[CopilotCLI] Tool call start without tool name, skipping: ${JSON.stringify(data).substring(0, 200)}`
					)
					break
				}
			...
			case 'tool.execution_complete': {
				...
				if (!activeTc || !activeTc.name) {
					logger.debug(
						`[CopilotCLI] Tool call complete without active tool, skipping: ${JSON.stringify(data).substring(0, 200)}`
					)
					if (toolCallId) {
					...
					if (result !== undefined && result !== null) {
						if (typeof result === 'string') {
							...
							if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
								try {
									resultOutput = JSON.parse(trimmed)
								} catch {
									resultOutput = result
								}
							} else {
			...
			default:
				logger.debug(
					`[CopilotCLI] unhandled event: ${type}`,
