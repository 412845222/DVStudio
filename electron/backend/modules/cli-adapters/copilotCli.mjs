...
function ensureDvstudioMcpConfig() {
  try {
    ...
    logger.info(`[CopilotCLI] MCP bridge path: ${STDIO_BRIDGE_PATH} (exists: ${fs.existsSync(STDIO_BRIDGE_PATH)})`);
    logger.info(`[CopilotCLI] Node executable path: ${nodePath} (exists: ${fs.existsSync(runtime.path)})`);
    ...
    if (configChanged) {
      ...
      logger.info(`[CopilotCLI] DVStudio MCP server config updated in ${configPath}, node: ${nodePath}`);
    }
...
export class CopilotCliAdapter extends BaseCLIAdapter {
  ...
  async *sendMessage(sessionId, content, options = {}) {
    ...
    const model = validModelIds.length > 0 && validModelIds.includes(requestedModel)
      ? requestedModel
      : validModelIds.length > 0 ? validModelIds[0] : 'auto';
    if (requestedModel !== model) {
      logger.warn(`[CopilotCLI] Model "${requestedModel}" not available in configured models, falling back to "${model}". Please refresh models in Settings.`);
    }
    const proxyEnv = getProxyEnvVars();
