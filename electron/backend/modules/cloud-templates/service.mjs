...
export class CloudTemplatesService {
			...
			this._boundOnPlatformChange = () => {
				if (this._adapter) {
					...
					if (currentPlatformId !== newPlatformId) {
						console.log(
							'[cloud-templates] Platform changed from',
							currentPlatformId,
							'to',
							newPlatformId,
	...
	_initAdapter() {
		...
		console.log(
			'[cloud-templates] adapter initialized:',
	...
	_shouldReinitAdapter() {
		...
		if (currentPlatformId !== expectedPlatformId) {
			console.log(
				'[cloud-templates] Adapter platform mismatch: adapter=' +
					currentPlatformId +
					', active=' +
					expectedPlatformId +
					' - reinitializing'
	...
	async _loadIndexFromCloud() {
		...
		try {
			...
			if (allFilesResult.ok && allFilesResult.items) {
				...
				console.log(
					'[cloud-templates] Could not list files:',
					allFilesResult.errMsg || 'unknown error'
				)
			}
			...
			console.log(
				'[cloud-templates] Index file exists check for',
				INDEX_FILE,
				':',
				JSON.stringify(existsResult)
			)
			...
			if (!fileExists) {
				...
				if (prefixFilesResult.ok && prefixFilesResult.items && prefixFilesResult.items.length > 0) {
					console.warn(
						'[cloud-templates] WARNING: Template files exist but index.json is missing! Files:',
			...
			console.log(
				'[cloud-templates] fileRead result:',
				result.ok ? `buffer size: ${result.buffer?.length || 0}` : `error: ${result.errMsg}`
			)
			...
			console.log(
				'[cloud-templates] Index content length:',
				content.length,
				'first 500 chars:',
			...
			if (!index || !Array.isArray(index.templates)) {
				console.warn(
					'[cloud-templates] Invalid index structure, resetting. templates field:',
					typeof index?.templates
				)
				return { ok: true, index: null, fromCloud: false, error: 'Invalid index structure' }
			}

			console.log(
				'[cloud-templates] Parsed index from cloud, templates count:',
				index.templates.length
			)
			for (const t of index.templates) {
				console.log(
					'[cloud-templates]  - cloud template:',
					t.id,
					t.name,
					'package:',
					t.packageFileName
	...
	async _getIndex(options = {}) {
		...
		if (this._cachedIndex && this._indexLoaded && !forceRefresh) {
			console.log(
				'[cloud-templates] Using cached index, templates:',
				this._cachedIndex.templates.length
			)
		...
		if (result.fromCloud && result.index) {
			console.log(
				'[cloud-templates] Caching index from cloud, templates:',
				result.index.templates.length
			)
	...
	async _saveIndex(index) {
		...
		console.log(
			'[cloud-templates] Saving index to cloud, size:',
			buffer.length,
			'bytes, templates:',
			...
			if (verifyExists.ok && verifyExists.exists) {
				...
				if (readBack.ok && readBack.buffer && readBack.buffer.length > 0) {
					try {
						...
						console.log(
							'[cloud-templates] Index verified after write, templates:',
							verified.templates?.length || 0
						)
					} catch (e) {
						console.error(
							'[cloud-templates] Index verification failed - parse error:',
							e.message,
							'content length:',
							readBack.buffer.length
						)
					}
				} else {
					console.error(
						'[cloud-templates] Index verification failed - could not read back:',
						readBack.errMsg,
						'buffer:',
						readBack.buffer?.length || 0
					)
				}
			} else {
				console.error(
					'[cloud-templates] Index verification failed - file does not exist after write!'
				)
			}
		} else {
	...
	async uploadTemplate(options) {
		...
		console.log('[cloud-templates] service.uploadTemplate:', {
			id,
			name,
			zipSize: zipBuffer?.length,
			hasCover: !!coverBuffer?.length
		})

		const adapter = this.getAdapter()
		console.log(
			'[cloud-templates] using adapter:',
			adapter.getPlatformId(),
			adapter.getPlatformName()
		)

		const packageFileName = `${TEMPLATE_PATH_PREFIX}${id}.zip`
		const coverFileName =
			coverBuffer && coverBuffer.length > 0 ? `${TEMPLATE_PATH_PREFIX}${id}_cover.png` : ''

		console.log(
			'[cloud-templates] writing package file:',
			packageFileName,
			'size:',
			zipBuffer.length
		)
		...
		if (coverFileName) {
			console.log(
				'[cloud-templates] writing cover file:',
				coverFileName,
				'size:',
				coverBuffer.length
			)
		...
		try {
			...
			if (listResult.ok) {
				console.log(
					'[cloud-templates] Files in cloud storage after upload:',
	...
	async downloadTemplate(templateId) {
		...
		if (!packageResult.ok || !packageResult.buffer || packageResult.buffer.length === 0) {
			console.error(
				'[cloud-templates] Failed to read package:',
				packageResult.errMsg,
				'buffer size:',
				packageResult.buffer?.length || 0
			)
	...
	async listTemplates(options = {}) {
		try {
			...
			const adapter = this.getAdapter()
			console.log(
				'[cloud-templates] listTemplates using adapter:',
				adapter?.getPlatformId?.(),
				'available:',
				adapter?.isAvailable?.()
			)
			...
			console.log(
				'[cloud-templates] Returning',
				index.templates?.length || 0,
				'templates, quota:',
				quotaResult.ok ? `${JSON.stringify(quotaResult.quota)}` : quotaResult.errMsg
			)
