...
const TOOLS = [
		...
		inputSchema: {
			...
			properties: {
				type: {
					type: 'string',
					description:
						'节点类型ID（actionId），如 text-generation、image-generation、video-generation、scene-understanding、scene-layout、scene-decompose、comfyui、model3d、blender、rotate-image、unreal-export 等'
				},
				title: {
					type: 'string',
	...
	},
	{
		...
		description:
			'【Blender工具】在Blender中执行任意Python(bpy)代码。代码执行后必须将结果赋值给名为result的字典变量。这是核心工具，所有场景操作都通过它完成。使用前需要用户先在Blender节点面板中连接Blender。',
		inputSchema: {
			type: 'object',
			properties: {
				code: {
	...
	},
	{
		...
		description:
			'【Blender工具】获取场景的集合层级结构和所有对象列表（名称、类型、位置、可见性、选中状态），以及材质/相机/灯光名称列表。开始操作前优先调用。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
		name: 'blender_get_object_detail_summary',
		description:
			'【Blender工具】获取指定对象的结构化详细信息，包括变换、尺寸、父子关系、修改器、约束、材质、可见性、所属集合等。',
		inputSchema: {
			type: 'object',
	...
	},
	{
		...
		description:
			'【Blender工具】返回blend文件的数据块统计（各类datablock数量）、当前工作区、渲染引擎和Blender版本。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
		name: 'blender_get_blendfile_summary_missing_files',
		description:
			'【Blender工具】报告磁盘上缺失的外部文件引用（图片、链接库、字体、声音、视频、缓存、序列等）。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
	...
	},
	{
		...
		description:
			'【Blender工具】获取blend文件的路径、保存状态、是否修改未保存、文件存在时长、大小和备份文件列表。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
		name: 'blender_get_blendfile_summary_usage_guess',
		description:
			'【Blender工具】猜测当前blend文件的主要用途（建模/渲染/动画/合成/几何节点/视频编辑/脚本/油脂笔/UV展开/运动跟踪/音频等），每项给出0-100分和置信度。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
		name: 'blender_get_screenshot_of_area_as_image',
		description:
			'【Blender工具】截取Blender中指定类型区域的截图并返回PNG图片。默认截取VIEW_3D区域，自动缩放到合适大小。修改场景后调用此工具验证结果。',
		inputSchema: {
			type: 'object',
			properties: {
				area_type: {
				...
				size_limit_in_bytes: {
					type: 'number',
					description: '图片大小限制（字节），默认786432(768KB)',
					default: 786432
				}
			}
		}
	...
	},
	{
		...
		description:
			'【Blender工具】返回Blender窗口布局的JSON描述，包括窗口尺寸、工作区、场景、所有区域类型/位置/大小、活动空间类型、活动对象和选中对象列表。',
		inputSchema: { type: 'object', properties: {} }
	},
	{
		name: 'blender_jump_to_tab_by_name',
		description:
			'【Blender工具】按名称切换到指定工作区（标签页），如"Modeling"、"Rendering"、"Animation"、"UV Editing"、"Scripting"、"Compositing"等。',
		inputSchema: {
			type: 'object',
	...
	},
	{
		...
		description:
			'【Blender工具】切换到主区域为指定空间类型的工作区。可选择是否在不存在时自动创建新工作区。',
		inputSchema: {
			type: 'object',
			properties: {
				space_type: {
	...
	},
	{
		...
		description:
			'【Blender工具】在3D视口中选中并聚焦到指定对象。可选择是否自动显示隐藏对象和启用集合。会退出相机视角并框选对象。',
		inputSchema: {
			type: 'object',
			properties: {
	...
	},
	{
		...
		description:
			'【Blender工具】导入3D模型文件到Blender当前场景。支持.glb/.gltf/.fbx/.obj/.stl/.dae等格式。',
		inputSchema: {
			type: 'object',
	...
	},
	{
		...
		description:
			'【Blender工具】读取工作区中的图片文件并返回图片内容（用于视觉分析）。可以用来重新查看之前保存的截图或参考图。',
		inputSchema: {
			type: 'object',
			properties: {
				path: {
					type: 'string',
					description:
						'图片的相对路径（相对于工作区根目录），如 "screenshots/20240101_120000.png" 或 "references/ref.png"。也可以只传文件名。'
				}
			},
			required: ['path']
		}
	},
	{
		...
		description:
			'【Blender工具】列出工作区中所有可用的图片文件（screenshots和references目录），返回文件名、相对路径、绝对路径和大小。',
		inputSchema: { type: 'object', properties: {} }
	}
]
...
function sendToElectron(requestData) {
	return new Promise((resolve, reject) => {
		...
		client.on('error', (err) => {
			reject(
				new Error(`Cannot connect to DVStudio: ${err.message}. Please ensure DVStudio is running.`)
			)
		})
	})
}
			...
			if (Array.isArray(message)) {
				...
				if (firstId !== undefined) {
					sendErrorResponse(
						firstId,
						-32600,
						'Batch requests are not supported. Please send one request at a time (strictly sequential tool calls).'
					)
				}
				process.stderr.write(
					'[DVStudio MCP Bridge] Rejected JSON-RPC batch request - sequential execution required\n'
				)
				continue
			}
