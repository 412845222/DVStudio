"""Utility functions for subtitle understanding.

Extracted from Django utils, removing Django-specific imports.
"""
...
def try_parse_json(text: str) -> Any:
    """Parse JSON, fallback to extracting first object."""
    raw = text.strip()
    if not raw:
        return None
    try:
...
def to_safe_str(v: Any) -> str:
    """Convert to string safely."""
    return str(v) if isinstance(v, str) else ""


def extract_top_bigrams(texts: List[str], *, limit: int = 6) -> List[str]:
    """Extract frequent 2-char tokens from texts."""
...
def normalize_cue_index(v: Any, *, min_v: int, max_v: int) -> int:
    """Normalize cue index to valid range."""
    try:
        x = int(v)
    except Exception:
        x = min_v
...
def repair_component_template(t: Any, fallback_id: str) -> Dict[str, Any]:
    """Repair component template structure."""
        ...
        for p in params:
            if isinstance(p, dict) and str(p.get("key") or "").strip() == k:
                ...
                if not isinstance(t0, str) or t0.strip() not in (
                    "string", "number", "boolean", "color", "asset:image"
                ):
                    p["type"] = "string"
                return
    ...
    if not out.get("nodes"):
        out["nodes"] = [
            {"localId": "root", "type": "rect", "props": {}, "transform": {"width": 800, "height": 300}}
        ]
...
def validate_unique_root(template: Dict[str, Any]) -> tuple[bool, str]:
    """Validate template has exactly one top-level root node."""
...
def normalize_glow_filters(template: Dict[str, Any]) -> None:
    """Normalize glow filter intensity."""
    nodes_val: Any = template.get("nodes")
    nodes: List[Any] = nodes_val if isinstance(nodes_val, list) else []
...
def collect_palette_colors(prompt_input: Any) -> Dict[str, str]:
    """Extract palette dict from promptInput."""
...
def enforce_palette_whitelist(template: Dict[str, Any], palette: Dict[str, str]) -> None:
    """Replace colors in template with palette whitelist."""
    if not palette:
        return
...
def ensure_glow_filter(template: Dict[str, Any], palette: Dict[str, str]) -> None:
...
def postprocess_component_template(msg_obj: Dict[str, Any], prompt_input: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        import uuid
        repaired = repair_component_template(tpl, fallback_id=str(uuid.uuid4()))

        normalize_glow_filters(repaired)

        pal = collect_palette_colors(prompt_input)
        ...
        if pal and (locked or True):
            enforce_palette_whitelist(repaired, pal)
        if require_glow:
            ensure_glow_filter(repaired, pal)
