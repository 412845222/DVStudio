...
ComponentTemplate.params 规则（强制，避免前端校验失败）：
- params 必须是数组（可为空数组）。
- params[] 每一项必须是对象：至少包含 key 与 type。
- key 必须是非空字符串（不得为 "" 或纯空白）。
- type 只能是：string | number | boolean | color | asset:image（其他值一律视为不合法）。
- 禁止输出缺少 key 的 params 项；如果不确定某个参数是否需要：宁可删除该 params 项，也不要输出空 key 占位。
