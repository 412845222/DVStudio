...
export async function runCli(rawArgv) {
    ...
    if (argv.includes('--version') || argv.includes('-v')) {
        if (argv.includes('--json')) {
            ...
            process.stdout.write(`dvscli v${process.env.npm_package_version || '0.2.4'} (Node.js ${process.version})\n`)
        }
    ...
    if (argv.includes('--help') || argv.includes('-h') || argv.length === 0
        ...
        const sub = (() => {
            ...
            if (helpIdx >= 0 && argv[helpIdx + 1] && !argv[helpIdx + 1].startsWith('-')) return argv[helpIdx + 1]
            return null
        })()
        const ctx = {
            argv,
    ...
    try {
        ...
        const skipGlobalFlags = () => {
            ...
            while (i < argv.length) {
                if (argv[i].startsWith('--') || argv[i].match(/^-[a-z]$/i)) {
                    // 带值的选项跳过下一个
                    if (['--host', '--port', '--token', '--timeout'].includes(argv[i]) && !argv[i].includes('=')) i++
                    i++
                    continue
                }
                return i
        ...
        switch (command) {
            ...
            case 'generate-image': case 'generateImage': case 'gen-img':
            ...
            case 'tools': {
                ...
                if (runtimeCtx.isJson) process.stdout.write(JSON.stringify({ ok: false, error: 'NOT_IMPLEMENTED', command: `tools ${sub}` }) + '\n')
            ...
            case 'task': {
                ...
                if (runtimeCtx.isJson) process.stdout.write(JSON.stringify({ ok: false, error: 'INVALID_PARAMS', message: 'usage: dvscli task <query|cancel> <taskId>' }) + '\n')
                else printText('用法: dvscli task <query|cancel> <taskId>')
                return EXIT_CODES.INVALID_PARAMS
            }
            default:
                if (runtimeCtx.isJson) {
                    process.stdout.write(JSON.stringify({ ok: false, error: 'INVALID_PARAMS', message: `Unknown command: ${command}` }) + '\n')
                } else {
                    printText([
                        `dvscli: 未知命令 '${command}'`,
                        '',
                        `运行 'dvscli help' 查看可用命令列表`
                    ])
                }
                return EXIT_CODES.INVALID_PARAMS
        }
