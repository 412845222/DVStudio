...
function getRuntimeCandidates() {
    ...
    if (process.platform !== 'win32') {
        candidates.push(path.join(os.homedir(), '.dvstudio', 'DVSResource', 'Runtime', RUNTIME_FILENAME))
    }
    return candidates
}
    ...
    return new Promise((resolve) => {
        const req = http.get({
            ...
            res.on('end', () => {
                ...
                } catch { resolve({ found: false, port }) }
            })
        })
        req.on('error', () => resolve({ found: false, port }))
        req.on('timeout', () => { req.destroy(); resolve({ found: false, port }) })
    })
}
...
export async function discoverInstance(argv = []) {
    ...
    return { found: false, host: explicitHost, port: 0, token: explicitToken || '', source: 'not-found' }
}
