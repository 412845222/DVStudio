...
| `comfyui_bridge` | `django-app/comfyui_bridge/` | ComfyUI 桥接；同时是**三方 API 共享实现库**（Meshy / Seedance / NanoBanana / SeeDream / 即梦 的实现函数都在 `api.py`） |
...
## 3. AI 对话与 SSE（`dwebapp/ai/api/chat/`）

> 实现已被拆分到 `dwebapp/ai/api/chat/` 包；`chat_api.py` 仍然作为 re-export 兼容旧 import 路径。
...
- `component_template/`：组件模板生成
- `outline_style/`：大纲 → 样式
- `conversation_component/`：对话式组件
- `video_gui/`：视频 GUI 规划
- `protocol/`：Agent ↔ UI 协议
- 每个 skill 子目录下通常包含：`agent.py`（实现）+ 多个 `*.md`（提示词 / 模板）
- 共用工具：`dwebapp/ai/_md_prompts.py`

## 5. CodeX / Copilot 桥接（`codex_bridge/`）
...
## 7. ComfyUI + 三方 API 网关（`comfyui_bridge/` + `third_party_api_gateway/`）
- `comfyui_bridge/api.py`：核心实现，包含 ComfyUI、Meshy、Seedance、NanoBanana、SeeDream、即梦等所有三方服务的调用函数
- `comfyui_bridge/models.py`：镜像表（含 `MeshyTaskMirror` / `VideoGenerationTaskMirror`）
- `comfyui_bridge/urls.py`：对外暴露的路由
- `third_party_api_gateway/models.py`：`VideoGenerationTaskMirror` 实际定义
- 路由前缀：`/api/workflow/`（mount 在 `dwebsite/urls.py`）
...
### 8.2 Electron LocalDB（better-sqlite3）
- 位于 `electron/localdb/`，**与 Django SQLite 是双轨**：
  - **LocalDB 是运行时事实来源**（新项目/任务/凭证都先写这里）
  - Django SQLite 主要承担"迁移期镜像"角色（避免重新发明 schema 迁移）
...
| Agent Skills | `django-app/agentSkills/views.py` |
