...
async function main() {
  ...
  // Init localdb
  const { initLocalDb, getRepos } = await import(path.join(projectRoot, 'electron', 'localdb', 'index.mjs'))
  ...
  for (const task of allTasks) {
    ...
    if (task.responsePayload) {
      try {
        const resp = typeof task.responsePayload === 'string' ? JSON.parse(task.responsePayload) : task.responsePayload
