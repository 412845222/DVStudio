...
const requestJson = (path, payload, method = 'POST') =>
			...
			(res) => {
				...
				res.on('end', () => {
					...
					if ((res.statusCode || 0) < 200 || (res.statusCode || 0) >= 300) {
						reject(
							new Error(`${method} ${url.pathname} failed: ${res.statusCode} ${raw.slice(0, 1000)}`)
						)
						return
					}
					resolve(data)
		...
		const req = http.request(
			...
			(res) => {
				if ((res.statusCode || 0) < 200 || (res.statusCode || 0) >= 300) {
					...
					res.on('end', () =>
						reject(new Error(`stream failed: ${res.statusCode} ${raw.slice(0, 1000)}`))
					)
					return
				}
				...
				res.on('end', () => {
					...
					if (!seen.includes('turn_done'))
						reject(new Error(`stream ended before turn_done; seen=${seen.join(',')}`))
				})
			}
		)
		req.on('error', reject)
...
const main = async () => {
	...
	console.log(
		`[copilot-sse] health provider=${health?.provider || ''} reachable=${health?.reachable}`
	)
	if (health?.provider !== 'copilot-cli')
		throw new Error(`unexpected provider: ${health?.provider}`)
	if (health?.reachable !== true)
		throw new Error(`copilot health not reachable: ${health?.error || ''}`)
	...
	if (session?.provider !== 'copilot-cli')
		throw new Error(`session provider mismatch: ${session?.provider}`)
	console.log(`[copilot-sse] session=${session.id} model=${session.model_name}`)

	const result = await streamSse(
		`/api/workflow/copilot/sessions/${encodeURIComponent(session.id)}/messages:stream`,
		{
			projectId,
			content,
			agentMode,
	...
	if (!result.seen.includes('assistant_delta'))
		throw new Error(`assistant_delta missing; seen=${result.seen.join(',')}`)
	if (!result.seen.includes('turn_done'))
		throw new Error(`turn_done missing; seen=${result.seen.join(',')}`)
	console.log(
		`[copilot-sse] ok events=${result.seen.join(',')} text=${JSON.stringify(result.assistantText)} elapsed=${result.elapsedMs}ms`
	)
}
