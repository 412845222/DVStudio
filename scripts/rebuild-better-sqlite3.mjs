  ...
  try {
    ...
    if (hasWinShim || hasNixShim) {
      res = spawnSync(hasWinShim ? winShim : nixShim, [probeScript], {
        ...
        timeout: 90 * 1000, encoding: 'utf-8', windowsHide: true, shell: IS_WIN,
      })
    } else {
      ...
      res = spawnSync(
        ...
        { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'], timeout: 120 * 1000, encoding: 'utf-8', windowsHide: true, shell: true },
      )
    }
    const combined = String(res.stdout || '') + '\n' + String(res.stderr || '')
    if (combined.includes('DWEB_ELECTRON_PROBE_OK')) return { ok: true, raw: combined.slice(0, 400) }
...
function tryElectronRebuild(electronVersion) {
  ...
  const cmd2 = 'npm rebuild better-sqlite3 --runtime=electron --target=' + electronVersion + ' --disturl=https://electronjs.org/headers --build-from-source'
...
console.log('[rebuild-better-sqlite3] target: Electron=' + electronVersion +
  ' (runtime=' + process.platform + ' node=' + process.version + ' modules=' + process.versions.modules + ')')
...
if (!existsSync(pkgPath)) {
  ...
  if (!ok) fail('better-sqlite3 未安装，且首次重建失败。请检查本机是否已安装 VS Build Tools + Python。')
  process.exit(0)
}
...
// Step 3: 需要重建
console.log('[rebuild-better-sqlite3] 需要重建（当前二进制与 Electron ABI 不兼容）。开始针对 Electron ' + electronVersion + ' 重建...')
...
if (rebuiltOk) {
  ...
  if (after.ok) {
    console.log('[rebuild-better-sqlite3] 重建后 Electron probe 通过 ✅ (current binaries: ' + JSON.stringify(enumerateBetterSqlite3Binaries()) + ')')
    process.exit(0)
  }
...
// Step 5: 致命失败
fail(
  '无法为 Electron ' + electronVersion + ' 生成可用的 better-sqlite3 原生二进制。\n' +
  '  请手动执行（任选其一）：\n' +
