...
const parseAssetRequest = (url) => {
	...
	try {
		...
		const parsed = base
			? new URL(text.replace(/^dweb:\/\/project-assets/, `${base}/project-assets`))
			: new URL(text)
		const search = parsed.searchParams
		const variant = String(search.get('variant') || search.get('mode') || '')
			.trim()
			.toLowerCase()
		...
		if (qIndex < 0)
			return { url: text, variant: '', isPreview: false, maxSize: 0, path: '', projectId: '' }
		const qs = new URLSearchParams(text.slice(qIndex + 1))
		const variant = String(qs.get('variant') || qs.get('mode') || '')
			.trim()
			.toLowerCase()
...
const main = () => {
	...
	const payload = {
		...
		summary: {
			...
			originalRequests,
			previewRatio:
				totalAssetRequests > 0 ? Number((previewRequests / totalAssetRequests).toFixed(4)) : 0,
			originalRatio:
				totalAssetRequests > 0 ? Number((originalRequests / totalAssetRequests).toFixed(4)) : 0,
			preview4xx5xx,
			original4xx5xx,
			overlapPathCount
		},
		previewSizeDistribution: Object.fromEntries(
			[...previewSizes.entries()].sort((a, b) => Number(a[0]) - Number(b[0]))
		),
		topPreviewPaths: [...previewPathHit.entries()]
			.sort((a, b) => b[1] - a[1])
			.slice(0, 20)
			.map(([pathValue, count]) => ({ path: pathValue, count })),
		topOriginalPaths: [...originalPathHit.entries()]
			.sort((a, b) => b[1] - a[1])
			.slice(0, 20)
			.map(([pathValue, count]) => ({ path: pathValue, count }))
	}
	...
	const outPath = toAbs(
		args.out || path.join(path.dirname(harPath), `aiwf-preview-audit-${Date.now()}.json`)
	)
	...
	console.log(
		`[aiwf-asset-preview-audit] total=${totalAssetRequests} preview=${previewRequests} original=${originalRequests}`
	)
}
