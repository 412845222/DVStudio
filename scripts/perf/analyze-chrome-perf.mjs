...
function analyzeHar(har) {
  ...
  const topTime = topNByMap(byPathTime, 20).map((r) => ({ path: r.key, totalTimeMs: Number(r.value.toFixed(2)) }));
  ...
  summary.avgEntryTimeMs = entries.length ? Number((summary.totalTimeMs / entries.length).toFixed(2)) : 0;
...
function renderMarkdown(data) {
  ...
  if (har.pollingHints.length === 0) {
    ...
    for (const p of har.pollingHints) {
      lines.push(`- ${p.endpoint}: count=${p.count}, median=${p.medianIntervalMs}ms, min=${p.minIntervalMs}ms, max=${p.maxIntervalMs}ms`);
    }
  }
  lines.push('');
  ...
  lines.push('- If polling median interval is low (<1000ms), throttle or pause endpoint polling during heavy interaction.');
  lines.push('- If package:// or ERR_UNKNOWN_URL_SCHEME appears, trace caller stack in renderer path that constructs snapshot URLs.');
