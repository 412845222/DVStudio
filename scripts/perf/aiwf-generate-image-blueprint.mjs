...
const main = () => {
  ...
  for (let i = 0; i < resourceCount; i += 1) {
    ...
    const relPath = `${String(args.mediaRelRoot || '').replace(/\\/g, '/').replace(/\/$/, '')}/${file}`;
  ...
  console.log(`[aiwf-generate-image-blueprint] nodes=${nodeOrder.length} edges=${edgeOrder.length} resources=${resourceOrder.length}`);
};
