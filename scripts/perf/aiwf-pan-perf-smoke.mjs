...
const parseArgs = (argv) => {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--batch' && next) {
      args.batch = next;
      const tiers = next.split(',').map((s) => s.trim().toUpperCase()).filter(Boolean);
...
const generateBatchSummary = (batchResults, outDir, stamp) => {
  const rows = batchResults.map((r) => {
    ...
    const thresholdFps = r.scenario.startsWith('S0') ? 55 : r.scenario.startsWith('S1') ? 50 : r.scenario.startsWith('S2') ? 40 : 30;
    ...
    return {
      ...
      edgeCullHitRateDisplay: diag.edgeCullHitRate != null ? `${(diag.edgeCullHitRate * 100).toFixed(1)}%` : '--',
      fpsPass,
      framePass,
      edgeCullPass,
...
const main = () => {
  ...
  if (args.batchConfigs && args.batchConfigs.length > 0) {
    ...
    console.log(`[aiwf-pan-perf-smoke] batch complete: ${batchResults.length}/${args.batchConfigs.length} scenarios`);
    return;
  }
