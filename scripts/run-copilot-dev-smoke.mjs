...
const children = []
const backendCandidates = String(process.env.DWEB_COPILOT_DEV_BACKEND_PORTS || '5800,5810,5811,5812')
  .split(',')
  .map((item) => Number(item.trim()))
  .filter((item) => Number.isFinite(item) && item > 0)
const frontendCandidates = String(process.env.DWEB_COPILOT_DEV_FRONTEND_PORTS || '5173,5174,5175,5176')
  .split(',')
  .map((item) => Number(item.trim()))
  .filter((item) => Number.isFinite(item) && item > 0)
...
const getJson = (port, path) => new Promise((resolve, reject) => {
  const req = http.get({ host: '127.0.0.1', port, path, timeout: 2500 }, (res) => {
    ...
    res.on('end', () => {
      ...
      try { resolve(JSON.parse(raw)) } catch { reject(new Error(`invalid json from ${path}`)) }
    })
  })
  req.on('error', reject)
...
const main = async () => {
  ...
  await new Promise((resolve, reject) => {
    const migrate = spawn('python', ['django-app/manage.py', 'migrate'], { cwd: root, stdio: 'inherit' })
  ...
  if (!backend.reuse) {
    spawnChild('django', 'python', ['django-app/manage.py', 'runserver', String(backend.port), '--noreload'])
  } else {
    console.log(`[copilot-dev-smoke] reusable django already listening on ${backend.port}`)
  }

  if (!frontend.reuse) {
    spawnChild('vite', 'npx', ['--no-install', 'vite', '--host', '127.0.0.1', '--port', String(frontend.port), '--strictPort'])
  } else {
  ...
  await new Promise((resolve, reject) => {
    ...
    check.on('exit', (code) => code === 0 ? resolve() : reject(new Error(`copilot sse check exited ${code}`)))
  })
  ...
  const verifyDeadline = Date.now() + Number(process.env.DWEB_COPILOT_DEV_VERIFY_TIMEOUT_MS || 30000)
  ...
  while (Date.now() < verifyDeadline) {
    try {
      ...
      for (const projectId of candidateProjectIds) {
        const sessionsRes = await getJson(backend.port, `/api/workflow/copilot/sessions?projectId=${encodeURIComponent(String(projectId))}`)
