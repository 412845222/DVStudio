...
function main() {
	...
	if (!fs.existsSync(dwebSteamDir)) {
		console.log(
			'[setup-steam-native] DwebSteamJS directory not found, skipping deployment (Mock mode only)'
		)
		return
	}
	...
	if (!fs.existsSync(buildReleaseDir)) {
		console.log(
			'[setup-steam-native] DwebSteamJS build/Release not found, run "cd DwebSteamJS && npm run build:electron" first'
		)
	...
	if (fs.existsSync(STEAM_APPID_SRC)) {
		...
		if (!fs.existsSync(defaultAppIdPath)) {
			...
			if (appId) {
				...
				console.log(
					'[setup-steam-native] Warning: No Steam AppID configured, skipping steam_appid.txt creation'
				)
				console.log(
					'[setup-steam-native] Create steam.config.json with your appId, or place steam_appid.txt in project root'
				)
			}
		}
	}
