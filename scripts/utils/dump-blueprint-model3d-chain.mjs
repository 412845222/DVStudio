...
const TARGET_IDS = ['node_1785687020687_euli2i', 'node_1785699396436_irgzpo']
...
for (const ID of TARGET_IDS) {
	...
	function printObject(obj, depth = 0, prefix = '') {
		...
		for (const k of keys) {
			...
			if (typeof v === 'string') {
				const hasKeyword =
					/(task|model|asset|source|path|url|format|id|glb|gltf|meshy|local|relative|project|output|summary|thumbnail|preferred|effective)/i.test(
						k
					) || /(019fc|meshy|glb|gltf|Content|dweb|localAsset|taskId)/i.test(v)
				if (hasKeyword || v.length < 120) {
					console.log(
						`${indent}${k}: ${JSON.stringify(v.length > 300 ? v.slice(0, 300) + '...' : v)}`
					)
				}
				...
				for (let i = 0; i < preview.length; i++) {
					if (typeof preview[i] === 'object' && preview[i] !== null) {
						console.log(
							`${indent}  [${i}]: (object keys=${Object.keys(preview[i]).slice(0, 10).join(',')})`
						)
				...
				const hasUsefulKey = subKeys.some((sk) =>
					/(task|model|asset|source|path|url|format|id|glb|gltf|meshy|local|relative|project|output|summary|thumbnail|preferred|effective|taskId|Task|Output|Relation)/i.test(
						sk
					)
				)
				const childHasUsefulValue =
					hasUsefulKey ||
					Object.values(v).some((cv) => {
						if (typeof cv === 'string')
							return /(019fc|meshy|glb|gltf|Content|dweb|localAsset|taskId|c79dbd7c)/i.test(cv)
						return false
					})
				if (hasUsefulKey || childHasUsefulValue) {
					console.log(
						`${indent}${k}: (object keys=${subKeys.slice(0, 20).join(',')}${subKeys.length > 20 ? '...' : ''})`
					)
