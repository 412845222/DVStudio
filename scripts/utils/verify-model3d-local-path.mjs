...
for (const node of model3dNodes) {
	...
	const meshyTaskId =
		node.model3dSettings?.meshyModelSettings?.taskId ||
	...
	;[
		...
		if (s[key]) {
			...
			const abs = isRel
				? path.join(PROJECT_ROOT, String(s[key]).replace(/^[\\/]+/, ''))
				: String(s[key])
			candidates.push({
				src: label,
				raw: s[key],
				abs,
				fileUrl: /^dweb:|^https?:|^file:/.test(String(s[key]).toLowerCase())
					? ''
					: winAbsToFileUrl(abs)
			})
		}
	...
	for (const c of candidates) {
		...
		const extOk = c.abs
			? /\.(glb|gltf|fbx|obj|stl|usdz)$/i.test(c.abs)
			: /\.(glb|gltf|fbx|obj|stl|usdz)/i.test(c.raw)
