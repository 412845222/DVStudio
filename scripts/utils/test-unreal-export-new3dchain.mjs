	...
	const valid = slots
		.filter(
			(s) =>
				s &&
				typeof s === 'object' &&
				String(s.slotId ?? '').trim() &&
				String(s.sourceObjectId ?? '').trim()
		)
		.map((s) => ({ ...s }))
...
function prepareResolvedSlotsForExport_EXPECTED(rawSlots, connectedModelBindings, layoutItems) {
		...
		if (!String(finalSlot.displayName ?? '').trim()) {
			...
			finalSlot.displayName =
				isClone && cloneCount > 1
					? `${finalSlot.objectName} [${cloneIndex + 1}/${cloneCount}]`
					: finalSlot.objectName
		}
	...
	for (const [objectId, binding] of bindingByObjectId.entries()) {
		if (!processedObjectIds.has(objectId)) {
			...
			warnings.push(
				`Model "${name}" (objectId=${objectId}) has binding but was not found in 3D preview; it will not be exported`
			)
		}
	}
...
function tryBackfillBindingPathsFromStore_EXPECTED(binding, nodesById, resourcesById) {
			...
			for (const out of fn.outputs) {
				...
				for (const src of [out.resolved, out.cached, out.value]) {
					...
					if (typeof src === 'string') {
						...
						const f = detectModelFormatFromPath(
	...
	if (m1 && m1[1]) {
		try {
			relPath = decodeURIComponent(m1[1]).split('?')[0].split('#')[0]
		} catch {
			/* ignore */
		}
...
function mergeViewerResolvedIntoFinalBindings_EXPECTED(
	exportData,
	precheckBindings,
	strict = true
) {
	const resolvedArr = Array.isArray(exportData?.sceneLayoutResolvedModelBindings)
		? exportData.sceneLayoutResolvedModelBindings
		: null
	const used = !!(strict && resolvedArr && resolvedArr.length > 0)
	const finalBindingsSource = used
		? resolvedArr
		: Array.isArray(precheckBindings)
			? precheckBindings
			: []
	...
	const bindings = [
		{
			...
			modelUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/tripo3d/c79dbd7c/model.glb',
			modelAssetUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/tripo3d/c79dbd7c/model.glb',
		...
		},
		{
			...
			sourceNodeType: 'tripo3d',
			modelUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/tripo3d/eaf91245/result.glb',
			modelAssetUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/tripo3d/eaf91245/result.glb',
		...
		},
		{
			...
			modelUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/meshy/c10a/textured.glb',
			modelAssetUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/meshy/c10a/textured.glb',
		...
		},
		{
			...
			modelUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/generic/scene/scene.gltf',
			modelAssetUrl:
				'file:///D:/DVStudioProjects/demo-ue-integration/Content/Media/generic/scene/scene.gltf',
			...
			modelMaterialOverrides: [
				{
					slotName: 'M_Surface',
					materialPath: '/Game/Content/Media/generic/scene/M_Surface.M_Surface'
				}
			]
		}
	]
	...
	const fields = (obj) =>
		obj && typeof obj === 'object'
			? {
					...
					modelAssetProjectRelativePath:
						obj.modelBinding?.modelAssetProjectRelativePath ?? obj.modelAssetProjectRelativePath,
					textureRefs: obj.modelBinding?.textureRefs ?? obj.textureRefs,
					modelMaterialOverrides:
						obj.modelBinding?.modelMaterialOverrides ?? obj.modelMaterialOverrides,
...
function runCaseA(fx) {
	...
	const { slots, warnings } = prepareResolvedSlotsForExport_EXPECTED(
		structuredClone(fx.slots),
		connected,
		fx.layoutItems
	)
	...
	slots.forEach((s, i) => {
		assertTruthy(
			`Case A2: slot[${i}] modelBinding 非空（${s.slotId}）`,
			s.modelBinding && typeof s.modelBinding === 'object' && hasValidModelPath(s.modelBinding),
			{ slot: s }
		)
	})
	slots.forEach((s, i) => {
		...
		if (!expectedBinding) {
			assertTruthy(`Case A3: slot[${i}] (${s.slotId}) 找不到对应 expected binding → FAIL`, false, {
				slot: s
			})
			return
		}
		...
		assertEq(
			`Case A3: slot[${i}] modelAssetUrl 正确不串 (${s.slotId})`,
			actualAssetUrl,
			expectedBinding.modelAssetUrl,
			{ slot: s, binding: expectedBinding }
		)
	})
	slots.forEach((s, i) => {
		const expectedBinding = byObj.get(s.sourceObjectId)
		...
		assertEq(
			`Case A4: slot[${i}] modelAssetProjectRelativePath 透传 (${s.slotId})`,
			actualRel,
			expectedRel,
			{ slot: s, binding: expectedBinding }
		)
...
function runCaseB(fx) {
	...
	const { finalBindingsSource } = mergeViewerResolvedIntoFinalBindings_EXPECTED(exportData, [])
	const connected = buildFinalConnectedModelBindings_EXPECTED(finalBindingsSource)
	const { slots } = prepareResolvedSlotsForExport_EXPECTED(
		structuredClone(fx.slots),
		connected,
		fx.layoutItems
	)
	...
	assertTruthy(
		'Case B1: 分离 gltf slot 的 modelAssetProjectRelativePath 透传',
		genericSlot?.modelBinding?.modelAssetProjectRelativePath,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B2: 分离 gltf slot 的 modelAssetProjectRelativePath 值正确',
		genericSlot.modelBinding.modelAssetProjectRelativePath,
		'Content/Media/generic/scene/scene.gltf',
		{ slot: genericSlot }
	)
	assertTruthy(
		'Case B3: textureRefs 数组被透传',
		Array.isArray(genericSlot.modelBinding.textureRefs) &&
			genericSlot.modelBinding.textureRefs.length === 4,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B4: textureRefs 四个元素 role+rel 正确（Albedo / Normal / Rough / Metal）',
	...
	assertTruthy(
		'Case B5: modelMaterialOverrides 被透传',
		Array.isArray(genericSlot.modelBinding.modelMaterialOverrides) &&
			genericSlot.modelBinding.modelMaterialOverrides.length >= 1,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B6: modelMaterialOverrides[0].slotName === M_Surface',
		genericSlot.modelBinding.modelMaterialOverrides[0].slotName,
		'M_Surface',
		{ slot: genericSlot }
	)
	assertEq('Case B7: modelFormat= gltf 被保留', genericSlot.modelBinding.modelFormat, 'gltf', {
		slot: genericSlot
	})
	...
	const idx = 3
	assertEq(
		'Case B8a: modelUrl 完全一致',
		genericSlot.modelBinding.modelUrl,
		fx.bindings[idx].modelUrl,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B8b: modelAssetUrl 完全一致',
		genericSlot.modelBinding.modelAssetUrl,
		fx.bindings[idx].modelAssetUrl,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B8c: modelAssetPath 完全一致',
		genericSlot.modelBinding.modelAssetPath,
		fx.bindings[idx].modelAssetPath,
		{ slot: genericSlot }
	)
	assertEq(
		'Case B8d: modelSourcePath 完全一致',
		genericSlot.modelBinding.modelSourcePath,
		fx.bindings[idx].modelSourcePath,
		{ slot: genericSlot }
	)
}
...
function runCaseC(fx) {
	...
	assertEq('Case C1: connected 三种形式都保留，结果 3 条', connected.length, 3, {
		bindings: input,
		connected
	})
	assertTruthy(
		'Case C2: 每条都有 modelAssetUrl（3 条都有）',
		connected.every((b) =>
			String(b.modelAssetUrl ?? '')
				.trim()
				.startsWith('file:///')
		),
		{ connected }
...
function runCaseD(fx) {
	...
	const { finalBindingsSource, usedViewerResolvedBindings } =
		mergeViewerResolvedIntoFinalBindings_EXPECTED(exportData, precheckBindings)
	assertEq(
		'Case D1: finalBindingsSource 优先使用 viewer resolvedBindings（4 条）',
		finalBindingsSource.length,
		4,
		{ finalBindingsSource }
	)
	assertTruthy('Case D2: usedViewerResolvedBindings === true', usedViewerResolvedBindings, {
		usedViewerResolvedBindings
	})

	const finalConnected = buildFinalConnectedModelBindings_EXPECTED(finalBindingsSource)
	assertEq('Case D3: finalConnectedModelBindings 仍然保留 4 条', finalConnected.length, 4, {
		finalConnected
	})
	assertTruthy(
		'Case D4: 每条都有 modelAssetProjectRelativePath 字段透传（不被 filter 裁剪）',
		finalConnected.every((b) =>
			String(b.modelAssetProjectRelativePath ?? '')
				.trim()
				.startsWith('Content/')
		),
		{ finalConnected }
...
function buildSlotsByLayoutItemsAndBindings(layoutItems, bindings) {
	const byId = new Map(
		bindings
			.filter(Boolean)
			.map((b) => [String(b.objectId ?? '').trim(), b])
			.filter(([id]) => id)
	)
	const slots = []
...
function runCaseE(fx) {
	...
	const { slots: onlyViewerSlots } = prepareResolvedSlotsForExport_EXPECTED(
		structuredClone(viewerSlots),
		finalConnected,
		fx.layoutItems
	)
	assertEq(
		'Case E1: 仅 viewer slots 时 → prepared=1 条（prepare 只基于入参 rawSlots）',
		onlyViewerSlots.length,
		1,
		{ onlyViewerSlots }
	)
	...
	const already = new Set(
		onlyViewerSlots.map((s) => String(s.sourceObjectId ?? '').trim()).filter(Boolean)
	)
	for (const s of pureDataSlots) {
		if (!already.has(String(s.sourceObjectId ?? '').trim())) merged.push(s)
	}
	const { slots: finalPrepared, warnings } = prepareResolvedSlotsForExport_EXPECTED(
		merged,
		finalConnected,
		fx.layoutItems
	)
	assertEq('Case E3: pure-data 补齐后，最终 prepared slots = 4', finalPrepared.length, 4, {
		finalPrepared
	})
	for (let i = 0; i < finalPrepared.length; i += 1) {
		...
		assertTruthy(
			`Case E4: prepared slot[${i}] (${s.slotId}) modelBinding 非空`,
			!!s.modelBinding && hasValidModelPath(s.modelBinding),
			{ slot: s }
		)
		assertEq(
...
function runCaseF() {
	...
	const bindings = [
		// 旧链路 bar_main：已经有 modelAssetPath / modelSourcePath
		{
			...
			sourceNodeType: 'model3d',
			modelAssetUrl:
				'dweb://project-assets?projectId=1&path=Content%2FMedia%2Ftripo3d_c79dbd7c.glb',
		...
		// 新链路 stool_left
		{
			...
			sourceNodeType: 'tripo3d',
			modelAssetUrl:
				'dweb://project-assets?projectId=1&path=Content%2FMedia%2Ftripo_stool_left.glb',
	...
	const bindingMap = new Map(
		finalConnected.filter(Boolean).map((b) => [String(b.objectId ?? '').trim(), structuredClone(b)])
	)
	const T = () => ({
		position: { x: 0, y: 0, z: 0 },
		rotation: { yaw: 0, pitch: 0, roll: 0 },
	...
	function mkSlot(slotId, sourceObjectId, bindingObjId, name) {
		...
		return {
			...
			relativeTransform: T(),
			worldTransform: T(),
			previewInstanceTransform: T(),
			previewInstanceWorldTransform: T(),
			slotTransform: T(),
			meshTransform: T(),
			worldBounds: null,
			placeholderTransform: null,
	...
	assertEq('Case F1: 出口过滤后 validSlots = 7（5 房间壳子被丢弃）', slots.length, 7, {
	...
	assertEq(
		'Case F2-2: bar_main 原 modelAssetPath 不被覆盖（保留旧链路值）',
		bar.modelBinding.modelAssetPath,
		PROJ + '\\Content\\Media\\tripo3d_c79dbd7c.glb',
		{ mb: bar.modelBinding }
	)
	assertEq(
		'Case F2-3: bar_main 原 modelSourcePath 不被覆盖',
		bar.modelBinding.modelSourcePath,
		PROJ + '\\Content\\Media\\tripo3d_c79dbd7c.glb',
		{ mb: bar.modelBinding }
	)
	assertEq(
		'Case F2-4: bar_main modelUrl 仍为 file://（不是 dweb），不被替换',
		!!String(bar.modelBinding.modelUrl ?? '').startsWith('file://'),
		true,
		{ modelUrl: bar.modelBinding.modelUrl }
	)
	...
	for (let i = 0; i < 4; i += 1) {
		...
		assertTruthy(`Case F3-${i * 5 + 1}: shelves_back clone_${i} slot 存在`, !!sl, {
			byId: [...byId.keys()]
		})
		assertEq(
			`Case F3-${i * 5 + 2}: shelves_back clone_${i} modelAssetPath 被回填成 relPath`,
			sl.modelBinding.modelAssetPath,
			'Content/Media/meshy_shelves.glb',
			{ mb: sl.modelBinding }
		)
		assertEq(
			`Case F3-${i * 5 + 3}: shelves_back clone_${i} modelSourcePath 被回填成 relPath`,
			sl.modelBinding.modelSourcePath,
			'Content/Media/meshy_shelves.glb',
			{ mb: sl.modelBinding }
		)
		assertTruthy(
			`Case F3-${i * 5 + 4}: shelves_back clone_${i} modelUrl 不再是 dweb://`,
			!String(sl.modelBinding.modelUrl ?? '').startsWith('dweb://'),
			{ modelUrl: sl.modelBinding.modelUrl }
		)
		assertEq(
			`Case F3-${i * 5 + 5}: shelves_back clone_${i} modelUrl 被替换为 relPath`,
			sl.modelBinding.modelUrl,
			'Content/Media/meshy_shelves.glb',
			{ mb: sl.modelBinding }
		)
	}
	...
	assertEq(
		'Case F4-2: stool_left modelAssetPath = relPath',
		sl.modelBinding.modelAssetPath,
		'Content/Media/tripo_stool_left.glb',
		{ mb: sl.modelBinding }
	)
	assertEq(
		'Case F4-3: stool_left modelSourcePath = relPath',
		sl.modelBinding.modelSourcePath,
		'Content/Media/tripo_stool_left.glb',
		{ mb: sl.modelBinding }
	)
	assertTruthy(
		'Case F4-4: stool_left modelUrl 不是 dweb://',
		!String(sl.modelBinding.modelUrl ?? '').startsWith('dweb://'),
		{ modelUrl: sl.modelBinding.modelUrl }
	)
	...
	assertEq(
		'Case F5-2: stool_mid modelAssetPath = relPath',
		sm.modelBinding.modelAssetPath,
		'Content/Media/meshy_stool_mid.glb',
		{ mb: sm.modelBinding }
	)
	assertEq(
		'Case F5-3: stool_mid modelSourcePath = relPath',
		sm.modelBinding.modelSourcePath,
		'Content/Media/meshy_stool_mid.glb',
		{ mb: sm.modelBinding }
	)
	assertTruthy(
		'Case F5-4: stool_mid modelUrl 不是 dweb://',
		!String(sm.modelBinding.modelUrl ?? '').startsWith('dweb://'),
		{ modelUrl: sm.modelBinding.modelUrl }
	)
}
...
function runCaseG() {
	...
	assertEq('Case G-1: 18 个新链路空 binding 都被 Ultimate Backfill 回填了路径', backfillOk, 18, {
		backfillOk
	})
	assertTruthy(
		'Case G-2: bar_main 原本有路径，现在还保留',
		!!processed.find((b) => b.objectId === 'bar_main' && hasAnyPathExtended_EXPECTED(b)),
		{ processed }
	)
	assertTruthy(
		'Case G-3: shell ceiling_main 还是无路径（因为没 sourceNodeId 也没 resourceId）',
		!!processed.find((b) => b.objectId === 'ceiling_main' && !hasAnyPathExtended_EXPECTED(b)),
		{ processed }
	)
	...
	assertEq(
		'Case G-4: validSlots（发送给 UE 的）= 23 个（=1旧 + 4 shelves克隆 + 18新链路）',
		finalSlots.length,
		23,
		{ finalSlots: finalSlots.length, warnings }
	)
	const slotIds = finalSlots.map((s) => s.slotId).sort()
	// 校验 18 个新链路 anchorSuffix 都出现在 slotIds 里
	for (const sfx of anchorSuffixes) {
		assertTruthy(
			`Case G-5 [${sfx}]: 新链路模型 ${sfx} 出现在导出 slots 里`,
			slotIds.includes(sfx),
			{ slotIds }
		)
	}
	...
	for (const sfx of anchorSuffixes) {
		...
		assertTruthy(
			`Case G-6 [${sfx}]: modelAssetPath 非空`,
			!!sl && !!sl.modelBinding.modelAssetPath,
			{ mb: sl?.modelBinding }
		)
		assertEq(
			`Case G-7 [${sfx}]: modelAssetProjectRelativePath = Content/Media/decompose/${sfx}.glb`,
			sl?.modelBinding?.modelAssetProjectRelativePath,
			`Content/Media/decompose/${sfx}.glb`,
			{ mb: sl?.modelBinding }
		)
	}
	// 校验 ceiling_main/floor_main 确实被 last-mile hasAnyPath 过滤丢弃了
	assertTruthy(
		'Case G-8: ceiling_main / floor_main 不在导出 slots 里',
		!slotIds.includes('ceiling_main') && !slotIds.includes('floor_main'),
		{ slotIds }
	)
}
...
function runAll() {
	...
	console.log('\n' + '='.repeat(60))
	console.log(
		`TOTAL: ${ok ? `PASS: ${passed.length}/${total}` : `FAIL: ${failed.length} failed, ${passed.length}/${total} passed`}`
	)
