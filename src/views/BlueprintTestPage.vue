	...
	<div class="blueprint-test-page">
		<div class="toolbar">
			...
			<div class="toolbar-right">
				...
				<span class="divider">|</span>
				<button @click="setSelectedNodeStatus('idle')" :disabled="selectedNodeCount !== 1">
					状态: idle
				</button>
				<button @click="setSelectedNodeStatus('running')" :disabled="selectedNodeCount !== 1">
					状态: running
				</button>
				<button @click="setSelectedNodeStatus('success')" :disabled="selectedNodeCount !== 1">
					状态: success
				</button>
				<button @click="setSelectedNodeStatus('error')" :disabled="selectedNodeCount !== 1">
					状态: error
				</button>
					...
					<div class="hint-content">
						...
						<ul>
							<li>
								🖱️
								<b>双击text/image节点</b>
								：打开NodeChatDialog对话框（Host Overlay层渲染）
							</li>
							<li>
								🖱️
								<b>左键拖拽节点</b>
...
function loadLegacyDemo() {
	...
	setTimeout(() => {
		hostRef.value?.fitToView?.()
		console.log(
			`演示项目加载成功: ${editorInstance.value?.getNodeCount?.()}个节点, ${editorInstance.value?.getEdgeCount?.()}条连线`
		)
	}, 50)
}
	...
	reader.onload = (e) => {
		try {
			...
			setTimeout(() => {
				...
				console.log(
					'蓝图加载成功，节点数:',
					editorInstance.value?.getNodeCount?.(),
					'连线数:',
					editorInstance.value?.getEdgeCount?.()
				)
			}, 50)
		} catch (err) {
