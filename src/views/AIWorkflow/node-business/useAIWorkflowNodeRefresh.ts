...
export const useAIWorkflowNodeRefresh = (payload: {
  ...
  comfyOutputForAnchor: (outputs: ComfyLocalizedOutput[], fromAnchorId: string, expectedKind: 'image' | 'video') => ComfyLocalizedOutput | undefined
  ...
  const onNodeRefresh = async (nodeId: string) => {
    ...
    if (nodeType === 'scene-layout') {
      ...
      for (const upstreamNodeId of upstreamModelNodeIds) {
        ...
        if (upstreamType === 'meshy') {
          const meshySettings = isRecord(upstreamNode.meshySettings) ? upstreamNode.meshySettings : {}
          const meshyRelationSummary = isRecord(meshySettings.meshyRelationSummary) ? meshySettings.meshyRelationSummary : {}
          const taskId = String(
            meshySettings.meshyTaskId ??
            meshyRelationSummary.effectiveTaskId ??
            ''
          ).trim()
          if (!taskId) continue
    ...
    for (const e of incoming) {
      ...
      if (fromNodeType === 'comfyui') {
        ...
        const outputs: ComfyLocalizedOutput[] = Array.isArray(comfySettings.outputs) ? comfySettings.outputs as ComfyLocalizedOutput[] : []
      ...
      if (expectedKind === 'image' && fromNodeType === 'scene-decompose') {
        const fromAnchorId = String(e.fromAnchorId ?? '')
        const outputUrl = String(payload.connectedImageOutputUrl(fromNode, fromAnchorId) ?? '').trim()
        if (!outputUrl) {
          reasons.push('场景分解上游暂无可用图片输出')
          continue
        }
        try {
          const fromNodeAlias = String(fromNode.alias ?? fromNode.title ?? 'decompose').replace(/\s+/g, '_') || 'decompose'
