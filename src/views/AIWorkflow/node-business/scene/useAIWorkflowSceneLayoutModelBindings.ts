...
export const useAIWorkflowSceneLayoutModelBindings = (options: {
  ...
  const sceneLayoutModelInputAnchorId = (objectId: string) => `in-model-${String(objectId ?? '').trim()}`
  ...
  const connectedSceneLayoutModelBindings = (nodeId: string): WorkflowSceneLayoutModelBinding[] => {
    ...
    const layoutItems = allLayoutItems.filter((item: any) => allowedObjectIds.has(String(item?.id ?? '').trim()))
    ...
    return layoutItems.map((item: any) => {
      ...
      if (manualBinding) {
        ...
        return {
          ...
          modelSourcePath: typeof manualBinding.modelSourcePath === 'string' ? String(manualBinding.modelSourcePath).trim() || undefined : undefined,
          modelFormat: manualBinding.modelFormat === 'gltf' ? 'gltf' : manualBinding.modelFormat === 'glb' ? 'glb' : undefined,
        }
      }
      ...
      if (fromNode.type === 'model3d') {
        ...
        return {
          ...
          modelFormat: settings.modelFormat === 'gltf' ? 'gltf' : settings.modelFormat === 'glb' ? 'glb' : undefined,
        }
      }
      ...
      if (fromNode.type === 'meshy') {
        ...
        return {
          ...
          modelSourceName: String(fromNode.alias ?? fromNode.title ?? objectName ?? objectId).trim() || undefined,
