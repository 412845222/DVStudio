export const useAIWorkflowSceneLayoutMetadata = (options: {
  ...
  const mergeSceneLayoutItemsWithMetadata = (layoutItems: any[], metadataSources: any[][]) => {
    const maps = metadataSources.map((items) => buildSceneLayoutMetadataMap(Array.isArray(items) ? items : []))
    return (Array.isArray(layoutItems) ? layoutItems : []).map((item) => {
      ...
      return {
        ...mergedSource,
        ...item,
        position: item?.position && typeof item.position === 'object' ? item.position : mergedSource.position,
        size: item?.size && typeof item.size === 'object' ? item.size : mergedSource.size,
        rotation: item?.rotation && typeof item.rotation === 'object' ? item.rotation : mergedSource.rotation,
  ...
  const getSceneLayoutSelectedPlaceholderPayload = (nodeId: string) => {
    ...
    return {
      ...
      sourceImageIndex: Number.isFinite(Number(item?.sourceImageIndex)) ? Number(item.sourceImageIndex) : undefined,
      observedImageIndices: Array.isArray(item?.observedImageIndices) ? item.observedImageIndices : undefined,
    }
  }
  ...
  const serializeSceneLayoutOutput = (nodeId: string): string => {
    ...
    const camera = settings?.camera && typeof settings.camera === 'object' ? settings.camera : undefined
