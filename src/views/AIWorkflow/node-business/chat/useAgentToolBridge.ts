...
export const useAgentToolBridge = (payload: AgentToolBridgePayload) => {
			...
			blueprint: {
				...
				nodes: includeNodes
					? nodes.slice(0, 100).map((n) => ({
							id: n.id,
							type: n.type,
							label: (n as { title?: string }).title || (n as { name?: string }).name || n.type,
							x: n.worldX,
							y: n.worldY
						}))
					: undefined,
	...
	const createNode = (args: Record<string, unknown>) => {
		...
		const lastSessionNodeId =
			sessionCreatedNodeIds.length > 0
		...
		if (lastSessionNode) {
			const anchorX =
				(lastSessionNode.worldX || 0) +
				((lastSessionNode.width || NODE_DEFAULT_WIDTH) + NODE_SPACING)
		...
		if (!usedEngineSuccess) {
			...
			if (validNodeTypes.includes(nodeType as (typeof validNodeTypes)[number])) {
				payload.store.commit('setNodeType', {
					nodeId: finalNodeId,
					type: nodeType as (typeof validNodeTypes)[number]
				})
			}
	...
	const updateNodeConfig = (args: Record<string, unknown>) => {
		...
		if (config && typeof config === 'object') {
			...
			payload.pushToast(
				t('aiworkflow.toast.agentNodeUpdated', { id: nodeId, keys: configKeys }),
				'info'
			)
			return {
				ok: true,
	...
	const connectNodes = (args: Record<string, unknown>) => {
		...
		if (payload.engineApi?.connectPorts) {
			connected = payload.engineApi.connectPorts(
				fromNodeId,
				resolvedFromPort,
				toNodeId,
				resolvedToPort
	...
	const disconnectNodes = (args: Record<string, unknown>) => {
		...
		if (nodeId) {
			...
			payload.pushToast(
				t('aiworkflow.toast.agentNodeDisconnected', { id: nodeId, count: edgesToRemove.length }),
				'info'
			)
			return {
				ok: true,
				...
				note: 'Node connections disconnected successfully'
			}
		}
	...
	const getNodeInfo = (args: Record<string, unknown>) => {
		...
		return {
			...
			node: {
				...
				outputs:
					node.outputs?.map((a) => ({ id: a.id, label: a.label, mediaType: a.mediaType })) || [],
	...
	const autoLayout = (args: Record<string, unknown>) => {
		...
		if (Array.isArray(args.nodeIds) && args.nodeIds.length > 0) {
			targetNodeIds = args.nodeIds
				.map((id) => String(id))
				.filter((id) => !!payload.store.state.nodesById[id])
		} else {
		...
		payload.pushToast(
			t('aiworkflow.toast.agentAutoLayout', { count: nodesToLayout.length }),
			'info'
		)
		return {
			ok: true,
