...
export const useAIWorkflowVideoTaskPanelController = (options: {
  comfyService: {
    ...
    seedanceSyncTasks: (payload?: { taskId?: string; pageNum?: number; pageSize?: number; projectId?: number; saveMedia?: boolean }) => Promise<any>
  }
  ...
  const patchTaskItem = (nextItem: VideoTaskPanelItem | null | undefined) => {
    ...
    if (!taskId) return
    videoTaskItems.value = videoTaskItems.value.map((item) => (item.taskId === taskId ? { ...item, ...nextItem } : item))
  }

  const ensureLocalMediaForTask = async (item: VideoTaskPanelItem | null | undefined, opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (!res.ok || !res.item) {
        if (!opts?.silent) options.pushToast('视频任务本地补存失败：' + String(res.error || 'unknown'), 'warn')
        return item
      }
      ...
      if (!opts?.silent) options.pushToast('视频任务本地补存失败：' + String(err?.message ?? err ?? 'unknown'), 'warn')
      return item
    }
  }
  ...
  const refreshVideoTaskItems = async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (!opts?.silent) options.pushToast('读取视频任务列表失败：' + String(res.error || 'unknown'), 'warn')
        return
      }
      videoTaskItems.value = Array.isArray(res.items) ? res.items.map((item: SeedanceTaskMirrorItem) => mapVideoTaskItem(item)) : []
      ...
      videoTaskFallbackReason.value = String(err?.message ?? err ?? 'unknown')
      if (!opts?.silent) options.pushToast('读取视频任务列表失败：' + String(err?.message ?? err ?? 'unknown'), 'warn')
    } finally {
  ...
  const selectVideoTask = async (taskId: string, opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (!opts?.silent) options.pushToast('读取视频任务详情失败：' + String(res.error || 'unknown'), 'warn')
        return
      }
      ...
      if (!opts?.silent) options.pushToast('读取视频任务详情失败：' + String(err?.message ?? err ?? 'unknown'), 'warn')
    } finally {
  ...
  const syncRemoteVideoTasks = async () => {
    ...
    try {
      ...
      options.pushToast('远端补齐视频任务失败：' + String(err?.message ?? err ?? 'unknown'), 'warn')
    } finally {
