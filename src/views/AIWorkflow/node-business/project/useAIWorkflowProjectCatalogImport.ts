...
export const useAIWorkflowProjectCatalogImport = (payload: {
	...
	const refreshProjectList = async () => {
		...
		if (!res.ok) {
			payload.pushToast(
				t('aiworkflow.runtime.projectListLoadFailed', { error: String(res.error || 'unknown') }),
				'warn'
			)
			return
		}
		...
		try {
			...
			if (
				!payload.hydrateBlueprintSnapshotSafely(
					runtimeSafeSnapshot,
					t('aiworkflow.runtime.importLocalBlueprintSource')
				)
			)
				return
			...
			payload.pushToast(
				t('aiworkflow.runtime.importFailedWithError', { error: getErrorMessage(err) }),
				'error'
			)
		}
	}
