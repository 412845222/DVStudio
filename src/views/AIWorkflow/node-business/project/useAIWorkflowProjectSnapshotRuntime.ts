...
import {
  ...
  isFileProtocolUrl,
  buildProjectAssetRuntimeUrl as buildDwebUrl,
...
export const useAIWorkflowProjectSnapshotRuntime = (payload: {
  ...
  const sanitizeBlueprintSnapshotForRuntime = (snapshot: any): AIWorkflowDraftSnapshot => {
          ...
          outputs: outputs
            ...
            .map((item: any, index: number) => {
              ...
              return {
                ...
                name: String(item.name ?? item.objectName ?? `obj_${index + 1}`).trim() || `obj_${index + 1}`,
    ...
    for (const [resourceId, resource] of Object.entries(resourcesById)) {
      ...
      const kind = String((resource as any).kind ?? '').toLowerCase() === 'video' ? 'video' : 'image'
      const originalName = String((resource as any).name ?? '').trim()
      ...
      if (!resolvedRelPath && originalUrl && originalUrl.toLowerCase().startsWith('dweb://project-assets')) {
        try {
      ...
      const originalPosterUrl = posterUrlIsFileProtocol ? '' : sanitizeWorkflowMediaUrl(rawPosterUrl)
      
      let resolvedPosterRelPath = posterProjectRelativePath
      if (!resolvedPosterRelPath && originalPosterUrl && originalPosterUrl.toLowerCase().startsWith('dweb://project-assets')) {
        try {
      ...
      if (resolvedPosterRelPath) {
        runtimePosterUrl = buildProjectAssetRuntimeUrl(runtimeProjectId, resolvedPosterRelPath, originalPosterUrl)
