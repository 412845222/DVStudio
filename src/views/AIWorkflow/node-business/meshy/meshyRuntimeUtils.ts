export const normalizeMeshyTaskStatus = (raw: unknown) => {
  const status = String(raw ?? '').trim().toLowerCase()
  if (status === 'pending' || status === 'queued') return 'pending' as const
  if (status === 'running' || status === 'processing' || status === 'in_progress') return 'running' as const
  if (status === 'succeeded' || status === 'success' || status === 'completed') return 'succeeded' as const
