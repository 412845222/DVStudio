...
import { buildSeedanceContent, type SeedanceGenerationRequest, type SeedanceTaskDetail } from './seedanceApi'
import { SEEDANCE_MAX_REF_IMAGES, SEEDANCE_MAX_REF_VIDEOS, SEEDANCE_MAX_REF_AUDIOS, isSeedance20Model } from './seedanceConstants'
...
export const useSeedanceVideoGeneration = () => {
	...
	const hasReferences = computed(() => {
		return references.value.images.length > 0 || 
	...
	const addReferenceImage = (url: string) => {
		if (references.value.images.length < SEEDANCE_MAX_REF_IMAGES && !references.value.images.includes(url)) {
	...
	const addReferenceVideo = (url: string) => {
		if (references.value.videos.length < SEEDANCE_MAX_REF_VIDEOS && !references.value.videos.includes(url)) {
	...
	const addReferenceAudio = (url: string) => {
		if (references.value.audios.length < SEEDANCE_MAX_REF_AUDIOS && !references.value.audios.includes(url)) {
