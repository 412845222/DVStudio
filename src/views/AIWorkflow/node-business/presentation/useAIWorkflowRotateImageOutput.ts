export const useAIWorkflowRotateImageOutput = (payload: {
  ...
  setNodeResourceWithCleanup: (input: { nodeId: string; resourceId: string | null; resourcePath?: string }) => void
  ...
  const onRotateImageOutput = (
    nodeId: string,
    input: { dataUrl: string; promptText: string; yaw: number; pitch: number; width: number; height: number },
  ) => {
    ...
    for (const edge of payload.getEdges()) {
      ...
      const cloned = new File([payload.dataUrlToBlob(dataUrl)], `rotate_${nodeId}_${toNode.id}.png`, {
        type: 'image/png',
      })
