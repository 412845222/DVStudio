...
export const useAIWorkflowTextOutputResolver = (payload: {
  ...
  const resolveNodeName = (node: any) => String(node?.alias ?? node?.title ?? '').trim() || undefined
  ...
  const sceneDecomposeTextOutputForAnchor = (node: any, anchorId: string) => {
    ...
    const outputs: WorkflowSceneDecomposeOutput[] = Array.isArray(rawOutputs)
      ? rawOutputs
      : []
    const item = outputs.find((entry) => String(entry?.textAnchorId ?? '') === String(anchorId ?? ''))
    if (item) return String(item.description ?? '').trim()
    return String(outputs[0]?.description ?? '').trim()
  }

  function getTextOutputForNode(nodeId: string, visited?: Set<string>, fromAnchorId?: string): string {
    ...
    if (node.type === 'text') {
      const inputAnchor = Array.isArray(node.inputs)
        ? node.inputs.find((anchor: any) => String(anchor?.mediaType ?? '') === 'text' || String(anchor?.id ?? '') === 'in-text')
        : null
      if (inputAnchor?.id) {
    ...
    if (node.type === 'scene-understanding') return String(node.sceneUnderstandingSettings?.outputJson ?? '')
    if (node.type === 'scene-decompose') return sceneDecomposeTextOutputForAnchor(node, String(fromAnchorId ?? ''))
  ...
  const getInputParamPreviewRefs = (nodeId: string): InputParamPreviewRef[] => {
    ...
    for (const edge of incomingEdges) {
      ...
      if (fromNode.type === 'image') {
        const previewUrl = sanitizeWorkflowMediaUrl(payload.nodeImagePreviewUrl(fromNode, 160))
  ...
  const getInputTextConnectedRefs = (nodeId: string): InputTextConnectedRef[] => {
    return getInputParamPreviewRefs(nodeId).filter((item): item is InputTextConnectedRef => item.kind === 'text' && !!item.text)
  }
