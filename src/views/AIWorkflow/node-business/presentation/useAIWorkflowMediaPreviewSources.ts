...
export const useAIWorkflowMediaPreviewSources = (payload: {
  ...
  comfyOutputForAnchor: (outputs: ComfyLocalizedOutput[], anchorId: string, expectedKind: 'image' | 'video') => any
}) => {
  const storyPreview = (node: WorkflowNode) => {
    const resourceInput = node.inputs?.find((a) => a.id === 'in-resource' || a.id === 'in-image' || a.id === 'in-video')
  ...
  const rotateImagePreviewUrl = (node: WorkflowNode) => {
    ...
    if (fromNode.type === 'comfyui') {
      const outputs = Array.isArray(fromNode.comfyuiSettings?.outputs)
        ...
        : []
      const media = payload.comfyOutputForAnchor(outputs, String((edge as any).fromAnchorId ?? ''), 'image')
  ...
  const connectedImageInputSource = (nodeId: string, inputId: string) => {
    ...
    if (fromNode.type === 'image') {
      ...
      return {
        url,
        width: Number.isFinite(Number(fromNode.imageSettings?.naturalWidth)) ? Number(fromNode.imageSettings?.naturalWidth) : undefined,
        height: Number.isFinite(Number(fromNode.imageSettings?.naturalHeight)) ? Number(fromNode.imageSettings?.naturalHeight) : undefined,
      }
    }
    ...
    if (fromNode.type === 'video') {
      ...
      return {
        url,
        width: Number.isFinite(Number(fromNode.videoSettings?.naturalWidth)) ? Number(fromNode.videoSettings?.naturalWidth) : undefined,
        height: Number.isFinite(Number(fromNode.videoSettings?.naturalHeight)) ? Number(fromNode.videoSettings?.naturalHeight) : undefined,
      }
    }

    if (fromNode.type === 'comfyui') {
      const url = String(payload.connectedImageOutputUrl(fromNode, String((edge as any).fromAnchorId ?? '')) ?? '').trim()
      if (!url) return null
      return { url }
    }
    ...
    if (fromNode.type === 'scene-decompose') {
      ...
      const outputs: WorkflowSceneDecomposeOutput[] = Array.isArray(rawOutputs)
        ? rawOutputs
        : []
      const item = outputs.find((entry) => String(entry?.imageAnchorId ?? '') === String((edge as any).fromAnchorId ?? ''))
      const url = String(payload.connectedImageOutputUrl(fromNode, String((edge as any).fromAnchorId ?? '')) ?? '').trim()
      if (!url) return null
      return {
        url,
