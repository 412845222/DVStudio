...
export const useAIWorkflowSceneLayoutModelBinding = (payload: {
  ...
  const onNodeUploadSceneLayoutModelFile = async (nodeId: string, file: File, preferredObjectId?: string) => {
    const node = payload.store.state.nodesById[nodeId]
    if (!node || node.type !== 'scene-layout') return

    const selectedLayoutItemId = String(preferredObjectId ?? node.sceneLayoutSettings?.selectedLayoutItemId ?? '').trim()
    ...
    const layoutItems = Array.isArray(node.sceneLayoutSettings?.layoutItems) ? node.sceneLayoutSettings.layoutItems : []
    const selectedLayoutItem = layoutItems.find((item: any) => String(item?.id ?? '').trim() === selectedLayoutItemId)
    ...
    try {
      ...
      if (projectId > 0) {
        const uploaded = await payload.blueprintProjectService.uploadAsset(file, 'file', { projectId })
        if (uploaded.ok) {
    ...
    const displayName = String(selectedLayoutItem.name ?? selectedLayoutItem.id ?? selectedLayoutItemId).trim() || selectedLayoutItemId
  ...
  const onNodeClearSceneLayoutModelBinding = (nodeId: string, objectIdRaw: string) => {
    ...
    const targetBinding = currentBindings.find((item) => String(item?.objectId ?? '').trim() === objectId)
    if (!targetBinding) return

    const nextBindings = currentBindings.filter((item) => String(item?.objectId ?? '').trim() !== objectId)
    const nextLayoutItems = (Array.isArray(node.sceneLayoutSettings?.layoutItems)
    ...
    const layoutItems = Array.isArray(node.sceneLayoutSettings?.layoutItems)
      ? node.sceneLayoutSettings.layoutItems
      : []
    const selectedLayoutItem = layoutItems.find((item: any) => String(item?.id ?? '').trim() === objectId)
    const displayName = String(selectedLayoutItem?.name ?? selectedLayoutItem?.id ?? objectId).trim() || objectId
