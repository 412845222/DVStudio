...
export const useAIWorkflowImportRecoveryState = (payload: {
  ...
  const updateImportProgressIfNeeded = (sessionId: string, resourceId: string) => {
    ...
    state.done = true
    session.processed += 1
    importOverlayProgress.value = session.total > 0 ? Math.max(0, Math.min(1, session.processed / session.total)) : 0
  ...
  const startImportSession = (session: Omit<ActiveImportSession, 'cancelled' | 'processed'> & { cancelled?: boolean; processed?: number; title?: string }) => {
  ...
  const cancelActiveImportSession = (payload?: { onBeforeClear?: (session: ActiveImportSession) => void }) => {
    const session = activeImportSession.value
    importOverlayOpen.value = false
    importOverlayProgress.value = 0
    importOverlayDetail.value = ''
  ...
  const updateRecoveryProgressIfNeeded = (sessionId: string, nodeId: string) => {
    ...
    state.done = true
    session.processed += 1
    recoveryOverlayProgress.value = session.total > 0 ? Math.max(0, Math.min(1, session.processed / session.total)) : 0
  ...
  const refreshRecoveryUrlReady = (sessionId: string, getResourceUrl: (resourceId: string) => string) => {
    const session = activeRecoverySession.value
    if (!session || session.id !== sessionId) return
  ...
  const finalizeRecoverySessionAfterUrlRecoveryAttempt = (sessionId: string, getResourceUrl: (resourceId: string) => string) => {
    const session = activeRecoverySession.value
    if (!session || session.id !== sessionId) return
    ...
    if (session.processed >= session.total) {
      ...
      activeRecoverySession.value = null
      if (missingUrl > 0) {
        payload.pushToast(`有 ${missingUrl} 个本地资源无法自动恢复（缺失 URL）。可在“加载项目”时选择文件夹重新绑定/授权。`, 'warn')
      }
    }
  }
