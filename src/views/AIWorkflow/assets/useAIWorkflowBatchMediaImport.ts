...
export const useAIWorkflowBatchMediaImport = (options: {
  ...
  startImportSession: (payload: {
    ...
    resourceState: Map<string, { kind: 'image' | 'video'; urlReady: boolean; nodeReady: boolean; done: boolean }>
    total: number
  }) => void
  ...
  const createMediaNodesFromFiles = async (opts: {
    ...
    for (let i = 0; i < media.length; i += 1) {
      ...
      const name = String(item.relativePath || item.file.name || (item.kind === 'image' ? 'image' : 'video'))
      const absPath = typeof (item.file as any)?.path === 'string' ? String((item.file as any).path).trim() : ''
      ...
      const sourceFingerprint = options.normalizeFileSignatureKey(sourceName, sourceSize, sourceLastModified)
    ...
    options.mediaImportManager.enqueue(
      imageTasks.map((task) => ({
        ...
        onResult: (result: any) => {
          ...
          const sourcePath = typeof result.sourcePath === 'string' ? String(result.sourcePath).trim() : ''
          if (sourcePath) {
