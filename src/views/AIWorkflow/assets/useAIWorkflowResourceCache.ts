...
export const useAIWorkflowResourceCache = () => {
	...
	const preCheckAndRepairResource = async (
		url: string
	): Promise<{ ok: boolean; url: string; repaired?: boolean }> => {
		const trimmed = String(url).trim()
		...
		return new Promise((resolve) => {
			...
			const handleError = async () => {
				...
				logLoad({
					...
					retryAttempts: resource.retryAttempts
				})
	...
	const validateAndPreloadResources = async (
		urls: string[],
		type: 'image' | 'video'
	...
	const getFailedResources = (): Array<{
		url: string
		errorCount: number
		lastErrorTime?: number
		retryAttempts: number
