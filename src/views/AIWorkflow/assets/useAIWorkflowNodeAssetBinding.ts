...
export const useAIWorkflowNodeAssetBinding = (options: {
  ...
  const uploadNodeResource = (
    ...
    file: File,
    kind: 'image' | 'video',
    opts?: { autoDistribute?: boolean; onAfterBind?: (payload: { resourceId: string; url: string }) => void },
  ) => {
    ...
    options.setObjectUrl(resourceId, url)
    ...
    if (kind === 'image') {
      ...
      img.onerror = () => {
        if (!nodeStillExists(nodeId)) return
        const fallbackWidth = Math.max(1, Math.floor(Number(node.imageSettings?.outputWidth || node.imageSettings?.naturalWidth || 1) || 1))
        const fallbackHeight = Math.max(1, Math.floor(Number(node.imageSettings?.outputHeight || node.imageSettings?.naturalHeight || 1) || 1))
        options.store.commit('setNodeImageSettings', {
          nodeId,
          imageSettings: {
      ...
      img.src = url
      void persistNodeImageResourceLocally({ resourceId, nodeId, file })
    }
  ...
  const bindMediaResourceToNode = (
    ...
    url: string,
    name: string,
    opts?: { posterUrl?: string; sourcePath?: string; projectRelativePath?: string; posterProjectRelativePath?: string },
  ) => {
    const node = options.store.state.nodesById[nodeId]
    if (!node) return
    const resourceId = options.makeResourceId()
    options.store.commit('addResource', {
      ...
      ...(opts?.projectRelativePath ? { projectRelativePath: String(opts.projectRelativePath) } : {}),
      ...(opts?.posterProjectRelativePath ? { posterProjectRelativePath: String(opts.posterProjectRelativePath) } : {}),
      createdAt: Date.now(),
    })
    options.setNodeResourceWithCleanup({
      nodeId,
