...
export type Fallback404Result =
  ...
  | { kind: 'missing'; url: string; assetName: string; missingPath: string; sources: ResourceBindingSource[] }
 ...
 * 纯前端实现，不依赖网络。
 */
export function parseDwebProjectAssetUrl(url: string): { projectId: number | null; relPath: string; variant?: string; maxSize?: number; version?: string } | null {
  const trimmed = String(url || '').trim()
  if (!trimmed) return null
  if (!trimmed.startsWith('dweb://project-assets') && !trimmed.startsWith('dweb:project-assets')) return null
  try {
...
export function useAIWorkflow404Fallback(options: AIWorkflow404FallbackOptions) {
  ...
  const notify = (message: string, tone: 'info' | 'warn' | 'error' = 'info') => {
    if (typeof pushToast === 'function') {
      try { pushToast(message, tone) } catch { /* ignore */ }
    } else {
   ...
   */
  async function handle404Error(url: string, source?: ResourceBindingSource): Promise<Fallback404Result> {
    const trimmed = String(url || '').trim()
    ...
    const promise: Promise<Fallback404Result> = (async () => {
      try {
        ...
        if (pid != null && isElectron()) {
          ...
          try {
            await validateDwebProjectRoot({ projectId: pid, expectedRootPath: expectedRoot || undefined })
          } catch { /* ignore */ }
        }
        ...
        const assetName = diag?.repairedAsset?.name
          || (parseDwebProjectAssetUrl(trimmed)?.relPath?.split('/').pop() || trimmed.split('/').pop() || trimmed)
        ...
        if (diag?.fileExists && diag?.repairedAsset) {
          ...
          console.info('[404-fallback] recovered via re-resolution:', { from: trimmed, to: newUrl, name: newAsset.name })
          notify(`资源已自动恢复：${newAsset.name}`, 'info')
          if (typeof onRecovered === 'function') {
            try { onRecovered({ url: trimmed, newUrl, assetName: newAsset.name, newAsset }) } catch (err) { console.warn('[404-fallback] onRecovered hook error:', err) }
          }
          recovering.value = false
          return { kind: 'recovered' as const, url: trimmed, assetName: newAsset.name, repairedUrl: newUrl, newAsset }
        }
        ...
        if (diag && (!diag.registered || diag.suggestion === 're_register_root')) {
          ...
          if (pid2 && isElectron()) {
            const expectedRoot = getCurrentProjectRootPath?.() ?? null
            try {
              const vr = await validateDwebProjectRoot({ projectId: pid2, expectedRootPath: expectedRoot || undefined })
              if (vr?.validation?.valid) {
                ...
                if (diag2?.fileExists && diag2?.repairedAsset) {
                  ...
                  if (typeof onRecovered === 'function') {
                    try {
                      onRecovered({ url: trimmed, newUrl: diag2.repairedAsset.url, assetName: diag2.repairedAsset.name, newAsset: diag2.repairedAsset })
                    } catch (err) { console.warn('[404-fallback] onRecovered hook error:', err) }
                  }
                  recovering.value = false
                  return { kind: 'recovered' as const, url: trimmed, assetName: diag2.repairedAsset.name, repairedUrl: diag2.repairedAsset.url, newAsset: diag2.repairedAsset }
                }
              }
            } catch (err) {
        ...
        const sources = findBindingSources?.(trimmed) || defaultFindBindingSources(trimmed, getStore?.())
        ...
        if (!existing) {
          ...
          if (typeof onMissingAsset === 'function') {
            try { onMissingAsset(pending) } catch (err) { console.warn('[404-fallback] onMissingAsset hook error:', err) }
          }
        } else {
          // 合并 sources
          const existingSourceKeys = new Set(existing.sources.map(s => `${s.type}:${s.resourceId || ''}:${s.nodeId || ''}:${s.field || ''}`))
        ...
        recovering.value = false
        return { kind: 'ignored' as const, url: trimmed, reason: `exception: ${String((err as any)?.message || err)}` }
      } finally {
  ...
  function confirmRemoveMissingAsset(pendingId: string): { ok: boolean; undoAvailable: boolean } {
    ...
    return { ok: true, undoAvailable: Object.keys(undoSnapshot.resourcesByIdPatch || {}).length > 0 || (undoSnapshot.nodePatches?.length || 0) > 0 }
  }
  ...
  function restoreFromSnapshot(pending: PendingMissingAsset): boolean {
    ...
    if (snap.resourcesByIdPatch) {
      for (const [_rid, backup] of Object.entries(snap.resourcesByIdPatch)) {
        if (backup) {
          try { store.commit('addResource', backup) } catch (err) { console.warn('[404-fallback] undo addResource failed:', err) }
        }
      }
    }
  ...
  function installGlobalErrorHandlers(): () => void {
    ...
    const rejectionHandler = (event: PromiseRejectionEvent) => {
      ...
      if (!reason) return
      const url = String(reason?.url || reason?.resourceUrl || (typeof reason === 'object' ? reason?.message : '') || '')
...
function defaultFindBindingSources(url: string, store: any): ResourceBindingSource[] {
  ...
  for (const [rid, res] of Object.entries(resourcesById) as Array<[string, any]>) {
    if (!res) continue
    if (res.url === url) {
      sources.push({ type: 'resource', resourceId: rid, detail: `resource.url => ${res.name || rid}` })
    } else if (res.previewUrl === url) {
      sources.push({ type: 'preview', resourceId: rid, detail: `resource.previewUrl => ${res.name || rid}` })
    } else if (res.posterUrl === url) {
      sources.push({ type: 'poster', resourceId: rid, detail: `resource.posterUrl => ${res.name || rid}` })
    }
  }
  ...
  for (const [nid, node] of Object.entries(nodesById) as Array<[string, any]>) {
    ...
    if (node?.inputs && typeof node.inputs === 'object') {
      for (const [key, val] of Object.entries(node.inputs)) {
        if (val === url) {
          sources.push({ type: 'node_input', nodeId: nid, nodeType, field: `inputs.${key}`, detail: `node.inputs.${key}` })
        }
      }
    }
    if (node?.outputs && typeof node.outputs === 'object') {
      for (const [key, val] of Object.entries(node.outputs)) {
        if (val === url) {
          sources.push({ type: 'node_output', nodeId: nid, nodeType, field: `outputs.${key}`, detail: `node.outputs.${key}` })
        }
      }
    }
  }
