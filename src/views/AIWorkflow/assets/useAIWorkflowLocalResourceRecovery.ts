...
export const useAIWorkflowLocalResourceRecovery = (payload: {
  ...
  collectDroppedFilesFromHandle: (handle: any, pathPrefix?: string) => Promise<Array<{ file?: File; fsHandle?: any }>>
  putLocalFileHandle: (key: string, handle: any) => Promise<boolean>
}) => {
  const recoverLocalResourcesFromHandles = async (opts?: { silent?: boolean }): Promise<RecoveryStats> => {
        ...
        try {
          ...
          if (dir) {
            ...
            if (stats.rebound > 0) {
              ...
              payload.pushToast(`已重新绑定 ${stats.rebound} 个文件句柄，正在恢复资源加载。`, 'info')
              return stats
            }
          }
        } catch {
