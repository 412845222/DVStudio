export const useAIWorkflowNodePreviewContextMenu = (payload: {
  ...
  onCanvasContextMenu: (menuPayload: { clientX: number; clientY: number; worldX: number; worldY: number }) => void
}) => {
  const onNodePreviewContextMenu = (nodeId: string, input: { clientX: number; clientY: number }) => {
