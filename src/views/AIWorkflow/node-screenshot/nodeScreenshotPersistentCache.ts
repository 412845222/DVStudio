...
const makeKey = (projectId: string, blueprintId: string, nodeId: string, version: string): string => {
...
export const loadAllScreenshotsForBlueprint = async (
  ...
  const result = new Map<string, { dataUrl: string; version: string; width: number; height: number }>()
  try {
    const db = await openDb()
    const tx = db.transaction(STORE_NAME, 'readonly')
    const store = tx.objectStore(STORE_NAME)
    ...
    await new Promise<void>((resolve, reject) => {
      req.onsuccess = () => {
        ...
        if (cursor) {
          ...
          result.set(entry.nodeId, { dataUrl: entry.dataUrl, version: entry.version, width: entry.width, height: entry.height })
          cursor.continue()
        } else {
          resolve()
        }
...
export const cleanupOldScreenshots = async (maxAgeMs: number = 7 * 24 * 60 * 60 * 1000): Promise<void> => {
  try {
