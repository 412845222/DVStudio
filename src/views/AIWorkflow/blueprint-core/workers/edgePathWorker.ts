...
self.onmessage = (ev: MessageEvent<WorkerRequest>) => {
	...
	for (const node of msg.nodes || []) {
		...
		outputIndexByNodeId.set(
			node.id,
			buildAnchorIndex(Array.isArray(node.outputs) ? node.outputs : [])
		)
	}
	...
	for (const edge of msg.edges || []) {
		...
		const fromIndex =
			outputIndexByNodeId.get(fromNode.id)?.get(String(edge.fromAnchorId ?? '')) ?? 0
		...
		if (!keepEdgeIdSet.has(edgeId)) {
			const shouldCull = shouldCullEdgeByViewport(
				start,
				end,
				msg.canvasViewportSize,
