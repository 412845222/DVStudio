...
export const useAIWorkflowPerfMonitor = (payload: {
	...
	const perfFrameText = computed(() =>
		perfFrameMs.value > 0 ? `${perfFrameMs.value.toFixed(1)} ms` : '--'
	)
	const perfAvgFrameText = computed(() =>
		perfAvgFrameMs.value > 0 ? `${perfAvgFrameMs.value.toFixed(1)} ms` : '--'
	)
	const perfWorstFrameText = computed(() =>
