...
export const useAIWorkflowSelectionState = (store: Store<WorkflowState>) => {
  ...
  const selectedNode = computed<WorkflowNode | null>(() => (selectedNodeId.value ? store.state.nodesById[selectedNodeId.value] : null))
  const selectedEdge = computed<WorkflowEdge | null>(() => (selectedEdgeId.value ? store.state.edgesById[selectedEdgeId.value] : null))
