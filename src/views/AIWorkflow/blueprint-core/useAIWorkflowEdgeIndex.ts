...
export const useAIWorkflowEdgeIndex = (payload: {
	...
	const edges = computed(() =>
		payload.store.state.edgeOrder.map((id) => payload.store.state.edgesById[id]).filter(Boolean)
	)
	...
	const getIncomingEdges = (nodeId: string, anchorId?: string) => {
		...
		if (anchorId) {
			return (
				edgeIndex.value.incomingByNodeAnchor.get(
					edgeNodeAnchorKey(normalizedNodeId, String(anchorId))
	...
	const getOutgoingEdges = (nodeId: string, anchorId?: string) => {
		...
		if (anchorId) {
			return (
				edgeIndex.value.outgoingByNodeAnchor.get(
					edgeNodeAnchorKey(normalizedNodeId, String(anchorId))
				) ?? EMPTY_WORKFLOW_EDGES
			)
		}
	...
	const getFirstIncomingEdge = (nodeId: string, anchorId?: string) =>
		getIncomingEdges(nodeId, anchorId)[0] ?? null
	const hasIncomingEdge = (nodeId: string, anchorId?: string) =>
		getIncomingEdges(nodeId, anchorId).length > 0
	const hasOutgoingEdge = (nodeId: string, anchorId?: string) =>
		getOutgoingEdges(nodeId, anchorId).length > 0

	const hasExactEdge = (edgePayload: {
		fromNodeId: string
		fromAnchorId: string
		toNodeId: string
		toAnchorId: string
		...
		return edgeIndex.value.exactEdges.has(
			`${fromNodeId}::${fromAnchorId}=>${toNodeId}::${toAnchorId}`
		)
	}
	...
	const renderEdges = computed(() => {
		...
		const baseEdges = showNanoRef
			...
			: edges.value.filter(
					(edge) =>
						edge.toNodeId !== payload.nanoAnchorNodeId &&
						edge.fromNodeId !== payload.nanoAnchorNodeId
				)

		return baseEdges.filter((edge) => {
			if (edge.id === payload.store.state.selectedEdgeId) return true
			return (
				payload.visibleRenderNodeIds.value.has(edge.fromNodeId) ||
				payload.visibleRenderNodeIds.value.has(edge.toNodeId)
			)
		})
	})
