...
export const useAIWorkflowThreejsLifecycleManager = (payload: {
  ...
  const updatePreviewProgress = (nodeId: string, payloadValue?: WorkflowThreePreviewProgressPayload) => {
    const targetId = String(nodeId ?? '').trim()
    const current = stateMap.value[targetId]
  ...
  const completePreviewSession = (nodeId: string) => {
    ...
    if (current.phase === 'interactive' && current.progress === 1 && current.label === '渲染已就绪') return
