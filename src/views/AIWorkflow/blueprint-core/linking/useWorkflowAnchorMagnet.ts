...
const phaseForDistance = (distance: number, lockRadius: number, activateRadius: number): WorkflowAnchorMagnetPhase => {
...
export const useWorkflowAnchorMagnet = () => {
  ...
  const phaseForAnchor = (nodeId: string, anchorId: string, active: WorkflowAnchorMagnetTarget | null): WorkflowAnchorMagnetPhase => {
    if (!active) {
