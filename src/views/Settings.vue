...
import {
	cliCheckEnvironment,
	cliGetAdapterConfig,
	cliSaveAdapterConfig,
	cliRunFixCommand,
	cliStartAuthFlow,
	cliCancelAuth,
	cliResetAdapterConfig,
	type AuthStreamChunk
} from '../network/CLIChatService'
import {
	setCopilotEnabled,
...
import {
	...
	AGENT_CONFIG_CONSTRAINTS,
	validateMaxToolCalls
} from '../core/agent/agentConfig'
...
async function enableCopilot() {
	...
	try {
		...
		if (saveRes?.ok) {
			...
			showSaveMessage(
				t('settings.saveFailed', { msg: saveRes?.error || t('common.error') }),
				'error'
			)
		}
	} catch (e: unknown) {
...
async function resetCopilotConfig() {
	...
	try {
		...
		if (resetRes?.ok) {
			...
			showSaveMessage(
				t('settings.saveFailed', { msg: resetRes?.error || t('common.error') }),
				'error'
			)
		}
	} catch (e: unknown) {
...
async function runCopilotFix(checkKey: string) {
	...
	try {
		...
		const resultData = (result as any)?.value || (result as any)?.data
		if (result?.ok && resultData) {
			copilotFixMessage.value = {
				key: checkKey,
				ok: resultData.ok,
				msg: resultData.message || (resultData.ok ? '修复成功' : '修复失败')
			}
...
function getCheckStatusLabel(status: CheckStatus) {
	switch (status) {
		case 'pass':
			return t('copilot.checkPass')
		case 'fail':
			return t('copilot.checkFail')
		case 'warn':
			return t('copilot.checkWarn')
		case 'skipped':
			return t('copilot.checkSkipped')
		case 'pending':
			return t('copilot.checkPending')
		default:
			return ''
	}
}
...
async function confirmClearCredentials() {
	...
	if (!r.ok)
		showSaveMessage(t('settings.clearFailed', { msg: r.error || t('common.error') }), 'error')
	else {
		form.geminiApiKey = ''
...
const platformStatusText = computed(() => {
	...
	if (platformIsSteam.value) {
		return s.user?.displayName
			? t('settings.platform.steamUser', { name: s.user.displayName })
			: 'Steam'
	}
...
const platformHintText = computed(() => {
	...
	if (platformIsSteam.value && s.loggedIn) {
		return t('settings.platform.steamLoggedIn', {
			name: s.user?.displayName || t('settings.platform.steamUserUnknown')
		})
	}
	return ''
})
	...
	<div class="settings-root">
		<div class="settings-page" :class="{ loading: loading }">
			<header class="settings-header">
				...
				<p
					v-if="saveMsg"
					:class="['settings-flash', saveMsgType === 'error' ? 'settings-flash-error' : '']"
				>
					{{ saveMsg }}
				</p>
			</header>
			...
			<section class="settings-section">
				...
				<div class="resolution-card">
					<div class="resolution-row">
						...
						<select
							v-model="form.defaultResolution"
				...
				<div class="platform-card">
					<div class="platform-header">
						<div class="platform-info">
							...
							<div v-if="platformStatus" class="platform-meta">
								...
								<span v-if="platformStatus.initialized">
									{{ t('settings.platformInitialized') }}
								</span>
					...
					<div v-if="platformIsSteam && platformStatus?.user" class="platform-user">
						...
						<div class="platform-user-info">
							...
							<div v-if="platformStatus.user.steamId" class="platform-user-id">
								Steam ID: {{ platformStatus.user.steamId }}
							</div>
						</div>
					</div>
					...
					<div v-if="platformOverlayEnabled" class="platform-overlay">
						<div class="platform-overlay-status">
							...
							Steam Overlay
							{{
								platformOverlayActive ? t('settings.overlayActive') : t('settings.overlayAvailable')
							}}
						</div>
			...
			<section class="settings-section">
				...
				<div class="platform-card">
					...
					<div class="platform-hint">
						打开 ComfyUI 环境设置窗口，可以检测系统环境、选择已安装的 ComfyUI 路径或安装新的 ComfyUI
						实例。
					</div>
				...
				<div class="network-card">
					<div class="network-row">
						<label class="network-label">
							...
							<small style="color: var(--vscode-fg-muted); font-size: 11px">
								{{ t('settings.network.httpProxyHint') }}
							</small>
						</label>
						<input
					...
					<div class="network-row">
						<label class="network-label">
							...
							<small style="color: var(--vscode-fg-muted); font-size: 11px">
								{{ t('settings.network.geminiBaseUrlHint') }}
							</small>
						</label>
						<input
					...
					<div
						...
						"
					>
						<div style="font-weight: 600; color: #22a06b; margin-bottom: 6px">
							{{ t('settings.network.proxyGuideTitle') }}
						</div>
				...
				<div class="agent-card">
					<div class="agent-group">
						...
						<div class="agent-switch-row">
							<label class="agent-switch-label">
								{{ t('settings.agent.enableToolCallWarning') }}
							</label>
							<label class="toggle-switch">
								<input
									type="checkbox"
									v-model="form.agent!.enableToolCallWarning"
									@change="onAgentSwitchChange"
								/>
					...
					<div class="agent-group">
						...
						<div class="agent-row">
							...
							<select
								v-model="form.agent!.defaultThinkingEffort"
								class="agent-select"
						...
						<div class="agent-switch-row">
							...
							<label class="toggle-switch">
								<input
									type="checkbox"
									v-model="form.agent!.showThoughtProcess"
									@change="onAgentSwitchChange"
								/>
								<span class="toggle-slider"></span>
							</label>
						...
						<div class="agent-switch-row">
							...
							<label class="toggle-switch">
								<input
									type="checkbox"
									v-model="form.agent!.autoScrollToBottom"
									@change="onAgentSwitchChange"
								/>
								<span class="toggle-slider"></span>
							</label>
		...
		>
			<template v-if="activeProviderConfig && activeProviderConfig.type === 'apikey'">
				<div class="provider-modal-body">
					<div class="provider-modal-head" :style="{ '--accent': activeProviderConfig.accent }">
					...
					<div class="provider-modal-fields">
						<label v-for="f in activeProviderConfig.fields" :key="f.key" class="field-row">
			...
			<template v-if="activeProviderConfig">
				<div class="copilot-modal-body">
					...
					<div class="copilot-actions">
						...
						<button class="btn btn-ghost" type="button" @click="openDocsForProvider('github')">
					...
					<div
						v-if="!copilotCheckResult && !copilotChecking && !copilotCheckError"
						class="copilot-hint"
					>
					...
					<div v-if="copilotCheckResult" class="copilot-checks">
						<div class="copilot-checks-header">
							<span
								:class="[
									'copilot-overall',
									copilotCheckResult.allPassed ? 'copilot-overall-pass' : 'copilot-overall-fail'
								]"
							>
							...
							>
								...
								<div class="copilot-check-body">
									...
									<div v-if="check.action" class="copilot-check-action">
										...
										<code class="copilot-check-cmd">
											{{ check.action.command || check.action.label }}
										</code>
										<button
								...
								<div class="copilot-check-status-label">
						...
						<div v-if="copilotModels.length > 0" class="copilot-models">
							...
							<div class="copilot-model-list">
								<div
									v-for="m in copilotModels"
									:key="m.id"
									class="copilot-model-tag"
									:class="{ recommended: m.recommended }"
								>
									<span class="copilot-model-name">{{ m.label }}</span>
									<span v-if="m.recommended" class="copilot-model-badge">
			...
			<template v-if="activeProviderConfig">
				<div class="copilot-modal-body">
					...
					<div class="copilot-actions">
						...
						<button class="btn btn-ghost" type="button" @click="openDocsForProvider('codex')">
					...
					<div v-if="codexCheckResult" class="copilot-checks">
						<div class="copilot-checks-header">
							<span
								:class="[
									'copilot-overall',
									codexCheckResult.allPassed ? 'copilot-overall-pass' : 'copilot-overall-fail'
								]"
							>
					...
					<div class="agreement-section">
						...
						<ol class="agreement-list">
							<li v-for="key in agreementItemKeys" :key="key">
								<strong>{{ t(getAgreementItemKey(key, 'title')) }}</strong>
								{{ t(getAgreementItemKey(key, 'desc')) }}
							</li>
						</ol>
		...
		<ModalDialog
			...
			title="Codex 登录"
			:confirm-text="
				codexAuthState === 'success'
					? '完成'
					: codexAuthState === 'manual_auth' || codexAuthState === 'fallback_manual'
						? '我已在浏览器完成认证'
						: '我已在浏览器完成'
			"
		...
		>
			<div class="codex-auth-body">
				...
				<div v-else-if="codexAuthState === 'fallback_manual'" class="codex-auth-fallback">
					...
					<p class="codex-auth-fallback-title">
						{{ codexAuthMessage || '自动获取验证码较慢，请手动打开认证页面' }}
					</p>
					<div v-if="codexAuthRawOutput" class="codex-auth-output">
						<div class="codex-auth-output-header">命令行输出：</div>
						<pre class="codex-auth-output-pre">{{ codexAuthRawOutput }}</pre>
					...
					<div class="codex-auth-fallback-actions">
						<button
							class="btn btn-primary codex-auth-open-btn"
							type="button"
							@click="openCodexAuthUrl()"
						>
							手动打开认证页面
						</button>
					</div>
					<p class="codex-auth-fallback-hint">
						在浏览器中完成认证后，点击下方「我已在浏览器完成认证」按钮
					</p>
				</div>
				...
				<div v-else-if="codexAuthState === 'error'" class="codex-auth-error">
					...
					<div class="codex-auth-error-actions">
						<button
							class="btn btn-primary codex-auth-retry-btn"
							type="button"
					...
					<p class="codex-auth-error-hint">
						如果自动认证持续失败，请检查代理设置或使用终端手动执行
				...
				<div v-else-if="codexAuthState === 'manual_auth'" class="codex-auth-manual">
					...
					<div class="codex-auth-manual-steps">
						...
						<p>
							2. 执行命令：
							<code
								class="codex-auth-terminal-cmd"
								@click.stop="copyManualAuthCommand"
								:title="codexAuthCmdCopied ? '已复制!' : '点击复制'"
							>
								codex login --device-auth
							</code>
							<span
								v-if="codexAuthCmdCopied"
								style="color: #10a37f; font-size: 12px; margin-left: 6px"
							>
								✓ 已复制
							</span>
						</p>
				...
				<div v-else class="codex-auth-steps">
					<div
						class="codex-auth-step"
						:class="{
							active: codexAuthState === 'code_ready',
							done: codexAuthState === 'waiting' || codexAuthState === 'browser_opened'
						}"
					>
						...
						<div class="codex-auth-step-content">
							...
							<div
								v-if="codexAuthUserCode"
								class="codex-auth-code-box"
								@click="copyCodexAuthCode"
								title="点击复制验证码"
							>
								<code class="codex-auth-code">{{ codexAuthUserCode }}</code>
								<button
									class="codex-auth-copy-btn"
									type="button"
									:title="codexAuthCopied ? '已复制!' : '复制'"
									@click.stop="copyCodexAuthCode"
								>
									{{ codexAuthCopied ? '✓' : '📋' }}
								</button>
					...
					<div
						...
						:class="{
							active:
								codexAuthState === 'browser_opened' ||
								codexAuthState === 'browser_open_failed' ||
								codexAuthState === 'waiting',
							done: codexAuthState === 'waiting'
						}"
					>
						...
						<div class="codex-auth-step-content">
							...
							<div v-if="codexAuthVerificationUri" class="codex-auth-url-section">
								<p
									v-if="codexAuthState === 'browser_open_failed'"
									class="codex-auth-url-hint"
									style="color: #f85149"
								>
									⚠️ 自动打开浏览器失败，请手动点击下方链接：
								</p>
								<p v-else class="codex-auth-url-hint">
									浏览器应该已自动打开。如果没有打开，请手动点击下方链接：
								</p>
								<a class="codex-auth-url" href="javascript:void(0)" @click="openCodexAuthUrl()">
									{{ codexAuthVerificationUri }}
								</a>
							</div>
						</div>
					</div>
