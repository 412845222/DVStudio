<template>
	<div class="image-markup-page" @contextmenu.prevent>
		<div class="markup-toolbar-row">
			<div class="markup-toolbar-group left-group">
				<button
					class="toolbar-btn"
					:class="{ active: currentTool === 'view' }"
					@click="setTool('view')"
					:title="t('nodes.imageMarkup.viewMode')"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					...
					@click="setTool('brush')"
					:title="t('nodes.imageMarkup.brushTool')"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					...
					:title="t('nodes.imageMarkup.eraserTool') || '橡皮擦'"
				>
					<svg
						width="16"
						height="16"
					...
					>
						<path
							d="M20 20H7L3 16c-.8-.8-.8-2 0-2.8L14.8 1.4c.8-.8 2-.8 2.8 0l5 5c.8.8.8 2 0 2.8L12 20"
						/>
						<path d="M6 12l6 6" />
					</svg>
				</button>
				<button
					class="toolbar-btn"
					:class="{ active: currentTool === 'screenshot' }"
					@click="setTool('screenshot')"
					:title="t('nodes.imageMarkup.cropTool') || '裁剪'"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					class="toolbar-btn subject-select-btn"
					:class="{ active: currentTool === 'subject-select' }"
					@click="setTool('subject-select')"
					:title="'主体选择'"
				>
					<svg
						width="16"
				...
				<button
					...
					:disabled="!canUndo"
					@click="doUndo"
					:title="t('nodes.imageMarkup.undo') || '撤销 (Ctrl+Z)'"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					...
					:disabled="!canRedo"
					@click="doRedo"
					:title="t('nodes.imageMarkup.redo') || '重做 (Ctrl+Y)'"
				>
					<svg
						width="16"
						height="16"
			...
			<div class="markup-toolbar-group center-group">
				<button
					...
					:title="t('nodes.imageMarkup.flipH') || '水平翻转'"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					class="toolbar-btn"
					@click="flipVertical"
					:title="t('nodes.imageMarkup.flipV') || '垂直翻转'"
				>
					<svg
						width="16"
						height="16"
				...
				>
					...
					>
						<path d="M21 2v6h-6" />
						<path d="M3 12a9 9 0 1 0 3-6.7L21 8" />
					</svg>
				</button>
			</div>
			<div class="markup-toolbar-group right-group">
				...
				<button
					...
					:title="t('nodes.imageMarkup.clearAnnotations')"
				>
					<svg
						width="16"
						height="16"
				...
				<button
					...
					:disabled="!hasValidScreenshotRect"
				>
					<svg
						width="16"
						height="16"
		...
		<div class="sub-toolbar" v-if="currentTool === 'eraser'">
			<button
				class="mode-btn"
				:class="{ active: eraserMode === 'rect-bg-remove' }"
				@click="setEraserMode('rect-bg-remove')"
				:title="t('nodes.imageMarkup.eraserModeRectBg') || '框选去背景'"
			>
				<svg
					width="14"
					height="14"
			...
			<button
				class="mode-btn"
				:class="{ active: eraserMode === 'click-bg-remove' }"
				@click="setEraserMode('click-bg-remove')"
				:title="t('nodes.imageMarkup.eraserModeClickBg') || '一键去背景'"
			>
				<svg
					width="14"
					height="14"
			...
			<button
				class="mode-btn"
				:class="{ active: eraserMode === 'brush' }"
				@click="setEraserMode('brush')"
				:title="t('nodes.imageMarkup.eraserModeBrush') || '画笔擦除'"
			>
				<svg
					width="14"
					height="14"
			...
			<template v-if="eraserMode === 'brush'">
				<span class="sub-toolbar-label">
					{{ t('nodes.imageMarkup.eraserSize') || '橡皮擦大小' }}
				</span>
			...
			<template v-else>
				...
				<input
					...
					min="0"
					max="20"
					v-model.number="eraserFeather"
					class="slider slider-short"
				/>
		...
		<div class="sub-toolbar" v-if="currentTool === 'subject-select'">
			<span class="sub-toolbar-hint">
				{{ subjectSelectRect ? '已选中主体（点击其他物体可切换）' : '未检测到主体' }}
			</span>
			<div class="toolbar-divider"></div>
			<label class="checkbox-label">
			...
			<div class="toolbar-divider"></div>
			<button
				class="mode-btn btn-export-subject"
				@click="exportSubjectCrop"
				:disabled="!subjectSelectRect || subjectExporting"
			>
				<svg
					width="14"
					height="14"
		...
		<div
			class="markup-viewport"
			ref="viewportRef"
			:class="{
				'has-subtoolbar':
					currentTool === 'eraser' || currentTool === 'brush' || currentTool === 'subject-select',
		...
		<div class="image-info-bar" v-if="isImageLoaded">
			...
			<span v-if="currentTool === 'screenshot' && screenshotRect">
				| {{ t('nodes.imageMarkup.selectionSize') }}: {{ Math.round(screenshotRect.w) }} ×
				{{ Math.round(screenshotRect.h) }}
			</span>
