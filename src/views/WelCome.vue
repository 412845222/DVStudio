...
const platformHintText = computed(() => {
	...
	if (platformIsSteam.value && s.loggedIn) {
		return t('welcome.platformSteamLoggedIn', {
			name: s.user?.displayName || t('welcome.platformSteamUserUnknown')
		})
	}
	return ''
})
...
const backendStatusText = computed(() => {
	...
	if (backendLastError.value)
		return t('welcome.notRunningWithError', { error: backendLastError.value })
...
async function handleRetryStep(stepKey: string) {
	...
	if (failed) {
		appendLocalLog(
			t('welcome.retryFailed', { label: failed.label, suggestion: getRetrySuggestion(stepKey) })
		)
	}
...
async function handleCleanupOldProject() {
	const ok = window.confirm(t('welcome.cleanupConfirmMessage'))
	if (!ok) return

	const r = await cleanupOldProject()
	if (r?.ok) {
		appendLocalLog(t('welcome.cleanupSuccess'))
	} else {
			...
			<div class="col right">
				<div class="rightTop">
					...
					<div class="buttons">
						...
						>
							{{
								backendActionBusy === 'restart'
									? t('welcome.restarting')
									: t('welcome.restartBackend')
							}}
						</button>
						<button
							class="btn"
				...
				<div class="platformCard">
					...
					<div v-if="platformOverlayEnabled" class="platformOverlayInfo">
						Steam Overlay
						{{
							platformStatus?.overlayActive
								? t('welcome.steamOverlayActive')
								: t('welcome.steamOverlayAvailable')
						}}
					</div>
				</div>
