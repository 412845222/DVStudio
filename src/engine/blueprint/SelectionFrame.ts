...
export function drawSelectionFrame(
  ...
  if (isSaved && label) {
    ...
    if (actuallyEditing) {
      ...
      roundRect(ctx, tagX + 4 / cameraZoom, tagY + 3 / cameraZoom, tagW - 8 / cameraZoom, tagH - 6 / cameraZoom, 3 / cameraZoom);
      ctx.fill();
    }
    ...
    if (actuallyEditing && editState!.cursorBlink) {
      const cursorX = tagX + TAG_BAR_PADDING_X / cameraZoom + ctx.measureText(displayText).width + 1 / cameraZoom;
    ...
    const availableForInput = w - countWidth - TAG_BAR_PADDING_X / cameraZoom * 2 - saveBtnWidth - saveBtnMargin * 2;
    const inputWidth = Math.max(inputMinWidth, Math.min(availableForInput > 0 ? availableForInput : inputMinWidth, w * 0.5));
...
export function pointInTempFrameInput(screenPoint: Vector2, worldRect: Rect, nodeCount: number, camera: { zoom: number; worldToScreen(p: Vector2): Vector2 }): boolean {
  const screenTopLeft = camera.worldToScreen(new Vector2(worldRect.x, worldRect.y));
  const tagBarH = TAG_BAR_HEIGHT * camera.zoom;
  const z = camera.zoom;
  ...
  const inputWidth = Math.max(inputMinWidth, Math.min(availableForInput > 0 ? availableForInput : inputMinWidth, fullW * 0.5));
...
export function pointInTempFrameSaveBtn(screenPoint: Vector2, worldRect: Rect, nodeCount: number, camera: { zoom: number; worldToScreen(p: Vector2): Vector2 }): boolean {
  const screenTopLeft = camera.worldToScreen(new Vector2(worldRect.x, worldRect.y));
  const tagBarH = TAG_BAR_HEIGHT * camera.zoom;
  const z = camera.zoom;
