...
export class Scene extends Group implements Disposable {
	...
	setViewport(vp: { zoom?: number; panX?: number; panY?: number }): void {
		...
		if (vp.panX !== undefined)
			this.camera.setPositionDirect(-vp.panX / safeZoom, this.camera.position.y)
		if (vp.panY !== undefined)
			this.camera.setPositionDirect(this.camera.position.x, -vp.panY / safeZoom)

		const posChanged =
			Math.abs(this.camera.position.x - oldPosX) > 1e-6 ||
