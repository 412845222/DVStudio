 ...
 */
export const isFrameFullyMoved = (frameNodeIds: string[], movingNodeIds: Set<string>): boolean => {
 ...
 * 只记录直接包含关系（不传递）
 */
export const buildFrameContainmentTree = (frames: SavedSelectionFrame[]): Map<string, string[]> => {
			...
			if (frameContains(potentialParent, potentialChild)) {
				...
				if (parentSize > maxChildSize) {
					...
					for (const middle of frames) {
						...
						if (frameContains(potentialParent, middle) && frameContains(middle, potentialChild)) {
							isImmediate = false
							break
						}
					}
