		...
		if (sl && Array.isArray(sl.manualModelBindings)) {
			for (const mb of sl.manualModelBindings) {
				...
				if (mUrl)
					out.push({
						value: mUrl,
						source: 'sceneLayout.modelBinding',
						description: String(mb?.objectId ?? '模型')
					})
				...
				if (mAsset)
					out.push({
						value: mAsset,
						source: 'sceneLayout.modelBinding',
						description: String(mb?.objectId ?? '模型资产')
					})
				...
				if (mSource)
					out.push({
						value: mSource,
						source: 'sceneLayout.modelBinding',
						description: String(mb?.objectId ?? '模型源文件')
					})
				...
				if (mPath)
					out.push({
						value: mPath,
						source: 'sceneLayout.modelBinding',
						description: String(mb?.objectId ?? '模型路径')
					})
			}
		}
	}
		...
		if (sd && Array.isArray(sd.outputs)) {
			for (const o of sd.outputs) {
				...
				if (genId)
					out.push({
						value: genId,
						source: 'sceneDecompose.output',
						description: String(o?.name ?? o?.id ?? '分解输出')
					})
			}
		}
	}
		...
		if (m3) {
			...
			for (const key of urls) {
				...
				if (v)
					out.push({
						value: v,
						source: key.startsWith('last') ? 'model3d.input' : 'model3d.asset',
						description: key
					})
			}
		}
