...
import type { TimelineState } from '../core/timeline/types'
...
type LegacyTimelineState = Partial<TimelineState> & Record<string, unknown>
...
const applySnapshot = (snap: EditorSnapshot) => {
	...
	const nextTimeline = cloneJsonSafe(snap.timeline) as LegacyTimelineState
	const rawFps = typeof nextTimeline === 'object' && nextTimeline !== null ? nextTimeline.fps : null
	const fps = Math.max(1, Math.min(240, Math.floor(Number(rawFps ?? 60) || 60)))
	if (typeof nextTimeline === 'object' && nextTimeline !== null) {
		nextTimeline.fps = fps
	}
	TimelineStore.replaceState(nextTimeline as TimelineState)
}
