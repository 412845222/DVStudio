...
const createCore = (options: AIWorkflowHistoryOptions = {}) => {
  const maxHistory = typeof options.maxHistory === 'number' && options.maxHistory > 0 ? options.maxHistory : 60
  const debounceMs = typeof options.debounceMs === 'number' && options.debounceMs >= 0 ? options.debounceMs : 200
