...
const isWallMountedSupportSurface = (item: WorkflowSceneLayoutItem) => {
	...
	const relationTags = Array.isArray(item.relationTags)
		...
		: []
	if (!(placement.includes('wall') || mountType.includes('wall') || canonicalWallRole(item.wallRole).length > 0)) return false
	if (keyElementType === 'wall') return false
	if (keyElementType === 'window' || keyElementType === 'door' || keyElementType === 'opening') return false
	...
	if (semanticRole.includes('built-in-fixture') || semanticRole.includes('architectural-opening')) return false
	...
	return /(desk|table|workbench|workstation|counter|console|bench|board|desktop|台|桌|桌板|台板|台面|工作台|工位|操作台|长台|层板|搁板)/.test(tokens)
}
...
const isDeskLikeSurface = (item: WorkflowSceneLayoutItem) => {
	...
	return /(desk|table|workbench|workstation|counter|console|bench|台|桌|工作台|工位|岛台|桌面)/.test(tokens)
}
...
const resolvePlaceholderColor = (item: WorkflowSceneLayoutItem) => {
	...
	if (/monitor|computer|screen|display|tower|电脑|显示器|主机/.test(`${category} ${String(item.subCategory ?? '').toLowerCase()}`)) return '#d97706'
	if (/chair|seat|椅/.test(`${category} ${String(item.subCategory ?? '').toLowerCase()}`)) return '#16a34a'
	if (semanticRole.includes('wall-fixture') || /poster|decor|海报|贴纸/.test(`${category} ${String(item.subCategory ?? '').toLowerCase()}`)) return '#ec4899'
...
const isWallSurfaceLike = (item: WorkflowSceneLayoutItem) => {
	...
	return (
		String(item.keyElementType ?? '').trim().toLowerCase() === 'wall' ||
		String(item.semanticRole ?? '').trim().toLowerCase().includes('wall-fixture') ||
...
const snapWallAttachedItemToParentSurface = (
	...
	if (String(parent.keyElementType ?? '').trim().toLowerCase() !== 'wall') return item
	const role = canonicalWallRole(item.wallRole || parent.wallRole)
	if (!role) return item
	const next = cloneItem(item)
	const wallDepth = Math.max(0.05, safeNumber(parent.size.depth, 0.2) * Math.max(0.01, safeNumber(parent.scale?.z ?? 1, 1)))
	const childDepth = Math.max(0.05, safeNumber(next.size.depth, 0.2) * Math.max(0.01, safeNumber(next.scale?.z ?? 1, 1)))
	...
	const supportSurface = String(next.supportSurface ?? '').trim().toLowerCase()
...
const compactWallMountedSupportSurface = (
	...
	const supportedChildren = children.filter((child) => String(child.placement ?? '').trim().toLowerCase() === 'on-top')
	...
	const minX = supportedChildren.length
		? Math.min(...supportedChildren.map((child) => safeNumber(child.position.x, 0) - Math.max(0.05, safeNumber(child.size.width, 0.5)) * 0.5))
		: safeNumber(next.position.x, 0) - width * 0.5
	const maxX = supportedChildren.length
		? Math.max(...supportedChildren.map((child) => safeNumber(child.position.x, 0) + Math.max(0.05, safeNumber(child.size.width, 0.5)) * 0.5))
		: safeNumber(next.position.x, 0) + width * 0.5
	const minZ = supportedChildren.length
		? Math.min(...supportedChildren.map((child) => safeNumber(child.position.z, 0) - Math.max(0.05, safeNumber(child.size.depth, 0.5)) * 0.5))
		: safeNumber(next.position.z, 0) - depth * 0.5
	const maxZ = supportedChildren.length
		? Math.max(...supportedChildren.map((child) => safeNumber(child.position.z, 0) + Math.max(0.05, safeNumber(child.size.depth, 0.5)) * 0.5))
		: safeNumber(next.position.z, 0) + depth * 0.5
...
const rotateOffsetByAxis = (offset: OrientationOffset, axis: AxisName, delta: number): OrientationOffset => {
	if (axis === 'x') {
		return {
			...offset,
...
private renderFrame() {
	...
	if (this.lastRenderTs > 0) {
		...
		if (frameDelta > 0) {
			...
			this.perfFrameMsEma = this.perfFrameMsEma > 0
				? this.perfFrameMsEma * 0.82 + frameDelta * 0.18
				: frameDelta
			const fps = 1000 / frameDelta
			this.perfFpsEma = this.perfFpsEma > 0
				? this.perfFpsEma * 0.82 + fps * 0.18
				: fps
		}
	}
	this.lastRenderTs = now
	this.ensureTransformAttachmentValid()
	if (this.orbiting || this.transforming || this.controls.autoRotate === true || this.controls.enableDamping === true) {
	...
	this.perfRenderMsEma = this.perfRenderMsEma > 0
		? this.perfRenderMsEma * 0.82 + renderCost * 0.18
		: renderCost
...
cameraCfg?: { position?: { x: number; y: number; z: number }; target?: { x: number; y: number; z: number } },
...
const prevItemIds = this.currentItems.map(i => String(i.id ?? '')).sort().join('|')
if (previewModeChanged) this.cameraDirty = false
const bindingMap = new Map(
((Array.isArray(renderOptions?.modelBindings) ? renderOptions!.modelBindings! : []) as WorkflowSceneLayoutModelBinding[])
...
opacity: useTransparent ? (hasBoundModel ? (inferred ? 0.12 : 0.18) : inferred ? 0.28 : isDeskLikeSurface(item) ? 0.76 : 0.58) : hasBoundModel ? 0.22 : inferred ? 0.65 : 1,
...
new THREE.LineBasicMaterial({ color: '#e2e8f0', transparent: true, opacity: hasBoundModel ? 0.25 : isDeskLikeSurface(item) ? 0.82 : 0.55 })
)
...
const nextItemIds = this.currentItems.map(i => String(i.id ?? '')).sort().join('|')
...
async exportResolvedLayoutForUnreal(): Promise<WorkflowUnrealResolvedLayoutExport> {
	...
	for (const item of this.currentItems) {
		...
		const placeholderTransform = placeholderMesh ? this.captureObjectTransform(placeholderMesh) : null
		const placeholderBounds = placeholderMesh ? this.captureObjectBounds(placeholderMesh) : null
		const instances = Array.isArray(boundModel.children) && boundModel.children.length
		...
		const slotTransform =
			parentReferenceResult.parentReference.relativeTransform ?? this.captureObjectTransform(boundModel, actorOrigin)
		...
		const constraintDiagnostics: WorkflowUnrealResolvedConstraintDiagnostics = {
			exportMode:
				parentReferenceResult.parentReference.mode === 'parent-slot'
					? 'parent-relative'
					: 'root-relative',
			notes: parentReferenceResult.warnings.length ? [...parentReferenceResult.warnings] : undefined,
		}
		for (let index = 0; index < instances.length; index += 1) {
			...
			const orientationMode = item.orientationFix?.mode === 'manual' ? 'manual' as const : 'auto' as const
			const fitMode = item.fitMode === 'forced'
				? 'forced' as const
				: item.fitMode === 'filled'
				? 'filled' as const
				...
				? 'oriented' as const
				: 'normal' as const
			const fillMode = item.fillMode === 'fill-x' || item.fillMode === 'fill-y' || item.fillMode === 'fill-z'
				? item.fillMode
				: 'single'
			...
			slots.push({
				...
				materialOverrides: Array.isArray(item.materialOverrides) ? item.materialOverrides.map((entry) => ({ ...entry })) : undefined,
...
private normalizeLightingType(raw: unknown) {
	...
	if (text === 'rectarea' || text === 'rect-light' || text === 'area' || text === 'area-light') return 'rect-area'
...
private resolveAtmosphereSettings(config: WorkflowSceneLightingPreviewConfig) {
	const preset = String(config.atmosphere?.preset ?? config.lightingStyle ?? '').trim().toLowerCase()
	const brightness = String(config.atmosphere?.brightness ?? '').trim().toLowerCase()
	const presetMap: Record<string, { intensityScale: number; exposure: number; environmentIntensity: number }> = {
		'soft-daylight-interior': { intensityScale: 1.16, exposure: 1.08, environmentIntensity: 0.38 },
	...
	const presetSettings = presetMap[preset] ?? { intensityScale: 1.06, exposure: 1.04, environmentIntensity: 0.32 }
	const brightnessScale = brightness === 'low' ? 1.02 : brightness === 'high' ? 1.1 : brightness === 'low-medium' ? 1.06 : 1.04
	const intensityScale = this.clampNumber(
		config.globalSettings?.intensityScale ?? config.atmosphere?.intensityScale ?? presetSettings.intensityScale,
		0.45,
		1.8,
		presetSettings.intensityScale,
	) * brightnessScale
	const exposure = this.clampNumber(config.globalSettings?.exposure, 0.42, 1.12, presetSettings.exposure)
	const environmentIntensity = this.clampNumber(config.globalSettings?.environmentIntensity, 0.02, 0.28, presetSettings.environmentIntensity)
...
private getSceneScaleHint() {
	...
	let maxZ = -Infinity
	for (const item of this.currentItems) {
		const width = Math.max(0.05, safeNumber(item.size.width, 1) * Math.max(0.01, safeNumber(item.scale?.x ?? 1, 1)))
		const height = Math.max(0.05, safeNumber(item.size.height, 1) * Math.max(0.01, safeNumber(item.scale?.y ?? 1, 1)))
		const depth = Math.max(0.05, safeNumber(item.size.depth, 1) * Math.max(0.01, safeNumber(item.scale?.z ?? 1, 1)))
		const posX = safeNumber(item.position.x, 0)
		const posY = safeNumber(item.position.y, 0)
		const posZ = safeNumber(item.position.z, 0)
...
private addPreviewAssistLight(type: string, color: string, position: Vector3Like, lightDef: LightDefinition, baseIntensity: number) {
	...
	if (type === 'rect-area') {
		...
		const assist = new THREE.SpotLight(color, Math.max(0.4, baseIntensity * (roleText === 'fill' ? 0.36 : 0.28)))
		assist.position.copy(position)
		assist.angle = Math.max(0.55, Math.min(1.05, Math.atan2(Math.max(width, height) * 1.6, Math.max(0.4, dir.length()))))
...
private addLightDebugHelper(type: string, color: string, position: Vector3Like, lightDef: LightDefinition) {
	...
	const marker = this.createLightDebugMarker(`#${helperColor.getHexString()}`, Math.max(0.025, scaleHint * 0.006))
	...
	if (type === 'rect-area') {
		...
		helperGroup.add(this.createDebugLine([position, position.clone().add(normal.normalize().multiplyScalar(normalLength))], color, 0.85))
	} else if (type === 'spot') {
		const target = this.resolveLightTarget(position, lightDef, { x: position.x, y: position.y - 1, z: position.z })
...
private resolveLightTarget(position: Vector3Like, lightDef: LightDefinition, fallback: { x: number; y: number; z: number }) {
	...
	if (direction.lengthSq() > 0.0001) {
		return position.clone().add(direction.normalize().multiplyScalar(Math.max(1.2, this.getSceneScaleHint() * 0.35)))
	}
...
private parseLightingPreviewConfig(lightingJson: string): WorkflowSceneLightingPreviewConfig | null {
	const raw = String(lightingJson ?? '').trim()
	if (!raw) return null
	try {
		const parsed = JSON.parse(raw)
		return parsed && typeof parsed === 'object' ? (parsed as WorkflowSceneLightingPreviewConfig) : null
	} catch {
		return null
	}
}
...
private applyLightingPreview(
	...
	if (!config) {
		...
		this.baseDirectionalLight.intensity = 0.12 * this.getLightingTypeScale('directional', controls)
	...
	const inspectionAmbient = new THREE.AmbientLight(
		'#d9e6ff',
		Math.max(0, atmosphere.environmentIntensity * 0.22 * this.getLightingTypeScale('ambient', controls)),
	)
	...
	inspectionFill.position.set(this.getSceneScaleHint() * 0.9, this.getSceneScaleHint() * 1.35, this.getSceneScaleHint() * 1.05)
	...
	if (config.mainDirectionalLight) {
		...
		const pos = this.normalizeVector3(config.mainDirectionalLight.position, { x: 200, y: 300, z: 160 })
		this.baseDirectionalLight.position.copy(pos)
		this.baseDirectionalTarget.position.copy(this.normalizeVector3(config.mainDirectionalLight.target, { x: 0, y: 40, z: 0 }))
	} else {
...
private hasUsableCamera(cameraCfg?: { position?: { x: number; y: number; z: number }; target?: { x: number; y: number; z: number } }) {
return !!(
cameraCfg?.position &&
cameraCfg?.target &&
...
const firstPass = items.map((item) => {
	...
	const placement = String(next.placement ?? '').trim().toLowerCase()
	const supportSurface = String(next.supportSurface ?? '').trim().toLowerCase()
	const mountType = String(next.mountType ?? '').trim().toLowerCase()
	const role = canonicalWallRole(next.wallRole)
	const isWallAligned =
		role.length > 0 && !isWallMountedSupportSurface(next) && (
			String(next.keyElementType ?? '').trim().toLowerCase() === 'wall' ||
			placement.includes('wall') ||
			mountType.includes('wall') ||
			supportSurface.includes('wall') ||
			supportSurface === 'left' || supportSurface === 'right' || supportSurface === 'front' || supportSurface === 'back'
		)
	if (isWallAligned) {
		const width = Math.max(0.05, safeNumber(next.size.width, 1))
		const depth = Math.max(0.05, safeNumber(next.size.depth, 1))
...
const normalized = firstPass.map((item) => {
	...
	const compacted = compactWallMountedSupportSurface(item, childrenByParent.get(String(item.id ?? '').trim()) ?? [], parent)
...
if (!Number.isFinite(minX) || !Number.isFinite(maxX) || !Number.isFinite(minZ) || !Number.isFinite(maxZ)) return items
...
private async syncModelBindings(revision: number, bindings: Map<string, WorkflowSceneLayoutModelBinding>) {
const entries = Array.from(bindings.entries())
const changed: boolean[] = []
const batchSize = 2
for (let index = 0; index < entries.length; index += batchSize) {
	if (this.disposed || revision !== this.layoutRevision || this.pendingBindingRevision !== revision) return
...
private async attachModelBinding(revision: number, itemId: string, binding: WorkflowSceneLayoutModelBinding) {
...
if (this.disposed || revision !== this.layoutRevision || this.pendingBindingRevision !== revision) return false
...
const changed = this.mountBoundModel(itemId, item, latestMesh, template, autoAdjust ? 'auto' : 'keep')
latestMesh.userData.itemId = item.id
const fitChanged = !item.fitMode
	? this.setFitState(item, 'normal', '当前按占位盒预览，可继续尝试调整朝向、循环填充或强制适配。')
	: false
this.requestRender()
...
return new THREE.Vector3(
Math.max(0.05, safeNumber(item.size.width, 1) * Math.max(0.01, safeNumber(item.scale?.x ?? 1, 1))),
Math.max(0.05, safeNumber(item.size.height, 1) * Math.max(0.01, safeNumber(item.scale?.y ?? 1, 1))),
Math.max(0.05, safeNumber(item.size.depth, 1) * Math.max(0.01, safeNumber(item.scale?.z ?? 1, 1))),
)
}
...
private async rebuildBoundModelForItem(itemId: string, mode: 'auto' | 'manual' | 'keep' = 'keep') {
	const targetId = String(itemId ?? '').trim()
...
private resolveDominantMismatchAxis(size: { x: number; y: number; z: number }, target: { x: number; y: number; z: number }): AxisName {
...
private resolveManualOrientationOffset(
	...
	const fallbackAxes = (['x', 'y', 'z'] as AxisName[]).filter((axis) => !surfaceAxes.includes(axis))
	...
	const measuredCandidates = uniqueCandidates
		.map((offset) => this.measureOrientationCandidate(object, baseScale, baseQuaternion, basePosition, target, offset, item))
		.sort((left, right) => left.score - right.score)
	const nextOffset = measuredCandidates[0]?.offset
	if (nextOffset) return nextOffset
	for (const axis of orderedAxes) {
		const fallback = this.applySurfaceFacingConstraint(rotateOffsetByAxis(existingOffset, axis, 90), item)
...
private resolveAxisReferenceScale(axis: FillAxis, previewScaleMode: string, scaleX: number, scaleY: number, scaleZ: number) {
	const others = axis === 'x' ? [scaleY, scaleZ] : axis === 'y' ? [scaleX, scaleZ] : [scaleX, scaleY]
...
private resolveFillSuggestion(size: { x: number; y: number; z: number }, target: { x: number; y: number; z: number }, previewScaleMode: string): FillSuggestion | null {
	const ratios = {
	...
	for (const axis of axes) {
		...
		const otherDiff = Math.abs(Math.log(Math.max(otherRatios[0], 0.0001)) - Math.log(Math.max(otherRatios[1], 0.0001)))
		if (otherDiff > FILL_MATCH_TOLERANCE) continue
		const referenceScale = this.resolveAxisReferenceScale(axis, previewScaleMode, ratios.x, ratios.y, ratios.z)
		const otherCoverage = others.map((value) => (size[value] * referenceScale) / Math.max(target[value], 0.0001))
		if (otherCoverage.some((value) => !Number.isFinite(value) || value < 0.72 || value > 1 + FILL_MATCH_TOLERANCE)) {
			continue
		}
		...
		const countCandidates = [Math.floor(coverage), Math.ceil(coverage)]
			.filter((value, index, list) => Number.isFinite(value) && value >= 2 && list.indexOf(value) === index)
		...
		const otherCoveragePenalty = otherCoverage.reduce((sum, value) => sum + Math.abs(1 - value), 0) / otherCoverage.length
...
private resolveWorldFillSuggestion(
	...
	for (const entry of gaps.sort((left, right) => right.gap - left.gap)) {
		...
		const countCandidates = [Math.floor(rawCount), Math.ceil(rawCount)]
			.filter((count, index, list) => Number.isFinite(count) && count >= 2 && list.indexOf(count) === index)
			.map((count) => Math.min(FILL_MAX_COUNT, count))
		if (!countCandidates.length) {
			countCandidates.push(2)
		}
		for (const count of countCandidates) {
			const axisScale = placeholderSize[entry.axis] / Math.max(modelSize[entry.axis] * count, 0.0001)
...
const decision = this.resolveOrientationDecision(object, baseScale, baseQuaternion, basePosition, target, item, existingOffset, mode)

object.position.copy(basePosition)
object.scale.copy(baseScale)
object.quaternion.copy(baseQuaternion)
object.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(
...
if (!rotatedBox.isEmpty()) {
	...
	const scaleZ = useForcedScale
		...
		? Math.max(fillAxisScale, 0.0001)
		: useModelScale || !!fillAxis
		? (fillAxis ? placeholderScaleZ : uniformScale)
		: placeholderScaleZ
...
if (!normalizedBox.isEmpty()) {
	...
	const nextFix = {
	mode: mode === 'manual' ? 'manual' : mode === 'keep' ? item.orientationFix?.mode ?? 'auto' : 'auto',
...
const changed = !previousFix ||
	previousFix.mode !== nextFix.mode ||
	previousFix.confidence !== nextFix.confidence ||
	!orientationOffsetEquals(
		{ yaw: safeNumber(previousFix.yaw, 0), pitch: safeNumber(previousFix.pitch, 0), roll: safeNumber(previousFix.roll, 0) },
...
async adjustSelectedModelOrientation(): Promise<SceneLayoutActionResult> {
	if (!this.selectedId) return { ok: false, applied: false, mode: 'normal', message: '请先选中一个占位体。' }
	...
	if (!item || !binding || !mesh) {
		return { ok: false, applied: false, mode: 'normal', message: '当前占位体还没有可调整的真实模型。' }
	}
	const sourceUrl = String(binding.modelUrl ?? binding.modelAssetUrl ?? '').trim()
	if (!sourceUrl) {
		const fitChanged = this.setFitState(item, 'normal', '当前绑定没有可用的模型地址，无法调整朝向。')
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '当前绑定没有可用的模型地址，无法调整朝向。' }
	}
	try {
		const template = await this.loadModelTemplate(sourceUrl)
		if (this.disposed) {
		...
		const existingOffset = this.resolveExistingOrientationOffset(item)
		const nextOffset = this.resolveManualOrientationOffset(probe, baseScale, baseQuaternion, basePosition, target, item, existingOffset)
		item.orientationFix = {
			mode: 'manual',
		...
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '模型模板加载失败，暂时无法调整朝向。' }
	}
}
...
async cycleFillSelectedModel(): Promise<SceneLayoutActionResult> {
	...
	const mesh = this.meshesById.get(this.selectedId)
	if (!item || !binding || !mesh) {
		return { ok: false, applied: false, mode: 'normal', message: '当前占位体还没有可循环填充的真实模型。' }
	}
	const sourceUrl = String(binding.modelUrl ?? binding.modelAssetUrl ?? '').trim()
	if (!sourceUrl) {
		const fitChanged = this.setFitState(item, 'normal', '当前绑定没有可用的模型地址，无法循环填充。')
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '当前绑定没有可用的模型地址，无法循环填充。' }
	}
	const template = await this.loadModelTemplate(sourceUrl)
	if (item.fillMode) {
	...
	probe.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(
	...
	if (box.isEmpty()) {
		const fitChanged = this.setFitState(item, 'normal', '当前模型无法测量有效尺寸，暂时不能循环填充。')
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '当前模型无法测量有效尺寸，暂时不能循环填充。' }
	}
	const size = box.getSize(new THREE.Vector3())
	const target = this.resolvePlaceholderWorldSize(item)
	const previewScaleMode = String(item.previewScaleMode ?? 'placeholder').trim().toLowerCase()
	const suggestion = this.resolveWorldFillSuggestion(mesh, template, item) ?? this.resolveFillSuggestion(size, target, previewScaleMode)
	if (!suggestion) {
		...
		const fitChanged = this.setFitState(item, item.orientationFix ? 'oriented' : 'normal', message)
		if (hadFill || fitChanged) this.emitLayoutChange()
		this.requestRender()
		return { ok: false, applied: false, mode: item.orientationFix ? 'oriented' : 'normal', message }
	}
...
async forceFitSelectedModel(): Promise<SceneLayoutActionResult> {
	...
	const mesh = this.meshesById.get(this.selectedId)
	if (!item || !binding || !mesh) {
		return { ok: false, applied: false, mode: 'normal', message: '当前占位体还没有可强制适配的真实模型。' }
	}
	const sourceUrl = String(binding.modelUrl ?? binding.modelAssetUrl ?? '').trim()
	if (!sourceUrl) {
		const fitChanged = this.setFitState(item, 'normal', '当前绑定没有可用的模型地址，无法强制适配。')
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '当前绑定没有可用的模型地址，无法强制适配。' }
	}
	try {
		const template = await this.loadModelTemplate(sourceUrl)
		this.clearFillState(item)
		...
		if (fitChanged) this.emitLayoutChange()
		return { ok: false, applied: false, mode: 'normal', message: '模型模板加载失败，暂时无法强制适配。' }
	}
}
...
private resolveShouldAutoAdjust(binding: WorkflowSceneLayoutModelBinding, item: WorkflowSceneLayoutItem) {
...
private measureOrientationCandidate(
	...
	const previewScaleMode = String(item?.previewScaleMode ?? 'placeholder').trim().toLowerCase()
	const effective = previewScaleMode === 'model'
		? Math.max(0.0001, Math.min(sx, sy, sz))
		: 1
	const normalized = previewScaleMode === 'model'
		? [
...
private buildSurfaceSemantics(item: WorkflowSceneLayoutItem): WorkflowUnrealResolvedSurfaceSemantics {
...
private resolveSurfaceCategory(item: WorkflowSceneLayoutItem): WorkflowUnrealResolvedSurfaceSemantics['category'] {
	const placement = String(item.placement ?? '').trim().toLowerCase()
	const supportSurface = String(item.supportSurface ?? '').trim().toLowerCase()
	const mountType = String(item.mountType ?? '').trim().toLowerCase()
	if (isWallMountedSupportSurface(item)) {
...
private buildParentReference(
	...
	if (!parentBoundModel) {
		warnings.push(`占位体 ${item.name || item.id} 的父参考 ${parentId} 未生成真实模型，已退回 actor 根节点导出。`)
		return {
			parentReference: {
				mode: 'root',
...
line.renderOrder = 3
this.group.add(line)
this.relationLines.push({ childId: item.id, parentId, placement: String(item.placement ?? ''), line })
}
}
...
return {
center: new THREE.Vector3(item.position.x, item.position.y + size.height * 0.5, item.position.z),
...
if (normalized === 'ceiling' || normalized === 'ceiling_main' || normalized === 'auto-ceiling-shell') {
return this.currentItems.find((item) => String(item.id).includes('ceiling') || String(item.placement ?? '') === 'ceiling-shell')
}
if (normalized === 'floor' || normalized === 'floor_main' || normalized === 'auto-floor-shell') {
return this.currentItems.find((item) => String(item.id).includes('floor') || String(item.supportSurface ?? '') === 'floor')
}
return undefined
}
...
private hasMeaningfulDragDelta() {
	...
	return rx > DRAG_ROTATION_EPSILON_RAD || ry > DRAG_ROTATION_EPSILON_RAD || rz > DRAG_ROTATION_EPSILON_RAD
}
...
private updateIdleLoop() {
	const shouldIdle = this.interactiveActive && !this.previewModeActive && !this.renderSuspended && !this.disposed
