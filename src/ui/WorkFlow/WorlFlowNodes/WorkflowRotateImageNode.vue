...
const affineFromTriangles = (s0: Vec2, s1: Vec2, s2: Vec2, d0: Vec2, d1: Vec2, d2: Vec2) => {
...
const buildNanoPrompt = (yawRad: number, pitchRad: number) => {
	...
	const yawHint =
		yawDeg > 0 ? 'right-side orbit' : yawDeg < 0 ? 'left-side orbit' : 'front-aligned orbit'
	const pitchHint =
		pitchDeg > 0 ? 'high-angle view' : pitchDeg < 0 ? 'low-angle view' : 'eye-level view'
...
const emitRotateOutputNow = () => {
	...
	const outputKey = `${yawDeg}|${pitchDeg}|${loadedImageVersion}|${loadedImageKey.slice(0, 96)}`
...
const draw = () => {
	...
	const drawLine = (p1: Vec3, p2: Vec3, color: string, width = 1, dash: number[] = []) => {
	...
	const axLen = gw * 0.4
	// X: Red (Right)
	drawLine({ x: 0, y: yFloor, z: 0 }, { x: axLen, y: yFloor, z: 0 }, 'rgba(255, 60, 60, 0.9)', 2)
	...
	// Z: Blue (Forward)
	drawLine({ x: 0, y: yFloor, z: 0 }, { x: 0, y: yFloor, z: axLen }, 'rgba(60, 100, 255, 0.9)', 2)
	...
	for (const face of faces) {
		if (face.id === 'front') {
			...
			const perspectiveStrength = Math.max(Math.abs(Math.sin(angY)), Math.abs(Math.sin(angX)))
			const strips = Math.max(10, Math.min(36, Math.round(12 + perspectiveStrength * 24)))
