			...
			<div ref="scrollContainerRef" class="aiwf-log-panel__body">
				...
				<div v-else class="aiwf-log-panel__list" role="list">
					<div
						...
						:class="[
							`is-level-${entry.level.toLowerCase()}`,
					...
					>
						...
						<div class="aiwf-log-panel__item-meta">
							...
							<span class="aiwf-log-panel__item-level" :data-level="entry.level">
								{{ entry.level }}
							</span>
							<span class="aiwf-log-panel__item-category">
						...
						<pre
							v-if="expandedIds.has(entry.id) && entry.detail !== undefined"
							class="aiwf-log-panel__item-detail"
...
const filteredEntries = computed<BlueprintLogEntry[]>(() => {
	...
	if (!kw && levels.length === ALL_LEVELS.length && cats.length === ALL_CATEGORIES.length)
		return all
	...
	for (let i = 0; i < all.length; i += 1) {
		...
		if (kw) {
			const haystack = [entry.message, entry.tag ?? ''].join(' ').toLowerCase()
