<template>
	<teleport to="body">
		<div
			v-if="open"
			ref="panelEl"
			class="wf-gemini-panel"
			:class="{ animating: !isInteracting }"
			:style="panelStyle"
			@pointerdown.stop
		>
			<div
				class="wf-gemini-panel-header"
				@pointerdown="onHeaderPointerDown"
				@dblclick="onHeaderDoubleClick"
			>
				<div class="wf-gemini-panel-title-wrap">
					<div class="wf-gemini-panel-title">{{ t('tasks.gemini.title') }}</div>
					<div class="wf-gemini-panel-subtitle">{{ t('tasks.gemini.subtitle') }}</div>
				</div>
				<div
					v-if="configuredText"
					class="wf-gemini-panel-config"
					:class="`is-${configuredTone}`"
					:title="configuredText"
				>
					<span class="wf-gemini-panel-config-label">{{ t('tasks.gemini.apiConfig') }}</span>
					<span class="wf-gemini-panel-config-value">{{ configuredText }}</span>
				</div>
				<div class="wf-gemini-panel-actions" @pointerdown.stop>
					<button
						class="wf-gemini-panel-icon-btn"
						type="button"
						:title="sortModeTitle"
						@click="cycleSortMode"
					>
						<svg viewBox="0 0 16 16" aria-hidden="true" class="wf-gemini-panel-icon">
							<path
								d="M4 3h8M4 6h6M4 9h4"
								fill="none"
								stroke="currentColor"
								stroke-width="1.2"
								stroke-linecap="round"
							/>
							<path v-if="sortMode === 'date-desc'" d="M12 12l-2-2h4z" fill="currentColor" />
							<path v-else d="M12 10l-2 2h4z" fill="currentColor" />
						</svg>
					</button>
					<button
						class="wf-gemini-panel-btn"
						type="button"
						:disabled="actionBusyType === 'clear-completed'"
						@click="onClearCompleted"
					>
						{{ actionBusyType === 'clear-completed' ? t('tasks.gemini.clearing') : t('tasks.gemini.clearCompleted') }}
					</button>
					<button class="wf-gemini-panel-btn" type="button" @click="toggleMinimize">-</button>
					<button class="wf-gemini-panel-btn" type="button" @click="toggleMaximize">[]</button>
					<button class="wf-gemini-panel-btn danger" type="button" @click="emit('close')">x</button>
				</div>
			</div>

			<div v-if="!minimized" class="wf-gemini-panel-body">
				<div v-if="dataStatusText" class="wf-gemini-panel-status">
					{{ dataStatusText }}
				</div>
				<div class="wf-gemini-panel-toolbar">
					<input
						v-model.trim="searchText"
						class="wf-gemini-search"
						type="text"
						:placeholder="t('tasks.gemini.searchPlaceholder')"
					/>
					<button
						class="wf-gemini-panel-btn"
						type="button"
						:disabled="refreshBusy"
						@click="emit('refresh')"
					>
						{{ refreshBusy ? t('tasks.gemini.syncing') : t('tasks.gemini.syncBackend') }}
					</button>
					<div class="wf-gemini-panel-stats">{{ t('tasks.gemini.totalCount', { count: visibleTaskCount }) }}</div>
				</div>

				<div v-if="!filteredTasks.length" class="wf-gemini-panel-empty">
					{{ t('tasks.gemini.empty') }}
				</div>

				<div v-else class="wf-gemini-task-list">
					<div v-for="task in filteredTasks" :key="task.id" class="wf-gemini-task-card-root">
						<div
							class="wf-gemini-task-card"
							draggable="true"
							@dragstart="onTaskDragStart($event, task)"
						>
							<div class="wf-gemini-task-card-top">
								<div class="wf-gemini-task-chip-row">
									<span class="wf-gemini-task-chip">
										{{ t('tasks.gemini.typeImage') }}
									</span>
									<span class="wf-gemini-task-chip subtle">{{ task.model }}</span>
									<span class="wf-gemini-task-chip status" :class="`is-${task.status}`">
										{{ task.statusLabel }}
									</span>
								</div>
								<div class="wf-gemini-task-date">{{ task.createdAtLabel }}</div>
							</div>

							<div class="wf-gemini-task-main">
								<div class="wf-gemini-task-copy">
									<div class="wf-gemini-task-title">{{ task.title }}</div>
									<div class="wf-gemini-task-prompt">{{ task.promptPreview }}</div>
									<div class="wf-gemini-task-meta">{{ task.metaText }}</div>
								</div>
								<div v-if="taskThumbSrc(task)" class="wf-gemini-task-thumb-shell">
									<img
										class="wf-gemini-task-thumb"
										:src="taskThumbSrc(task)"
										alt="thumbnail"
										draggable="false"
										@error="onTaskThumbError(task.id)"
									/>
								</div>
							</div>

							<div class="wf-gemini-task-progress-row">
								<div class="wf-gemini-task-progress-track">
									<div
										class="wf-gemini-task-progress-fill"
										:style="{ width: `${task.progress}%` }"
									/>
								</div>
								<div class="wf-gemini-task-progress-label">{{ task.progress }}%</div>
							</div>

							<div class="wf-gemini-task-foot">
								<div class="wf-gemini-task-footnote">
									{{ task.status === 'completed' ? t('tasks.gemini.footnoteCompleted') : task.status === 'failed' ? t('tasks.gemini.footnoteFailed') : t('tasks.gemini.footnoteProcessing') }}
								</div>
								<div class="wf-gemini-task-action-row">
									<button
										class="wf-gemini-task-preview-btn"
										type="button"
										@click="onPreviewTask(task.id)"
									>
										{{ t('tasks.gemini.viewDetails') }}
									</button>
									<button
										class="wf-gemini-task-preview-btn"
										type="button"
										:disabled="!task.taskId || isBusy(task, 'refresh')"
										@click="onTaskAction(task, 'refresh')"
									>
										{{ isBusy(task, 'refresh') ? t('tasks.gemini.refreshing') : t('tasks.gemini.refreshStatus') }}
									</button>
									<button
										class="wf-gemini-task-preview-btn"
										type="button"
										:disabled="!task.taskId || task.status !== 'completed' || isBusy(task, 'import-output')"
										@click="onTaskAction(task, 'import-output')"
									>
										{{ isBusy(task, 'import-output') ? t('tasks.gemini.pulling') : t('tasks.gemini.pullArtifacts') }}
									</button>
									<button
										class="wf-gemini-task-preview-btn danger"
										type="button"
										:disabled="!task.taskId || isBusy(task, 'delete')"
										@click="onTaskAction(task, 'delete')"
									>
										{{ isBusy(task, 'delete') ? t('tasks.gemini.deleting') : t('tasks.gemini.deleteTask') }}
									</button>
								</div>
							</div>
						</div>
					</div>
				</div>

				<div v-if="detailVisible" class="wf-gemini-task-detail-mask" @click.self="closeDetail">
					<div class="wf-gemini-task-detail" @click.stop>
						<div class="wf-gemini-task-detail-header">
							<div class="wf-gemini-task-detail-title-wrap">
								<div class="wf-gemini-task-detail-title">
									{{ detailTask?.title || t('tasks.gemini.detailTitle') }}
								</div>
								<div class="wf-gemini-task-detail-subtitle">
									{{ detailTask?.model || t('tasks.gemini.detailSubtitle') }} ·
									{{ detailTask?.statusLabel || t('tasks.gemini.loading') }}
								</div>
							</div>
							<div class="wf-gemini-task-action-row">
								<button
									class="wf-gemini-task-preview-btn"
									type="button"
									:disabled="!detailTask?.taskId || detailLoading"
									@click="onDetailAction('refresh')"
								>
									{{ t('tasks.gemini.refreshStatus') }}
								</button>
								<button
									class="wf-gemini-task-preview-btn"
									type="button"
									:disabled="!detailTask?.taskId || detailTask?.status !== 'completed'"
									@click="onDetailAction('import-output')"
								>
									{{ t('tasks.gemini.pullArtifacts') }}
								</button>
								<button
									class="wf-gemini-task-preview-btn danger"
									type="button"
									:disabled="!detailTask?.taskId"
									@click="onDetailAction('delete')"
								>
									{{ t('tasks.gemini.deleteTask') }}
								</button>
								<button class="wf-gemini-panel-btn danger" type="button" @click="closeDetail">
									{{ t('common.close') }}
								</button>
							</div>
						</div>

						<div v-if="detailLoading" class="wf-gemini-task-detail-loading">{{ t('tasks.gemini.loadingDetails') }}</div>
						<div v-else-if="detailTask" class="wf-gemini-task-detail-body">
							<div v-if="detailTaskThumbSrc" class="wf-gemini-task-detail-thumb-section">
								<img
									class="wf-gemini-task-detail-thumb"
									:src="detailTaskThumbSrc"
									alt="result"
								/>
							</div>
							<div v-if="detailTask.resultImages && detailTask.resultImages.length > 1" class="wf-gemini-task-detail-images">
								<div
									v-for="(img, idx) in detailTask.resultImages"
									:key="idx"
									class="wf-gemini-task-detail-image-item"
								>
									<img
										:src="getResultImageUrl(img as Record<string, unknown>)"
										:alt="`result-${idx}`"
										class="wf-gemini-task-detail-image-thumb"
									/>
								</div>
							</div>
							<div class="wf-gemini-task-detail-grid">
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.taskId') }}</div>
									<div class="wf-gemini-task-detail-value monospace">
										{{ detailTask.taskId || t('tasks.gemini.notWritten') }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.source') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.nodeId ? t('tasks.gemini.localNode') : t('tasks.gemini.backendRecord') }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.modelLabel') }}</div>
									<div class="wf-gemini-task-detail-value highlight">
										{{ detailTask.model || '-' }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.imageCount') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.numImages ?? 1 }} 张
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.aspectRatio') }}</div>
									<div class="wf-gemini-task-detail-value highlight">
										{{ detailTask.aspectRatio || '1:1' }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.imageSize') }}</div>
									<div class="wf-gemini-task-detail-value highlight">
										{{ detailTask.imageSize || '2K' }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.thinkingLevel') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.thinkingLevel || 'minimal' }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.progress') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.progress }}%
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.createdAt') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.createdAtLabel || '-' }}
									</div>
								</div>
								<div class="wf-gemini-task-detail-card">
									<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.updatedAt') }}</div>
									<div class="wf-gemini-task-detail-value">
										{{ detailTask.updatedAtLabel || '-' }}
									</div>
								</div>
							</div>

							<div v-if="detailTask.prompt" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.prompt') }}</div>
								<div class="wf-gemini-task-detail-block">{{ detailTask.prompt }}</div>
							</div>
							<div v-if="detailTask.negativePrompt" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.negativePrompt') }}</div>
								<div class="wf-gemini-task-detail-block">
									{{ detailTask.negativePrompt }}
								</div>
							</div>
							<div v-if="detailTask.statusText" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.statusDesc') }}</div>
								<div class="wf-gemini-task-detail-block">{{ detailTask.statusText }}</div>
							</div>
							<div v-if="detailTask.errorMessage" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.errorMessage') }}</div>
								<div class="wf-gemini-task-detail-block error">
									{{ detailTask.errorMessage }}
								</div>
							</div>
							<div v-if="detailTask.resultImages && detailTask.resultImages.length > 0" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.artifacts') }}</div>
								<div class="wf-gemini-task-detail-links">
									<div
										v-for="(img, idx) in detailTask.resultImages"
										:key="idx"
										class="wf-gemini-task-detail-block monospace"
									>
										[{{ idx + 1 }}] {{ String((img as Record<string, unknown>).filename || (img as Record<string, unknown>).relativePath || (img as Record<string, unknown>).localPath || '') }}
									</div>
								</div>
							</div>
							<div v-if="detailRequestJson" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.requestPayload') }}</div>
								<pre class="wf-gemini-task-detail-code">{{ detailRequestJson }}</pre>
							</div>
							<div v-if="detailResponseJson" class="wf-gemini-task-detail-section">
								<div class="wf-gemini-task-detail-label">{{ t('tasks.gemini.responsePayload') }}</div>
								<pre class="wf-gemini-task-detail-code">{{ detailResponseJson }}</pre>
							</div>
						</div>
						<div v-else class="wf-gemini-task-detail-empty">{{ t('tasks.gemini.noDetails') }}</div>
					</div>
				</div>
			</div>

			<div
				class="wf-gemini-resize wf-gemini-resize-n"
				@pointerdown.stop="onResizeStart('n', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-s"
				@pointerdown.stop="onResizeStart('s', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-e"
				@pointerdown.stop="onResizeStart('e', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-w"
				@pointerdown.stop="onResizeStart('w', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-ne"
				@pointerdown.stop="onResizeStart('ne', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-nw"
				@pointerdown.stop="onResizeStart('nw', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-se"
				@pointerdown.stop="onResizeStart('se', $event)"
			/>
			<div
				class="wf-gemini-resize wf-gemini-resize-sw"
				@pointerdown.stop="onResizeStart('sw', $event)"
			/>
		</div>
	</teleport>
</template>

<script lang="ts">
export type GeminiTaskPanelItem = {
	id: string
	nodeId?: string
	title: string
	taskId?: string
	model: string
	modelId?: string
	status: 'submitting' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'idle'
	statusLabel: string
	progress: number
	promptPreview: string
	metaText: string
	thumbnailUrl?: string
	aspectRatio?: string
	imageSize?: string
	thinkingLevel?: string
	numImages?: number
	negativePrompt?: string
	resultImages?: Array<Record<string, unknown>>
	errorMessage?: string
	statusText?: string
	createdAt: number
	createdAtLabel?: string
	updatedAtLabel?: string
	payload: Record<string, unknown>
}

export type GeminiTaskPanelDetail = {
	id: string
	title: string
	taskId?: string
	nodeId?: string
	model?: string
	modelId?: string
	status: string
	statusLabel: string
	progress: number
	prompt?: string
	negativePrompt?: string
	statusText?: string
	errorMessage?: string
	thumbnailUrl?: string
	aspectRatio?: string
	imageSize?: string
	thinkingLevel?: string
	numImages?: number
	createdAtLabel?: string
	updatedAtLabel?: string
	resultImages?: Array<Record<string, unknown>>
	requestPayload?: Record<string, unknown>
	responsePayload?: Record<string, unknown>
}

export type GeminiTaskPanelAction = 'refresh' | 'delete' | 'import-output' | 'clear-completed'
</script>

<script setup lang="ts">
import { computed, onBeforeUnmount, ref, watch } from 'vue'
import { useI18n } from '../../i18n'

const { t } = useI18n()

const props = defineProps<{
	open: boolean
	tasks: GeminiTaskPanelItem[]
	dataStatusText?: string
	configured?: boolean
	refreshBusy?: boolean
	detailTaskId?: string
	detailTask?: GeminiTaskPanelDetail | null
	detailLoading?: boolean
	actionBusyTaskId?: string
	actionBusyType?: GeminiTaskPanelAction | ''
}>()

const emit = defineEmits<{
	(e: 'close'): void
	(e: 'preview-task', taskId: string): void
	(e: 'refresh'): void
	(
		e: 'task-action',
		payload: {
			taskId: string
			action: GeminiTaskPanelAction
			nodeId?: string
		}
	): void
}>()

const panelEl = ref<HTMLElement | null>(null)
const minimized = ref(false)
const maximized = ref(false)
const position = ref({ x: 452, y: 16 })
const size = ref({ w: 460, h: 560 })
const isInteracting = ref(false)
const searchText = ref('')
const sortMode = ref<'date-desc' | 'date-asc'>('date-desc')
const openedDetailTaskId = ref('')
const failedTaskThumbIds = ref<Set<string>>(new Set())

const getResultImageUrl = (img: Record<string, unknown>): string => {
	const dwebUrl = String(img?.dwebUrl || '').trim()
	if (dwebUrl) return dwebUrl
	const localPath = String(img?.localPath || '').trim()
	if (localPath) return `file://${localPath.replace(/\\/g, '/')}`
	return ''
}

const taskThumbSrc = (task: GeminiTaskPanelItem) => {
	const id = String(task?.id ?? '').trim()
	if (!id) return ''
	if (failedTaskThumbIds.value.has(id)) return ''
	const thumb = String(task?.thumbnailUrl ?? '').trim()
	if (thumb) return thumb
	const resultImages = Array.isArray(task?.resultImages) ? task.resultImages : []
	if (resultImages.length > 0) {
		return getResultImageUrl(resultImages[0] as Record<string, unknown>)
	}
	return ''
}

const onTaskThumbError = (taskId: string) => {
	const id = String(taskId || '').trim()
	if (!id) return
	const next = new Set(failedTaskThumbIds.value)
	next.add(id)
	failedTaskThumbIds.value = next
}

let drag: {
	startX: number
	startY: number
	originX: number
	originY: number
} | null = null

let resize: {
	dir: string
	startX: number
	startY: number
	startW: number
	startH: number
	startLeft: number
	startTop: number
} | null = null

const clamp = (v: number, min: number, max: number) => Math.max(min, Math.min(max, v))
const safeTopInset = () => {
	const raw = getComputedStyle(document.documentElement).getPropertyValue('--aiwf-safe-top')
	const parsed = Number.parseFloat(String(raw || '').trim())
	if (!Number.isFinite(parsed)) return 0
	return Math.max(0, parsed)
}
const minTopBound = (pad = 12) => Math.max(pad, Math.round(safeTopInset()) + 8)
const MIN_WIDTH = 360
const MIN_HEIGHT = 280
const MINIMIZED_HEIGHT = 36

const resetPosition = () => {
	const pad = 16
	const minTop = minTopBound(pad)
	const w = size.value.w
	const h = size.value.h
	position.value = {
		x: clamp(window.innerWidth - w - pad, pad, Math.max(pad, window.innerWidth - w - pad)),
		y: minTop
	}
}

watch(
	() => props.open,
	(open) => {
		if (!open) return
		minimized.value = false
		maximized.value = false
		size.value = { w: 460, h: 560 }
		resetPosition()
	}
)

watch(
	() => props.open,
	(open) => {
		if (open) return
		openedDetailTaskId.value = ''
	}
)

watch(
	() => props.tasks,
	(next) => {
		const keep = new Set(
			(Array.isArray(next) ? next : [])
				.map((task) => String(task?.id ?? '').trim())
				.filter((id) => !!id)
		)
		const remained = new Set<string>()
		for (const id of failedTaskThumbIds.value.values()) {
			if (keep.has(id)) remained.add(id)
		}
		failedTaskThumbIds.value = remained
	}
)

const filteredTasks = computed(() => {
	const keyword = searchText.value.trim().toLowerCase()
	let list = Array.isArray(props.tasks) ? props.tasks.slice() : []

	const matchesKeyword = (item: GeminiTaskPanelItem) => {
		const hay = [
			item.title,
			item.model,
			item.promptPreview,
			item.metaText,
			item.taskId,
			item.aspectRatio
		]
			.map((x) => String(x ?? '').toLowerCase())
			.join('\n')
		return hay.includes(keyword)
	}

	if (keyword) {
		list = list.filter(matchesKeyword)
	}

	list.sort((a, b) =>
		sortMode.value === 'date-asc' ? a.createdAt - b.createdAt : b.createdAt - a.createdAt
	)
	return list
})

const visibleTaskCount = computed(() => filteredTasks.value.length)

const sortModeTitle = computed(() =>
	sortMode.value === 'date-desc' ? t('tasks.gemini.sortDesc') : t('tasks.gemini.sortAsc')
)
const detailTask = computed(() => props.detailTask ?? null)
const detailVisible = computed(() => !!openedDetailTaskId.value)
const detailLoading = computed(
	() => props.detailLoading === true && openedDetailTaskId.value === props.detailTaskId
)
const detailTaskThumbSrc = computed(() => {
	const task = detailTask.value
	if (!task) return ''
	const thumb = String(task.thumbnailUrl || '').trim()
	if (thumb) return thumb
	const resultImages = Array.isArray(task.resultImages) ? task.resultImages : []
	if (resultImages.length > 0) {
		return getResultImageUrl(resultImages[0] as Record<string, unknown>)
	}
	return ''
})
const detailRequestJson = computed(() => {
	if (!detailTask.value?.requestPayload) return ''
	try {
		return JSON.stringify(detailTask.value.requestPayload, null, 2)
	} catch {
		return ''
	}
})
const detailResponseJson = computed(() => {
	if (!detailTask.value?.responsePayload) return ''
	try {
		return JSON.stringify(detailTask.value.responsePayload, null, 2)
	} catch {
		return ''
	}
})

const cycleSortMode = () => {
	sortMode.value = sortMode.value === 'date-desc' ? 'date-asc' : 'date-desc'
}

const toggleMinimize = () => {
	minimized.value = !minimized.value
	if (!minimized.value) return
	const pad = 16
	const minTop = minTopBound(pad)
	position.value = {
		x: clamp(position.value.x, pad, Math.max(pad, window.innerWidth - size.value.w - pad)),
		y: clamp(
			window.innerHeight - MINIMIZED_HEIGHT - pad,
			minTop,
			Math.max(minTop, window.innerHeight - MINIMIZED_HEIGHT - pad)
		)
	}
}

const toggleMaximize = () => {
	maximized.value = !maximized.value
	const pad = 12
	const minTop = minTopBound(pad)
	if (maximized.value) {
		position.value = { x: pad, y: minTop }
		size.value = {
			w: Math.max(MIN_WIDTH, window.innerWidth - pad * 2),
			h: Math.max(MIN_HEIGHT, window.innerHeight - minTop - pad)
		}
		return
	}
	size.value = { w: 460, h: 560 }
	resetPosition()
}

const onTaskDragStart = (event: DragEvent, task: GeminiTaskPanelItem) => {
	const dt = event.dataTransfer
	if (!dt) return
	try {
		dt.effectAllowed = 'copy'
		const resultImages = Array.isArray(task.resultImages) ? task.resultImages : []
		let firstImageUrl = task.thumbnailUrl || ''
		if (!firstImageUrl && resultImages.length > 0) {
			firstImageUrl = getResultImageUrl(resultImages[0] as Record<string, unknown>)
		}
		dt.setData('application/x-dweb-gemini-task-item', JSON.stringify(task.payload))
		if (firstImageUrl) {
			dt.setData('text/uri-list', firstImageUrl)
			dt.setData('downloadURL', `image/png:${task.taskId || 'gemini-image'}.png:${firstImageUrl}`)
		}
		dt.setData('text/plain', task.promptPreview || task.title)
	} catch {
		// ignore
	}
}

const closeDetail = () => {
	openedDetailTaskId.value = ''
}

const onPreviewTask = (taskId: string) => {
	openedDetailTaskId.value = taskId
	emit('preview-task', taskId)
}

const isBusy = (task: GeminiTaskPanelItem, action: GeminiTaskPanelAction) => {
	const taskId = String(task.taskId ?? '').trim()
	if (!taskId) return false
	return (
		String(props.actionBusyTaskId ?? '').trim() === taskId &&
		String(props.actionBusyType ?? '').trim() === action
	)
}

const onTaskAction = (task: GeminiTaskPanelItem, action: GeminiTaskPanelAction) => {
	const taskId = String(task.taskId ?? '').trim()
	if (!taskId && action !== 'clear-completed') return
	emit('task-action', {
		taskId,
		action,
		nodeId: String(task.nodeId ?? '').trim() || undefined
	})
}

const onClearCompleted = () => {
	emit('task-action', { taskId: '', action: 'clear-completed' })
}

const onDetailAction = (action: GeminiTaskPanelAction) => {
	const task = detailTask.value
	if (!task) return
	const taskId = String(task.taskId ?? '').trim()
	if (!taskId) return
	emit('task-action', {
		taskId,
		action,
		nodeId: String(task.nodeId ?? '').trim() || undefined
	})
}

const panelStyle = computed(() => ({
	left: `${position.value.x}px`,
	top: `${position.value.y}px`,
	width: `${size.value.w}px`,
	height: minimized.value ? `${MINIMIZED_HEIGHT}px` : `${size.value.h}px`
}))

const dataStatusText = computed(() => String(props.dataStatusText ?? '').trim())
const configuredText = computed(() => {
	if (props.configured === true) return t('tasks.gemini.configured')
	if (props.configured === false) return t('tasks.gemini.notConfigured')
	return ''
})
const configuredTone = computed(() => (props.configured ? 'ok' : 'warn'))

const setGlobalPanelInteraction = (active: boolean) => {
	document.body.classList.toggle('wf-gemini-panel-no-select', active)
}

const onHeaderPointerDown = (event: PointerEvent) => {
	if (event.button !== 0) return
	if (maximized.value) return
	const target = event.target as HTMLElement | null
	if (target?.closest('button, .wf-gemini-resize')) return
	event.preventDefault()
	const startX = event.clientX
	const startY = event.clientY
	const startLeft = position.value.x
	const startTop = position.value.y
	isInteracting.value = true
	setGlobalPanelInteraction(true)
	drag = {
		startX,
		startY,
		originX: startLeft,
		originY: startTop
	}
	const move = (ev: PointerEvent) => {
		if (!drag) return
		const pad = 12
		const minTop = minTopBound(pad)
		const nextX = clamp(
			drag.originX + (ev.clientX - drag.startX),
			pad,
			Math.max(pad, window.innerWidth - size.value.w - pad)
		)
		const currentHeight = minimized.value ? MINIMIZED_HEIGHT : size.value.h
		const nextY = clamp(
			drag.originY + (ev.clientY - drag.startY),
			minTop,
			Math.max(minTop, window.innerHeight - currentHeight - pad)
		)
		position.value = { x: nextX, y: nextY }
	}
	const up = () => {
		drag = null
		isInteracting.value = false
		setGlobalPanelInteraction(false)
		window.removeEventListener('pointermove', move)
		window.removeEventListener('pointerup', up)
		window.removeEventListener('pointercancel', up)
	}
	window.addEventListener('pointermove', move)
	window.addEventListener('pointerup', up, { once: true })
	window.addEventListener('pointercancel', up, { once: true })
}

const onHeaderDoubleClick = () => toggleMaximize()

const onResizeStart = (dir: string, event: PointerEvent) => {
	if (event.button !== 0) return
	if (maximized.value) return
	event.preventDefault()
	if (minimized.value) minimized.value = false
	isInteracting.value = true
	setGlobalPanelInteraction(true)
	resize = {
		dir,
		startX: event.clientX,
		startY: event.clientY,
		startW: size.value.w,
		startH: size.value.h,
		startLeft: position.value.x,
		startTop: position.value.y
	}
	const move = (ev: PointerEvent) => {
		if (!resize) return
		const dx = ev.clientX - resize.startX
		const dy = ev.clientY - resize.startY
		let nextW = resize.startW
		let nextH = resize.startH
		let nextLeft = resize.startLeft
		let nextTop = resize.startTop

		if (resize.dir.includes('e')) nextW = resize.startW + dx
		if (resize.dir.includes('s')) nextH = resize.startH + dy
		if (resize.dir.includes('w')) {
			nextW = resize.startW - dx
			nextLeft = resize.startLeft + dx
		}
		if (resize.dir.includes('n')) {
			nextH = resize.startH - dy
			nextTop = resize.startTop + dy
		}

		nextW = Math.max(MIN_WIDTH, nextW)
		nextH = Math.max(MIN_HEIGHT, nextH)

		const minTop = minTopBound(12)
		const maxLeft = Math.max(12, window.innerWidth - nextW - 12)
		const maxTop = Math.max(minTop, window.innerHeight - nextH - 12)
		nextLeft = clamp(nextLeft, 12, maxLeft)
		nextTop = clamp(nextTop, minTop, maxTop)

		size.value = { w: nextW, h: nextH }
		position.value = { x: nextLeft, y: nextTop }
	}
	const up = () => {
		resize = null
		isInteracting.value = false
		setGlobalPanelInteraction(false)
		window.removeEventListener('pointermove', move)
		window.removeEventListener('pointerup', up)
		window.removeEventListener('pointercancel', up)
	}
	window.addEventListener('pointermove', move)
	window.addEventListener('pointerup', up, { once: true })
	window.addEventListener('pointercancel', up, { once: true })
}

onBeforeUnmount(() => {
	drag = null
	resize = null
	setGlobalPanelInteraction(false)
})
</script>

<style scoped>
.wf-gemini-panel {
	position: fixed;
	z-index: var(--aiwf-floating-z-index, 101);
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.88);
	background: linear-gradient(
		180deg,
		rgb(from var(--dweb-defualt-dark) r g b / 0.92),
		rgb(from var(--dweb-defualt) r g b / 0.88)
	);
	box-shadow: 0 24px 64px rgb(0 0 0 / 0.34);
	backdrop-filter: blur(12px);
	display: flex;
	flex-direction: column;
	overflow: hidden;
}

.wf-gemini-panel.animating {
	transition:
		left 0.16s ease,
		top 0.16s ease,
		width 0.16s ease,
		height 0.16s ease;
}

.wf-gemini-panel-status {
	margin-bottom: 10px;
	padding: 8px 10px;
	border: 1px solid rgba(99, 162, 255, 0.24);
	background: rgba(99, 162, 255, 0.1);
	color: rgba(231, 238, 255, 0.92);
	font-size: 12px;
	line-height: 1.5;
	min-height: 52px;
	box-sizing: border-box;
	overflow: hidden;
}

.wf-gemini-resize {
	position: absolute;
	z-index: 2;
	background: transparent;
}

.wf-gemini-resize-n,
.wf-gemini-resize-s {
	left: 8px;
	right: 8px;
	height: 6px;
	cursor: ns-resize;
}

.wf-gemini-resize-n {
	top: -3px;
}

.wf-gemini-resize-s {
	bottom: -3px;
}

.wf-gemini-resize-e,
.wf-gemini-resize-w {
	top: 8px;
	bottom: 8px;
	width: 6px;
	cursor: ew-resize;
}

.wf-gemini-resize-e {
	right: -3px;
}

.wf-gemini-resize-w {
	left: -3px;
}

.wf-gemini-resize-ne,
.wf-gemini-resize-nw,
.wf-gemini-resize-se,
.wf-gemini-resize-sw {
	width: 10px;
	height: 10px;
}

.wf-gemini-resize-ne {
	top: -5px;
	right: -5px;
	cursor: nesw-resize;
}

.wf-gemini-resize-nw {
	top: -5px;
	left: -5px;
	cursor: nwse-resize;
}

.wf-gemini-resize-se {
	right: -5px;
	bottom: -5px;
	cursor: nwse-resize;
}

.wf-gemini-resize-sw {
	left: -5px;
	bottom: -5px;
	cursor: nesw-resize;
}

.wf-gemini-panel-header {
	display: flex;
	align-items: center;
	justify-content: space-between;
	gap: 12px;
	padding: 8px 10px;
	border-bottom: 1px solid var(--vscode-border);
	cursor: move;
	user-select: none;
}

.wf-gemini-panel-title-wrap {
	min-width: 0;
	display: grid;
	gap: 2px;
}

.wf-gemini-panel-title {
	font-size: 13px;
	color: var(--vscode-fg);
}

.wf-gemini-panel-subtitle {
	font-size: 11px;
	color: var(--vscode-fg-muted);
}

.wf-gemini-panel-actions {
	display: flex;
	gap: 6px;
	margin-left: auto;
}

.wf-gemini-panel-config {
	display: inline-flex;
	align-items: center;
	gap: 8px;
	min-width: 0;
	padding: 5px 10px;
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.88);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.72);
	color: var(--vscode-fg);
	font-size: 11px;
}

.wf-gemini-panel-config-label {
	color: var(--vscode-fg-muted);
}

.wf-gemini-panel-config-value {
	max-width: 100px;
	overflow: hidden;
	text-overflow: ellipsis;
	white-space: nowrap;
}

.wf-gemini-panel-config.is-ok {
	border-color: rgb(56 189 140 / 0.55);
	color: #bbf7d0;
}

.wf-gemini-panel-config.is-warn {
	border-color: rgb(248 113 113 / 0.55);
	color: #fecaca;
}

.wf-gemini-panel-btn,
.wf-gemini-panel-icon-btn,
.wf-gemini-task-preview-btn {
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.88);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.68);
	color: var(--vscode-fg);
	font-size: 12px;
	padding: 4px 8px;
	cursor: pointer;
}

.wf-gemini-panel-btn:disabled,
.wf-gemini-panel-icon-btn:disabled,
.wf-gemini-task-preview-btn:disabled {
	opacity: 0.6;
	cursor: not-allowed;
}

.wf-gemini-panel-btn.danger {
	color: #fecaca;
}

.wf-gemini-task-preview-btn.danger {
	color: #fecaca;
}

.wf-gemini-panel-icon {
	width: 14px;
	height: 14px;
}

.wf-gemini-panel-body {
	display: flex;
	flex-direction: column;
	gap: 10px;
	padding: 10px;
	min-height: 0;
	flex: 1;
}

.wf-gemini-panel-toolbar {
	display: flex;
	align-items: center;
	gap: 10px;
}

.wf-gemini-search {
	flex: 1;
	min-width: 0;
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.72);
	color: var(--vscode-fg);
	padding: 8px 10px;
	font-size: 12px;
}

.wf-gemini-panel-stats,
.wf-gemini-panel-empty,
.wf-gemini-task-meta,
.wf-gemini-task-footnote,
.wf-gemini-task-date,
.wf-gemini-task-progress-label {
	font-size: 12px;
	color: var(--vscode-fg-muted);
}

.wf-gemini-task-list {
	min-height: 0;
	overflow: auto;
	display: grid;
	align-content: start;
	grid-auto-rows: max-content;
	gap: 10px;
}

:global(body.wf-gemini-panel-no-select) {
	user-select: none;
	-webkit-user-select: none;
}

.wf-gemini-task-card {
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.58);
	padding: 10px;
	display: grid;
	gap: 10px;
	overflow: hidden;
	height: 200px;
	grid-template-rows: auto 1fr auto auto;
}

.wf-gemini-task-card-top,
.wf-gemini-task-foot,
.wf-gemini-task-progress-row {
	display: flex;
	align-items: center;
	justify-content: space-between;
	gap: 10px;
}

.wf-gemini-task-action-row {
	display: flex;
	align-items: center;
	gap: 6px;
	flex-wrap: wrap;
	justify-content: flex-end;
}

.wf-gemini-task-chip-row {
	display: flex;
	flex-wrap: wrap;
	gap: 6px;
}

.wf-gemini-task-chip {
	padding: 2px 7px;
	border: 1px solid rgb(139 92 246 / 0.48);
	color: #c4b5fd;
	font-size: 11px;
}

.wf-gemini-task-chip.subtle {
	border-color: rgb(from var(--vscode-border) r g b / 0.68);
	color: var(--vscode-fg-muted);
}

.wf-gemini-task-chip.status.is-processing,
.wf-gemini-task-chip.status.is-submitting {
	border-color: rgb(90 180 255 / 0.68);
}

.wf-gemini-task-chip.status.is-completed {
	border-color: rgb(56 189 140 / 0.72);
	color: #bbf7d0;
}

.wf-gemini-task-chip.status.is-failed {
	border-color: rgb(248 113 113 / 0.72);
	color: #fecaca;
}

.wf-gemini-task-chip.status.is-cancelled {
	border-color: rgb(251 191 36 / 0.55);
	color: #fde68a;
}

.wf-gemini-task-main {
	display: grid;
	grid-template-columns: minmax(0, 1fr) 90px;
	gap: 10px;
	min-height: 0;
}

.wf-gemini-task-copy {
	min-width: 0;
	display: grid;
	gap: 6px;
}

.wf-gemini-task-title {
	font-size: 13px;
	color: var(--vscode-fg);
	line-height: 1.35;
	display: -webkit-box;
	-webkit-line-clamp: 1;
	-webkit-box-orient: vertical;
	overflow: hidden;
}

.wf-gemini-task-prompt {
	font-size: 12px;
	color: var(--vscode-fg);
	line-height: 1.45;
	display: -webkit-box;
	-webkit-line-clamp: 2;
	-webkit-box-orient: vertical;
	overflow: hidden;
}

.wf-gemini-task-thumb-shell {
	width: 90px;
	height: 90px;
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	overflow: hidden;
	display: grid;
	place-items: center;
}

.wf-gemini-task-thumb {
	width: 100%;
	height: 100%;
	object-fit: contain;
	display: block;
}

.wf-gemini-task-progress-track {
	flex: 1;
	height: 6px;
	background: rgb(255 255 255 / 0.08);
	overflow: hidden;
}

.wf-gemini-task-progress-fill {
	height: 100%;
	background: linear-gradient(90deg, #8b5cf6, #34d399);
}

.wf-gemini-task-detail-mask {
	position: absolute;
	inset: 0;
	z-index: 4;
	display: flex;
	justify-content: flex-end;
	background: rgba(6, 10, 18, 0.42);
}

.wf-gemini-task-detail {
	width: min(440px, 100%);
	height: 100%;
	display: flex;
	flex-direction: column;
	border-left: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	background: linear-gradient(
		180deg,
		rgb(from var(--dweb-defualt-dark) r g b / 0.96),
		rgb(from var(--dweb-defualt) r g b / 0.9)
	);
}

.wf-gemini-task-detail-header {
	display: flex;
	align-items: center;
	justify-content: space-between;
	gap: 10px;
	padding: 12px;
	border-bottom: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
}

.wf-gemini-task-detail-title-wrap {
	min-width: 0;
	display: grid;
	gap: 4px;
}

.wf-gemini-task-detail-title {
	font-size: 14px;
	color: var(--vscode-fg);
}

.wf-gemini-task-detail-subtitle,
.wf-gemini-task-detail-loading,
.wf-gemini-task-detail-empty,
.wf-gemini-task-detail-label {
	font-size: 12px;
	color: var(--vscode-fg-muted);
}

.wf-gemini-task-detail-loading,
.wf-gemini-task-detail-empty {
	padding: 14px 12px;
}

.wf-gemini-task-detail-body {
	flex: 1;
	min-height: 0;
	overflow: auto;
	display: grid;
	gap: 12px;
	padding: 12px;
}

.wf-gemini-task-detail-thumb-section {
	display: grid;
	place-items: center;
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.58);
	padding: 10px;
}

.wf-gemini-task-detail-thumb {
	max-width: 100%;
	max-height: 240px;
	object-fit: contain;
}

.wf-gemini-task-detail-images {
	display: flex;
	gap: 8px;
	flex-wrap: wrap;
}

.wf-gemini-task-detail-image-item {
	width: 64px;
	height: 64px;
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	overflow: hidden;
}

.wf-gemini-task-detail-image-thumb {
	width: 100%;
	height: 100%;
	object-fit: cover;
}

.wf-gemini-task-detail-grid {
	display: grid;
	grid-template-columns: repeat(2, minmax(0, 1fr));
	gap: 10px;
}

.wf-gemini-task-detail-card,
.wf-gemini-task-detail-block,
.wf-gemini-task-detail-code {
	border: 1px solid rgb(from var(--vscode-border) r g b / 0.85);
	background: rgb(from var(--dweb-defualt-dark) r g b / 0.58);
}

.wf-gemini-task-detail-card {
	padding: 10px;
	display: grid;
	gap: 6px;
}

.wf-gemini-task-detail-value {
	font-size: 12px;
	color: var(--vscode-fg);
	word-break: break-word;
}

.wf-gemini-task-detail-value.monospace,
.wf-gemini-task-detail-block.monospace,
.wf-gemini-task-detail-code {
	font-family: Consolas, 'Courier New', monospace;
}

.wf-gemini-task-detail-value.highlight {
	color: #c4b5fd;
	font-weight: 500;
}

.wf-gemini-task-detail-section {
	display: grid;
	gap: 6px;
}

.wf-gemini-task-detail-block {
	padding: 10px;
	font-size: 12px;
	line-height: 1.5;
	color: var(--vscode-fg);
	word-break: break-word;
}

.wf-gemini-task-detail-block.error {
	color: #fecaca;
}

.wf-gemini-task-detail-links {
	display: grid;
	gap: 6px;
}

.wf-gemini-task-detail-code {
	margin: 0;
	padding: 10px;
	overflow: auto;
	font-size: 11px;
	line-height: 1.55;
	color: #ddd6fe;
	white-space: pre-wrap;
}

@media (max-width: 720px) {
	.wf-gemini-task-main {
		grid-template-columns: 1fr;
	}

	.wf-gemini-task-thumb-shell {
		width: 100%;
		height: 120px;
	}

	.wf-gemini-task-detail {
		width: 100%;
	}

	.wf-gemini-task-detail-grid {
		grid-template-columns: 1fr;
	}
}
</style>
