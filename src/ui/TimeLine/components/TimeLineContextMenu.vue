<template>
	<div v-if="visible" ref="menuEl" class="tl-menu" :style="menuStyle" @contextmenu.prevent>
		...
		<button
			...
			@click="emit('remove-keyframe')"
		>
			取消关键帧
		</button>
		<div class="tl-menu-sep" />
		<button class="tl-menu-item" type="button" :disabled="!canCopy" @click="emit('copy')">
			复制帧
		</button>
		<button class="tl-menu-item" type="button" :disabled="!canPaste" @click="emit('paste')">
			粘贴帧
		</button>
		<div class="tl-menu-sep" />
		<button
			class="tl-menu-item"
			type="button"
			:disabled="!canEnableEasing"
			@click="emit('enable-easing')"
		>
			开启缓动效果
		</button>
		<button
			class="tl-menu-item"
			type="button"
			:disabled="!canDisableEasing"
			@click="emit('disable-easing')"
		>
			关闭缓动效果
		</button>
		<button
			class="tl-menu-item"
			type="button"
			:disabled="!canEditEasingCurve"
			@click="emit('edit-easing-curve')"
		>
			编辑缓动曲线
		</button>
	</div>
...
watch(
	() =>
		[
			...
			props.x,
			props.y,
			props.canAddKeyframe,
			props.canRemoveKeyframe,
			props.canCopy,
			props.canPaste,
			props.canEnableEasing,
			props.canDisableEasing,
			props.canEditEasingCurve
		] as const,
	() => {
