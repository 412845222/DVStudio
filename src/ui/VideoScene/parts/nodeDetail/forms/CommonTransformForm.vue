					...
					@pointerdown="
						(e: PointerEvent) =>
							onNumberScrubPointerDown(
								...
								() => draft.x,
								(v) => (draft.x = v),
								{ step: 1, min: -999999, max: 999999, onCommit: applyTransform }
							)
					"
				/>
			</label>
					...
					@pointerdown="
						(e: PointerEvent) =>
							onNumberScrubPointerDown(
								...
								() => draft.scaleX,
								(v) => (draft.scaleX = v),
								{ step: 0.01, min: 0, max: 100, onCommit: applyTransform }
							)
					"
				/>
			</label>
					...
					@pointerdown="
						(e: PointerEvent) =>
							onNumberScrubPointerDown(
								...
								() => draft.width,
								(v) => (draft.width = v),
								{ step: 1, min: 1, max: 999999, onCommit: applyTransform }
							)
					"
				/>
			</label>
					...
					@pointerdown="
						(e: PointerEvent) =>
							onNumberScrubPointerDown(
								...
								() => draft.opacity,
								(v) => (draft.opacity = v),
								{ step: 0.01, min: 0, max: 1, onCommit: applyTransform }
							)
					"
				/>
			</label>
				...
				<div class="vs-quick">
					<button class="vs-quick-btn" type="button" title="靠左" @click="applyQuick('left')">
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M5 4v16H3V4h2zm16 3v2H9V7h12zm0 8v2H9v-2h12z" />
						</svg>
					</button>
					<button class="vs-quick-btn" type="button" title="靠右" @click="applyQuick('right')">
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M21 4v16h-2V4h2zM15 7v2H3V7h12zm0 8v2H3v-2h12z" />
						</svg>
					</button>
					<button
						class="vs-quick-btn"
						type="button"
						title="水平居中"
						@click="applyQuick('hcenter')"
					>
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M12 3h1v18h-1V3zm8 6v2H4V9h16zm0 4v2H4v-2h16z" />
						</svg>
					</button>
					<button
						class="vs-quick-btn"
						type="button"
						title="垂直居中"
						@click="applyQuick('vcenter')"
					>
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M3 12h18v1H3v-1zM8 4h8v6H8V4zm0 10h8v6H8v-6z" />
						</svg>
					</button>
					<button class="vs-quick-btn" type="button" title="宽度填充" @click="applyQuick('fillW')">
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M4 6h2v12H4V6zm14 0h2v12h-2V6zM7 9h10v6H7V9z" />
						</svg>
					</button>
					<button class="vs-quick-btn" type="button" title="高度填充" @click="applyQuick('fillH')">
						<svg viewBox="0 0 24 24" aria-hidden="true">
							<path d="M6 4h12v2H6V4zm0 14h12v2H6v-2zM9 7h6v10H9V7z" />
						</svg>
					</button>
				</div>
			</label>
