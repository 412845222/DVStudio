<template>
  <div ref="rootEl" class="vs-toolbar" @pointerdown.stop>
    ...
    <button class="vs-tool-btn" type="button" @click="onImportComponent">
      导入高级组件
    </button>
    <button
      ref="aiBtnRef"
    ...
    <button class="vs-tool-btn" type="button" title="保存 (Ctrl+S)" @click="save">
      保存
    </button>
  </div>
</template>
...
const onImportSubtitleFile = async (e: Event) => {
	...
	try {
		...
		if (!layer) return
		await TimelineStore.dispatch('setSubtitleTrack', { layerId: layer.id, cues, cueRanges, spans, fps: importFps })
