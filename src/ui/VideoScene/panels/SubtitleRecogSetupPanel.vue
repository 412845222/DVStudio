<template>
	<div class="subtitle-recog-setup">
			...
			<div v-if="currentStep?.id === 'overview'" class="step-panel">
				...
				<div class="env-check-list">
					<div
						...
						:class="{ ok: envStatus?.ffmpeg?.ok, error: !envStatus?.ffmpeg?.ok }"
					>
					...
					<div
						class="env-check-item"
						:class="{ ok: envStatus?.binary?.ok, error: !envStatus?.binary?.ok }"
					>
					...
					<div
						class="env-check-item"
						:class="{ ok: envStatus?.defaultModel, error: !envStatus?.defaultModel }"
					>
				...
				<div class="step-note">所有组件将下载到本地用户数据目录，不会随安装包分发。</div>
			</div>
				...
				<div v-else class="status-error">
					...
					<div class="install-hint">
						...
						<ol>
							<li>
								Windows: 从
								<a href="#" @click.prevent="openFfmpegGuide">ffmpeg.org</a>
								下载，或使用 winget:
					...
					<div class="download-section">
						...
						<div v-if="downloadingBinary || binaryDownloadProgress > 0" class="progress-section">
							...
							<div class="progress-text">
								{{ binaryDownloadMessage || `${binaryDownloadProgress}%` }}
							</div>
						</div>
		...
		<div class="setup-footer">
			...
			<button v-if="currentStep?.id === 'overview'" class="vs-btn primary" @click="startSetup">
				开始配置
			</button>
			<button
			...
			<button v-if="currentStep?.id === 'done'" class="vs-btn primary" @click="$emit('close')">
				关闭
			</button>
		</div>
	</div>
