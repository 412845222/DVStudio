		...
		<div class="m3de-panel-header">
			<div v-if="selectedObject" class="m3de-selected-name">
				<span class="m3de-selected-icon" :class="selectedObject.type">
					...
					<svg
						v-else-if="selectedObject.type === 'mesh'"
						viewBox="0 0 16 16"
						width="12"
						height="12"
					>
						<polygon
							points="8,2 14,7 12,14 4,14 2,7"
							fill="none"
							stroke="currentColor"
							stroke-width="1.2"
						/>
				...
				<span class="m3de-selected-name-text" :title="selectedObject.name">
					{{ selectedObject.name }}
				</span>
			</div>
			<div class="m3de-tabs">
...
const onPosXInput = (e: Event) => {
	posX.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onPosYInput = (e: Event) => {
	posY.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onPosZInput = (e: Event) => {
	posZ.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onRotXInput = (e: Event) => {
	rotX.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onRotYInput = (e: Event) => {
	rotY.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onRotZInput = (e: Event) => {
	rotZ.value = parseFloat((e.target as HTMLInputElement).value) || 0
}
const onSclXInput = (e: Event) => {
	sclX.value = parseFloat((e.target as HTMLInputElement).value) || 1
}
const onSclYInput = (e: Event) => {
	sclY.value = parseFloat((e.target as HTMLInputElement).value) || 1
}
const onSclZInput = (e: Event) => {
	sclZ.value = parseFloat((e.target as HTMLInputElement).value) || 1
}
...
.m3de-selected-name {
	...
	border-bottom: 1px solid
		color-mix(in srgb, var(--wf-primary) 10%, var(--wf-border-subtle, transparent));
}
...
.m3de-selected-icon.model {
	color: var(--wf-primary);
}
.m3de-selected-icon.mesh {
	color: #6ea8d8;
}
.m3de-selected-icon.group {
...
.m3de-properties-content::-webkit-scrollbar {
	width: 5px;
}
.m3de-properties-content::-webkit-scrollbar-track {
	background: transparent;
}
.m3de-properties-content::-webkit-scrollbar-thumb {
	background: color-mix(in srgb, var(--wf-primary) 25%, transparent);
}
.m3de-properties-content::-webkit-scrollbar-thumb:hover {
...
.m3de-prop-label.axis-x {
	color: #ff5555;
	text-shadow: 0 0 4px #ff555566;
}
.m3de-prop-label.axis-y {
	color: #55ff55;
	text-shadow: 0 0 4px #55ff5566;
}
.m3de-prop-label.axis-z {
	color: #5588ff;
...
.m3de-vector-input input {
	...
	border: 1px solid
		color-mix(in srgb, var(--wf-primary) 20%, var(--wf-input-border, rgba(255, 255, 255, 0.08)));
	color: var(--wf-text);
	font-size: 11px;
	font-family: 'Consolas', 'Monaco', monospace;
	outline: none;
...
.m3de-vector-input input:focus {
	border-color: color-mix(
		in srgb,
		var(--wf-primary) 50%,
		var(--wf-input-focus-border, transparent)
	);
...
.m3de-toggle-track {
	...
	border: 1px solid
		color-mix(in srgb, var(--wf-primary) 25%, var(--wf-border-subtle, rgba(255, 255, 255, 0.1)));
	position: relative;
	transition: all 160ms ease;
}
