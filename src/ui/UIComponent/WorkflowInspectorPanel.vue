				...
				<div v-if="selectedNode" class="wf-config">
					...
					<div v-if="isMediaNode" class="wf-media-config">
						...
						<div class="wf-media-row">
							...
							<div class="wf-media-actions">
								<button class="wf-media-btn" type="button" @click="onUploadClick">上传</button>
								<button
					...
					<div v-if="isStoryNode" class="wf-story-config">
						<div class="wf-story-header">
							...
							<button class="wf-story-add" type="button" @click="onAddBranch">新增</button>
						</div>
						<div
						...
						>
							...
							/>
							<button class="wf-story-remove" type="button" @click="onRemoveBranch(branch.id)">
								删除
							</button>
						</div>
					</div>
...
const meshyHelp = computed(() => {
	...
	const rawTarget = String(settings.meshyTaskTarget ?? '3d').trim() === 'image' ? 'image' : '3d'
	const rawModelType =
		String(settings.meshyModelType ?? 'standard').trim() === 'lowpoly' ? 'lowpoly' : 'standard'
	...
	if (rawFamily === 'refine') {
		return {
			...
			inputs: ['必须提供 Preview Task ID。', '仍然建议提供提示词或负向提示词，帮助控制细化方向。'],
			outputs: [
	...
	if (rawFamily === 'remesh' || rawFamily === 'retexture') {
		return {
			...
			outputs: ['设计目标仍然是 out-model 输出。', '当前不会真正发起 remesh / retexture 请求。'],
			statusNotes: ['独立代理接口尚未接入。', '这部分属于当前 Meshy 业务闭环里的明确剩余项。']
		}
	}
...
const fileAccept = computed(() => (props.selectedNode?.type === 'video' ? 'video/*' : 'image/*'))
