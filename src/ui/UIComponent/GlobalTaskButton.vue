<template>
	...
	>
		...
		<span v-if="activeCount > 0" class="task-count-badge">
...
const overallProgress = computed(() =>
	Math.round(TaskQueueStore.state.summary.overallProgress || 0)
)
const panelVisible = computed(() => TaskQueueStore.state.panelVisible)

const buttonTitle = computed(() => {
	if (activeCount.value > 0) {
		return t('taskQueue.runningCount', {
...
.task-queue-btn {
	...
	border: 1px solid
		color-mix(in srgb, var(--theme-accent, #1f9d84) 25%, var(--theme-border, #3c3c3c));
	border-radius: 2px;
...
.task-queue-btn.panel-open {
	...
	box-shadow:
		0 0 14px color-mix(in srgb, var(--theme-accent, #1f9d84) 22%, transparent),
