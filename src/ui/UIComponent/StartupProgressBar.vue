...
const completedCount = computed(() =>
  props.state.steps.filter((s) => s.status === 'ok').length
)
...
const hasFailure = computed(() =>
  props.state.steps.some((s) => s.status === 'error')
)
...
<template>
  <div v-if="localVisible" class="startup-progress-bar" role="status" aria-live="polite">
    <div class="startup-progress-bar-header">
      ...
      <button class="startup-progress-bar-close" type="button" @click="localVisible = false; $emit('dismiss')" aria-label="关闭">
        ■
      </button>
    </div>
    ...
    <ul class="startup-progress-bar-steps">
      <li v-for="step in state.steps" :key="step.key" class="startup-progress-bar-step" :class="'is-' + step.status">
        <span class="startup-progress-bar-step-icon" aria-hidden="true">{{ statusIcon(step.status) }}</span>
