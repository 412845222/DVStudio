...
export const addNodeTreeToLayer = (args: {
	...
	const normalizeNodeTreeInPlace = (n: LegacyNode) => {
		...
		if (typeof n.createdAt !== 'number' || !Number.isFinite(n.createdAt as number)) (n as Record<string, unknown>).createdAt = Date.now()
		if (typeof n.name !== 'string' || !n.name.trim()) (n as Record<string, unknown>).name = 'Node'
		if (n.category !== 'user' && n.category !== 'project') (n as Record<string, unknown>).category = 'user'

		if (n.category === 'user') {
			if (!n.transform || typeof n.transform !== 'object') {
				(n as Record<string, unknown>).transform = { x: 0, y: 0, scaleX: 1, scaleY: 1, pivotX: 0.5, pivotY: 0.5, width: 200, height: 120, rotation: 0, opacity: 1 }
			} else {
				...
				;(n as Record<string, unknown>).transform = {
					...
					scaleY: sy,
					scale: legacyScale,
					pivotX: typeof t.pivotX === 'number' && Number.isFinite(t.pivotX) ? Math.max(0, Math.min(1, t.pivotX)) : 0.5,
					pivotY: typeof t.pivotY === 'number' && Number.isFinite(t.pivotY) ? Math.max(0, Math.min(1, t.pivotY)) : 0.5,
					...
					opacity: typeof t.opacity === 'number' && Number.isFinite(t.opacity) ? Math.max(0, Math.min(1, t.opacity)) : 1,
				}
			}
			if (!n.props || typeof n.props !== 'object' || Array.isArray(n.props)) (n as Record<string, unknown>).props = {}
			...
			if (userTypeStr === 'text') {
				...
				Object.assign(n.props as object, normalizeLineLocalPoints({ props: propsRecord, width: tr?.width ?? 200, height: tr?.height ?? 120 }))
			}
		}
...
const normalizeTransformPatch = (prev: VideoSceneNodeTransform, patch: NodeTransformPatch): VideoSceneNodeTransform => {
	...
	const scaleX = typeof patch.scaleX === 'number' && Number.isFinite(patch.scaleX) ? clampScale(patch.scaleX, 1) : prev.scaleX ?? legacyScale
	const scaleY = typeof patch.scaleY === 'number' && Number.isFinite(patch.scaleY) ? clampScale(patch.scaleY, 1) : prev.scaleY ?? legacyScale
	const pivotX = typeof patch.pivotX === 'number' && Number.isFinite(patch.pivotX) ? Math.max(0, Math.min(1, patch.pivotX)) : prev.pivotX
	const pivotY = typeof patch.pivotY === 'number' && Number.isFinite(patch.pivotY) ? Math.max(0, Math.min(1, patch.pivotY)) : prev.pivotY
	...
	const opacity = typeof patch.opacity === 'number' && Number.isFinite(patch.opacity) ? Math.max(0, Math.min(1, patch.opacity)) : prev.opacity
	return { x, y, scaleX, scaleY, scale: legacyScale, pivotX, pivotY, width, height, rotation, opacity }
}
...
export const updateUserNodeTransform = (layer: VideoSceneLayer, nodeId: string, patch: NodeTransformPatch): boolean => {
	...
	const prev: VideoSceneNodeTransform = node.transform ?? { x: 0, y: 0, scaleX: 1, scaleY: 1, pivotX: 0.5, pivotY: 0.5, width: 10, height: 10, rotation: 0, opacity: 1 }
...
export const setUserNodeType = (layer: VideoSceneLayer, nodeId: string, type: VideoSceneUserNodeType): boolean => {
	...
	const tr: VideoSceneNodeTransform = (node.transform ??= { x: 0, y: 0, scaleX: 1, scaleY: 1, pivotX: 0.5, pivotY: 0.5, width: 10, height: 10, rotation: 0, opacity: 1 })
