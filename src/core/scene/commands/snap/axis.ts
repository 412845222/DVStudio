...
export function applySnapAxis(
	...
	if (axis === 'x' && lock?.x) {
		...
		const next =
			lock.x.mode === 'l' ? line + size / 2 : lock.x.mode === 'r' ? line - size / 2 : line
	...
	if (axis === 'y' && lock?.y) {
		...
		const next =
			lock.y.mode === 't' ? line + size / 2 : lock.y.mode === 'b' ? line - size / 2 : line
		const dist = Math.abs(next - center)
	...
	if (!best)
		return {
			center,
			snappedLine: null as number | null,
			lock: null,
	...
	const next =
		best.mode === 'l' || best.mode === 't'
			...
			: best.mode === 'r' || best.mode === 'b'
				...
				: best.line
	if (axis === 'x') {
		const mode: SnapModeX =
			best.mode === 'l' || best.mode === 'c' || best.mode === 'r' ? best.mode : 'c'
		return {
			center: next,
	...
	const mode: SnapModeY =
		best.mode === 't' || best.mode === 'c' || best.mode === 'b' ? best.mode : 'c'
	return {
		center: next,
		snappedLine: best.line,
		lock: { target: best.line, mode },
