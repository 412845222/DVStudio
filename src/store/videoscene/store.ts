...
export const VideoSceneStore = createStore<VideoSceneState>({
	...
	mutations: {
		...
		addNodeTree(
			state,
			payload: { layerId: string; node: VideoSceneTreeNode; parentId?: string | null }
		) {
			const layer = findLayer(state, payload.layerId)
			if (!layer) return
		...
		setLayoutInsets(state, payload: Partial<VideoSceneLayoutInsets>) {
			state.layoutInsets = {
				...
				bottomToolbarHeight: clampPx(
					payload.bottomToolbarHeight,
					state.layoutInsets.bottomToolbarHeight
				)
			}
		},
		...
		addRenderableNode(
			state,
			payload: { layerId: string; type: VideoSceneUserNodeType; parentId?: string | null }
		) {
			const layer = findLayer(state, payload.layerId)
			if (!layer) return
			const r = addRenderableNodeToLayer({
				state,
				layerId: payload.layerId,
				type: payload.type,
				parentId: payload.parentId
		...
		updateNodeTransform(
			state,
			payload: { layerId: string; nodeId: string; patch: Partial<VideoSceneNodeTransform> }
		) {
			const layer = findLayer(state, payload.layerId)
			if (!layer) return
		...
		updateNodesPropsBatch(
			state,
			payload: { layerId: string; nodeIds: string[]; patch: NodePropsPatch }
		) {
			const layer = findLayer(state, payload.layerId)
			if (!layer) return
		...
		patchNodeById(
			...
			const explicitLayerId =
				typeof payload.layerId === 'string' && payload.layerId.trim() ? payload.layerId.trim() : ''
			const findLayerId = () => {
				if (explicitLayerId) {
	...
	actions: {
		...
		addRenderableNode(
			{ commit, state },
			payload: { type: VideoSceneUserNodeType; layerId?: string; parentId?: string | null }
		) {
			commit('addRenderableNode', {
				layerId: payload.layerId ?? state.activeLayerId,
				type: payload.type,
				parentId: payload.parentId
			})
		...
		setNodeType(
			{ commit, state },
			payload: { nodeId: string; type: VideoSceneUserNodeType; layerId?: string }
		) {
			commit('setNodeType', {
				layerId: payload.layerId ?? state.activeLayerId,
				nodeId: payload.nodeId,
		...
		updateNodeProps(
			{ commit, state },
			payload: { nodeId: string; patch: NodePropsPatch; layerId?: string }
		) {
			commit('updateNodeProps', {
				layerId: payload.layerId ?? state.activeLayerId,
				nodeId: payload.nodeId,
				patch: payload.patch
			})
		...
		updateNodesPropsBatch(
			{ commit, state },
			payload: { nodeIds: string[]; patch: NodePropsPatch; layerId?: string }
		) {
			commit('updateNodesPropsBatch', {
				layerId: payload.layerId ?? state.activeLayerId,
