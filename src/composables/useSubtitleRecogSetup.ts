...
export function useSubtitleRecogSetup() {
  ...
  const steps = ref<SetupStep[]>([
    ...
    { id: 'binary', title: '安装 Whisper 引擎', description: '下载 Whisper.cpp 二进制', status: 'pending' },
  ...
  async function loadModelConfig(size?: SubtitleRecogModelSize) {
    ...
    modelConfig.value = await dweb.subtitleRecog.getModelConfig({ size: modelSize, useMirror: useMirror.value })
    return modelConfig.value
  }

  async function* downloadModel(size?: SubtitleRecogModelSize): AsyncGenerator<SubtitleRecogProgressChunk, void, void> {
    const modelSize = size || selectedModelSize.value
    ...
    try {
      const generator = dweb.subtitleRecog.downloadModel({ size: modelSize, useMirror: useMirror.value })
      for await (const chunk of generator) {
        if (chunk.type === 'progress') {
